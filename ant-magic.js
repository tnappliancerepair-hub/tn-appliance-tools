...
.ant-candle{position:absolute;pointer-events:none;font-size:19px;filter:drop-shadow(0 0 8px rgba(255,190,80,.7));animation:antFloat 4s ease-in-out infinite}\
.ant-candle .fl{position:absolute;top:-9px;left:5px;font-size:11px;animation:antFlicker 1.6s ease-in-out infinite}\
.ant-pts-badge{position:relative;z-index:4;display:inline-flex;align-items:center;gap:5px;font:800 13px/1 "Cinzel",Georgia,serif;color:#241a10;background:linear-gradient(180deg,#e7c65a,#b8860b);border:1px solid #8a6508;border-radius:999px;padding:6px 12px;box-shadow:0 2px 6px rgba(0,0,0,.35);cursor:pointer;white-space:nowrap;user-select:none}\
.ant-pts-float{position:fixed;z-index:2147482002;pointer-events:none;font:900 15px/1 "Cinzel",Georgia,serif;color:#e7c65a;text-shadow:0 1px 3px rgba(0,0,0,.6);animation:antPts 1.1s ease-out forwards}';
  ...
  function ambient() {
    ...
    spots.forEach(function (p, i) {
      var cd = document.createElement('div'); cd.className = 'ant-candle'; cd.innerHTML = '🕯️<span class="fl">🔥</span>';
  ...
  var API = {
    ...
    owl: function () {
      ...
      var el = document.createElement('div'); el.className = 'ant-owl'; el.innerHTML = '🦉<span class="env">✉️</span>';
