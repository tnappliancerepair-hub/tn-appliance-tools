// build-la-vent-hubs.js — fresh, UNIQUE dryer-vent-cleaning landers for the
// Louisiana markets (North Shore, Greater New Orleans, Baton Rouge area) + a
// Louisiana regional hub. Each city page carries genuinely local content
// (housing stock, parish, humidity, the tech who runs it) so it's not a thin
// doorway lander — that's what actually indexes. Booking form funnels to the
// same /book-repair endpoint the main vent page uses.
//
// Idempotent: overwrites its own output files each run.
//   node tools/seo-build/build-la-vent-hubs.js
'use strict';
const fs = require('fs');
...
// hood = neighborhoods; the copy is written per city, not templated.
const CITIES = [
  { slug: 'new-orleans', name: 'New Orleans', st: 'LA', parish: 'Orleans Parish', region: 'the South Shore', tech: 'Andre',
    factor: "New Orleans homes run the gamut — raised shotguns and doubles Uptown, historic doubles in Mid-City and the Marigny, and slab homes out in New Orleans East. A lot of them push the dryer's exhaust through a long interior run or straight up through the roof, and in this climate that run packs with damp lint fast.",
    hoods: 'Uptown, Mid-City, Lakeview, the Marigny, Gentilly, Algiers and New Orleans East' },
  { slug: 'metairie', name: 'Metairie', st: 'LA', parish: 'Jefferson Parish', region: 'the South Shore', tech: 'Andre',
    factor: "Most Metairie homes are slab-built with the laundry set well inside the house, so the dryer duct travels a long way to the exterior wall before it ever gets outside. The longer the run, the more lint it traps — and the harder your dryer has to work.",
    hoods: 'Old Metairie, Bucktown, Fat City and along Veterans Memorial' },
  { slug: 'kenner', name: 'Kenner', st: 'LA', parish: 'Jefferson Parish', region: 'the South Shore', tech: 'Andre',
    factor: "Kenner's mix of slab ranches and two-story homes near the lake often run the dryer vent through a long side-wall or roof path. Between the humidity off Lake Pontchartrain and everyday use, those runs clog quietly until drying slows down.",
    hoods: 'Rivertown, University City, Chateau Estates and the Esplanade area' },
  { slug: 'chalmette', name: 'Chalmette', st: 'LA', parish: 'St. Bernard Parish', region: 'the South Shore', tech: 'Andre',
    factor: "A lot of Chalmette and St. Bernard homes are post-storm rebuilds — newer construction, but plenty of them tucked the laundry room deep inside with a long duct to daylight. New house, same lint problem when the run is long and humid.",
    hoods: 'Chalmette, Arabi, Meraux and Violet' },
  { slug: 'hammond', name: 'Hammond', st: 'LA', parish: 'Tangipahoa Parish', region: 'the North Shore', tech: 'John',
    factor: "Hammond runs from older homes near downtown and Southeastern to newer subdivisions off the interstate. The two-story houses especially tend to run the dryer exhaust a long way up and out — a run that clogs faster than most folks expect in this humidity.",
    hoods: 'downtown Hammond, the Southeastern area, Ponchatoula Road and the I-12 subdivisions' },
  { slug: 'mandeville', name: 'Mandeville', st: 'LA', parish: 'St. Tammany Parish', region: 'the North Shore', tech: 'John',
    factor: "Mandeville is full of larger two- and three-story homes on wooded lots, and those homes usually vent the dryer up through a long roof run. Long roof-exit runs are exactly where lint packs in — and exactly the runs most vent crews would rather not climb. We do.",
    hoods: 'Old Mandeville, the lakefront, Beau Chêne and the subdivisions off Highway 190' },
  { slug: 'covington', name: 'Covington', st: 'LA', parish: 'St. Tammany Parish', region: 'the North Shore', tech: 'John',
    factor: "Covington's wooded North Shore lots and roomy two-story homes mean long dryer-vent runs, often exiting through the roof. Shade, humidity and a long duct add up to lint buildup you can't see until the dryer starts taking two cycles.",
    hoods: 'downtown Covington, Tchefuncte, Terra Bella and the Highway 21 corridor' },
  { slug: 'ponchatoula', name: 'Ponchatoula', st: 'LA', parish: 'Tangipahoa Parish', region: 'the North Shore', tech: 'John',
    factor: "Ponchatoula's mix of older strawberry-country homes and newer builds often carries the dryer vent through a long or roof-exit run. In this humidity, that run traps damp lint quickly — the quiet reason a good dryer suddenly can't keep up.",
    hoods: 'downtown Ponchatoula, Hammond-adjacent subdivisions and the rural stretches off Highway 22' },
  { slug: 'slidell', name: 'Slidell', st: 'LA', parish: 'St. Tammany Parish', region: 'the North Shore', tech: 'John',
    factor: "Slidell homes near the lake and out toward Lacombe deal with heavy humidity and salt air, and many run a long dryer duct to an exterior or roof vent. That combination cakes lint faster than a drier climate ever would.",
    hoods: 'Olde Towne Slidell, Eden Isles, Northshore and the Fremaux area' },
  { slug: 'baton-rouge', name: 'Baton Rouge', st: 'LA', parish: 'East Baton Rouge Parish', region: 'the Baton Rouge area', tech: 'John',
    factor: "Baton Rouge spans everything from older homes near LSU and Mid City to sprawling newer subdivisions in the south and east. The bigger two-story homes route the dryer exhaust a long way up and out, and Louisiana humidity turns that long run into a lint trap.",
    hoods: 'Mid City, the LSU area, the Garden District, Shenandoah and the south Baton Rouge subdivisions' },
  { slug: 'denham-springs', name: 'Denham Springs', st: 'LA', parish: 'Livingston Parish', region: 'the Baton Rouge area', tech: 'John',
    factor: "Denham Springs and Livingston Parish saw a wave of rebuilds and new construction, and many of those homes run the laundry room well inside with a long duct to the outside wall. New house or old, a long humid run still clogs.",
    hoods: 'Denham Springs, Watson, Walker-adjacent subdivisions and the Juban Road corridor' },
  { slug: 'gonzales', name: 'Gonzales', st: 'LA', parish: 'Ascension Parish', region: 'the Baton Rouge area', tech: 'John',
    factor: "Ascension Parish is one of the fastest-growing corners of Louisiana, and Gonzales is full of newer two-story subdivisions. Those homes almost always run the dryer vent up and out a long way — a fresh build with a lint problem waiting to happen if it's never cleaned.",
    hoods: 'Gonzales, Prairieville-adjacent subdivisions and the Highway 44 area' },
];
...
function pageHtml(c, others) {
  ...
  const faq = [
    { q: `Do you clean dryer vents in ${c.name}?`, a: `Yes — dryer vent cleaning in ${c.name} and across ${c.parish} is exactly what we do. We're CSIA Certified Dryer Exhaust Technicians (C-DET), we price-match any licensed competitor, and we can often get you cleaned out the same day.` },
    { q: `Can you clean my ${c.name} dryer vent today?`, a: `Often yes. Text or call and we'll get right back to you with the soonest opening — usually same-day in ${c.name} and ${c.region}.` },
  ...
  const bread = { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Louisiana', item: SITE + '/dryer-vent-cleaning-louisiana.html' },
...
<meta property="og:description" content="${esc(`The dryer vent cleaning people for ${c.name} and ${c.region}. Same-day, price-match, CSIA-certified — and we clean the dryer itself, not just the vent.`)}">
...
<div id="form-wrap">
  ...
  <div class="cred"><span class="seal">💵</span> $80 books a full inspection + a 4-ft vent cleaning included</div>
  ...
  <div style="text-align:center;font-size:12.5px;color:#8a93a5;margin-bottom:14px;line-height:1.6">Tell us your setup, pick when you're free, $80 locks it in. Past the first 4 ft we quote <b style="color:#c9d2e0">by the foot &amp; by the corner</b> — your floor and any complexities factor in. <b style="color:#c9d2e0">Annual plan members save.</b></div>
  ...
  <div class="local">
    <b>${c.name} homes, ${c.name} vents.</b> ${c.factor} We know the ${c.hoods} runs, and Louisiana's humidity makes it worse — damp lint clumps, holds moisture, and cakes the duct faster than a dry climate ever would. ${c.tech} runs ${c.region} for us and gets it cleared right.
  </div>
  ...
  <div class="card">
    ...
    <input id="phone" type="tel" placeholder="(985) 555-0199" autocomplete="tel" inputmode="tel">
...
function hubHtml() {
  ...
  const groups = Object.entries(byRegion).map(([region, list]) => {
    ...
    return `<h2 style="font-size:17px;color:#12203a;margin:20px 0 10px;text-transform:capitalize">${region.replace(/^the /, '')}</h2><div>${links}</div>`;
  }).join('');
  const svcLd = { '@context': 'https://schema.org', '@type': 'Service', serviceType: 'Dryer vent cleaning', areaServed: ['Southeast Louisiana', 'Greater New Orleans', 'North Shore Louisiana', 'Baton Rouge area'], provider: { '@type': 'LocalBusiness', name: 'TN Appliance Exchange', telephone: '+1-615-280-2949', url: SITE + '/', hasCredential: { '@type': 'EducationalOccupationalCredential', credentialCategory: 'certification', name: 'C-DET — Certified Dryer Exhaust Technician', recognizedBy: { '@type': 'Organization', name: 'Chimney Safety Institute of America (CSIA)', url: 'https://www.csia.org/' } } }, description: 'Same-day dryer vent cleaning across Southeast Louisiana — the North Shore, Greater New Orleans and the Baton Rouge area. CSIA C-DET certified, price-match guaranteed. We also break down and clean the dryer itself.' };
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
<title>Dryer Vent Cleaning Louisiana — North Shore, New Orleans, Baton Rouge</title>
<meta name="description" content="The dryer vent cleaning people for Southeast Louisiana — the North Shore, Greater New Orleans and the Baton Rouge area. Same-day, price-match guaranteed, CSIA C-DET certified. We're the only crew that also cleans the dryer itself.">
<link rel="canonical" href="${SITE}/dryer-vent-cleaning-louisiana.html">
<meta property="og:title" content="Dryer Vent Cleaning Louisiana — Same-Day, Price-Match">
<meta property="og:image" content="${SITE}/og-image.jpg">
<meta name="theme-color" content="#ff8a00">
<script async src="https://www.googletagmanager.com/gtag/js?id=G-0EF3THNXLE"></script>
<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','G-0EF3THNXLE');</script>
...
fs.writeFileSync(path.join(REPO, 'dryer-vent-cleaning-louisiana.html'), hubHtml());
console.log(`Built ${n} LA vent city pages + 1 Louisiana hub. Slugs: ${CITIES.map((c) => c.slug).join(', ')}`);
