...
exports.handler = async function (event) {
  ...
  const pool = await loadPool();
  pool.push({ key, author: String(b.author || '').slice(0, 80), stars: Number(b.stars) || 5, text: String(b.text || '').slice(0, 600), is_la: !!b.is_la, tech: String(b.tech || '').slice(0, 12) || null, towns: String(b.towns || '').slice(0, 80) || null, phone: String(b.phone || '').slice(0, 20) || null, spotlight: !!b.spotlight, fmt: b.fmt === 'story' ? 'story' : 'square', posted: false, added_ms: Date.now() });
  await savePool(pool);
