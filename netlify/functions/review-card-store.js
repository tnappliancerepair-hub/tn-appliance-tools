...
exports.handler = async function (event) {
  ...
  const pool = await loadPool();
  pool.push({ key, author: String(b.author || '').slice(0, 80), stars: Number(b.stars) || 5, text: String(b.text || '').slice(0, 600), is_la: !!b.is_la, fmt: b.fmt === 'story' ? 'story' : 'square', posted: false, added_ms: Date.now() });
  await savePool(pool);
