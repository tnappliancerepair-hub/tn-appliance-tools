...
// and AUTO-SAVES it to the vault as YOUTUBE_REFRESH_TOKEN. No paste, no redeploy.
'use strict';
const { getSecretPreferVault, setSecret } = require('./_lib/secrets');
const { REDIRECT, TOKEN_URL } = require('./_lib/youtube');
...
exports.handler = async function (event) {
  ...
  const code = q.code;
  if (!code) return { statusCode: 400, headers: { 'Content-Type': 'text/html' }, body: page('Missing authorization code', '<p>Start over at <code>/.netlify/functions/youtube-oauth-start</code>.</p>') };
  ...
  try {
    ...
    let saved = false;
    try { await setSecret('YOUTUBE_REFRESH_TOKEN', d.refresh_token); saved = true; } catch (_) {}
    return {
      statusCode: 200, headers: { 'Content-Type': 'text/html' },
      body: page('✅ YouTube connected',
        saved
          ? '<p><b>Done.</b> Ant can now upload the business\'s own videos to your channel (as private drafts you flip public in Studio). Verify: open <code>/.netlify/functions/youtube-check?secret=YOUR_ADMIN</code> — it should show your channel name.</p>'
          : ('<p>Connected, but auto-save failed. Add this to the vault as <b>YOUTUBE_REFRESH_TOKEN</b>:</p><textarea readonly style="width:100%;height:120px;font-family:monospace" onclick="this.select()">' + d.refresh_token + '</textarea>')),
    };
  } catch (err) {
