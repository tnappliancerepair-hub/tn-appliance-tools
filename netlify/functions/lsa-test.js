...
exports.handler = async function (event) {
  ...
  // ?legacy=1 hits the old Local Services accountReports endpoint (usually empty - the
  // LSA account is not under a manager we can reach). Default is the real report.
  if (q.legacy === '1') { try { return json(200, await lsa.accountReports(days)); } catch (e) { return json(200, { ok: false, error: String((e && e.message) || e) }); } }
  try { return json(200, await lsa.report(days)); }
