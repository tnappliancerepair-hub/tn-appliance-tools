...
exports.handler = async function (event) {
  ...
  if (action === 'primary_playbook') {
    ...
    const BLOCK = `${MARK}\n## 📞 WHAT ALMOST EVERY CALLER WANTS — recognize them, then handle it right (top priority)\n`
      ...
      + `Immediately call lookup_customer_by_phone for the caller ID. If it returns a customer AND caller_id_masked is false, greet them BY NAME to confirm: "Hi, is this Ms. {customer_last_name}?" Wait for yes/no. If it's not them, or masked/not found, ask who you're helping and find them by claim/work-order number (lookup_by_claim_number) or name + city (search_customers) — found:false is NORMAL for warranty callers, NEVER say "you're not in our system."\n`
