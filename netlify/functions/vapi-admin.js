...
function isBizHoursCT(d) {
  ...
  // 9:00 AM through 5:59 PM CT (Teddy 2026-09-15, confirmed close hour: "6").
  // MUST stay in lockstep with the office-texml ring-group gate and relay-to-tech,
...
`;
  ...
  // Phone hours (Teddy 2026-09-15): a live person answers Mon–Fri 9–6 CT ONLY —
  ...
  if (action === 'business_hours') {
    ...
    // did nothing, so the prompt kept quoting stale hours forever. Strip is global
  ...
  if (action === 'primary_playbook') {
    ...
    const BLOCK = `${MARK}\n## 📞 WHAT ALMOST EVERY CALLER WANTS — recognize them, then handle it right (top priority)\n`
      + `Getting the small facts right — their NAME, the right claim, the right day + tech — is how we earn trust. Getting it RIGHT matters more than having an answer: if you don't know, say "I don't know that" and find out — NEVER guess.\n\n`
      + `### STEP 1 — RECOGNIZE & GREET BY NAME (do this first, every call)\n`
      + `Immediately call lookup_customer_by_phone for the caller ID. If it returns a customer AND caller_id_masked is false, greet them BY NAME to confirm: "Hi, is this Ms. {customer_last_name}?" Wait for yes/no. If it's masked, not found, or not them, FIRST ask for their phone number — "No problem — what's the best phone number for your account?" — and run lookup_customer_by_phone on the number they give you (our line often hides the real caller ID, so the number they say usually finds them). If that still doesn't find them, ask for a claim / work-order number (lookup_by_claim_number) or their name + city (search_customers). found:false is NORMAL for warranty callers — NEVER say "you're not in our system."\n`
      + `Once confirmed, PROACTIVELY tell them what we already have, warmly, in one breath — the claim source (warranty_company), the appliance + their complaint, and whether they're scheduled: "Great — I see your claim from AHS for your washer that's not spinning, and it looks like we don't have you scheduled yet. Are you calling to get that scheduled?" (Use the job's warranty_company, appliance_type, and complaint fields. If they ARE already scheduled, tell them the day + tech per STEP 3 instead.) Then let their answer route you: schedule → STEP 2 · verify/when/who → STEP 3 · parts → STEP 4.\n`
      + `If you truly can't find them after phone AND claim AND name+city, take a callback (capture_callback) — never dead-end, never guess.\n\n`
      + `### STEP 2 — GET THEM SCHEDULED (they want on the schedule; we do NOT book on the phone)\n`
      + `Offer the intake TEXT first: "The fastest way is a link I'll text you — and the sooner you finish it, the sooner we get you booked. Just a short video of what it's doing, a photo of the model-number sticker so we bring the right part, and pick the days you're available. Then a quick waiver to sign so our tech can work in your home, and one optional add-on — fresh hoses or water line so you don't get a surprise leak, totally your call." Be warm about the add-on, people say yes to people they like — offer once, NEVER hard-sell, drop it instantly if they're not interested (it also documents the offer either way).\n`
...
'',
'## FIRST — KNOW WHO YOU ARE TALKING TO (do this before anything else)',
'At the very start of the call, SILENTLY call lookup_customer_by_phone with the caller number ({{customer.number}}) to see if they are already one of our customers. This single check is what keeps every answer ACCURATE.',
'- RETURNING CUSTOMER (an existing or recent job comes back): they are NOT a new lead. Do NOT pitch the hundred-dollar trip fee or the fifty-dollar Quick Check, and do NOT run the closing playbook. Warmly greet them by name — "Hey [name], good to hear from you, I have got your [appliance] repair right here" — then HELP them accurately: answer status with get_job_arrival_status / get_parts_status / get_schedule_history, move their day with initiate_customer_reschedule, or get a message to their tech with relay_to_tech. If a lookup is slow or comes back empty, take a callback with capture_callback — never guess. ONLY move into intake/booking if they bring up a DIFFERENT appliance that is not already on a job with us.',
'- NEW LEAD (found:false, or no active job on file): run the full closing flow below.',
'- WARRANTY COMPANY / CLAIM REP: handle per the warranty rules — confirm the claim, never quote a fee.',
'A paid customer calling back should hear "I have got your repair right here," never a sales pitch.',
'',
'## THE CALL FLOW',
...
'   - THE VALUE (say it with genuine belief — this is why saying yes is the smart, easy choice): You are handing them the EASIEST, FASTEST, and often CHEAPEST path to a working appliance. The fifty-dollar Quick Check is less money than a visit AND a faster answer — no waiting around for a tech. Lean into it warmly: "Honestly, the Quick Check is the easy button — it is less money, you get a real answer today, and if it is something simple we will even get you the exact part and point you the right way so you can knock it out yourself. Whatever is easiest and cheapest for YOU." We are DIY-FRIENDLY: we never gatekeep and never push a service call someone does not need — that honesty is exactly why people trust us. We have the answer to their problem, and we get them there the easy way.',
...
'- Promete de menos, entrega de más.',
    ...
    model.tools = tools;
    // Spanish line: lock only the EARS (transcriber → es) so it hears Spanish
    // accurately; keep the existing multilingual Cartesia sonic-2 voice, which
    // speaks Spanish from the Spanish text — the same way the main multilingual
    // line already does. (Forcing voice.language='es' triggers a Cartesia
    // voiceId validation whose listed IDs aren't accessible on this account.)
    const voiceOut = src.voice;
