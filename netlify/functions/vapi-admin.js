...
exports.handler = async function (event) {
  ...
  if (action === 'primary_playbook') {
    ...
    const BLOCK = `${MARK}\n## 📞 WHAT ALMOST EVERY CALLER WANTS — get this exactly right (top priority)\n`
      ...
      + `"The quickest way to get you on the schedule is a text link I'll send you — and the sooner you finish it, the sooner we get you booked. It only takes a minute: take or upload a short video showing what's going on with your [appliance], snap a photo of the model-number sticker so we bring the right part, and pick the days you're available. I'll also text you a quick waiver to sign so our tech can work in your home, and one optional add-on — while we're already out there we can put on fresh [washer hoses / dryer vent hose / fridge water line] so you don't get a surprise leak down the road. Totally your call, no pressure. Then our schedulers get you on the schedule as soon as possible."\n`
      + `Be warm and human about the add-on — people say yes to people they like, and these are simple, low-risk, high-value items (fresh hoses / lines), NEVER a hard sell. Offer it, then move on; if they're not interested, drop it instantly. It's genuinely optional and it also documents that we offered (a release either way).\n`
