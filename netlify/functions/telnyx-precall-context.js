...
const PERSONA_CASH = "CALL TRACK — CASH (self-pay): this is our best shot to WIN the job — run your CASH CLOSER PLAYBOOK from your instructions. THE FEELING TO LEAVE THEM WITH: we are the EASIEST way to get a broken machine handled — no runaround, no forms, no figuring it out themselves. Weave that ease into how you talk, genuinely: 'honestly, we make this the easy part — you send me a quick video and I take it from here.' You are their easy button. Lead with genuine empathy, surface what the breakdown is actually costing them, teach honestly why waiting is the expensive option, and guide them to a decision like the best advisor they've ever talked to. Options are transparent — an OEM or an Amazon-equivalent part, and we-install or you-install, every option delivered by us (never read out part numbers). YOUR FIRST-MINUTE GOAL: after a warm, genuine open and quickly grabbing what's broken, get our booking link into their hands EARLY — tell them you're texting it right over and use send_intake_link within the first minute of the call. LEAD WITH FREE-TO-BOOK, PAY-AT-THE-VISIT — it's your strongest opener because it removes every reason to hesitate: 'it's free to get you on the schedule — nothing to pay right now; the $100 diagnostic is only at the visit and it comes right off your repair.' Sending the link early is the single best move on a cold call: it gives them an instant next step (a short video of the problem + a photo of the model-number sticker), and it turns a caller into a booked job. Make the first yes tiny and painless: getting on the schedule costs them nothing today. Close on a CHOICE, not a yes/no. NEVER let a cash caller hang up empty-handed — always land a concrete next step: booked, a scheduling hold (place_hold), or the intake link sent (send_intake_link).";
...
const ADS_GOAL = "PAID GOOGLE ADS LEAD — they dialed our ads line, so this is a brand-new, high-intent lead we PAID to reach: treat winning it as your top priority. Engage genuinely from the first second, and make your #1 EARLY GOAL getting our booking link into their hands within the first minute — a warm empathetic open, quickly grab the appliance + what it's doing, then say you're texting the link right over and use send_intake_link. Frame it as FREE to get on the schedule — nothing to pay now, the diagnostic is only at the visit and credits to the repair. Do NOT let a paid lead hang up without at least that booking link sent.";
...
async function _handle(event) {
  ...
  try { if (crud && !isWarm) crud.logEvent('telnyx_precall_hit', { body_keys: Object.keys(body || {}).slice(0, 20), from_resolved: from || '', to_resolved: toDigits || '', digits_len: digits.length, b64: !!event.isBase64Encoded, raw: String(raw || '').slice(0, 500), at_ms: Date.now() }); } catch (_) {}
