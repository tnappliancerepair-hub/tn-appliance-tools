...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (action === 'ai') {
      ...
      let voices = {};
      for (const vp of ['/text-to-speech/voices', '/text-to-speech/voices?provider=cartesia', '/ai/assistants/voices']) { try { const r = await fetch(`${TELNYX}${vp}`, { headers: H, signal: AbortSignal.timeout(12000) }); const d = await r.json().catch(() => ({})); let arr = Array.isArray(d && d.data) ? d.data : (Array.isArray(d) ? d : (d && typeof d === 'object' ? (d.voices || []) : [])); if (r.ok && arr && arr.length) { const provs = {}; arr.forEach((v) => { const p = v.provider || (String(v.id || v.name || '').split('.')[0]) || '?'; provs[p] = (provs[p] || 0) + 1; }); const cart = arr.filter((v) => /cartesia|brooke|sonic/i.test(JSON.stringify(v))).slice(0, 4); voices[vp] = { status: r.status, total: arr.length, providers: provs, cartesia_or_brooke: cart }; } else { voices[vp] = { status: r.status, raw: JSON.stringify(d).slice(0, 260) }; } } catch (e) { voices[vp] = { error: String((e && e.message) || e).slice(0, 80) }; } }
