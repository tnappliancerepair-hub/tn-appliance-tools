...
exports.handler = async (event) => {
  ...
  // ── HCP: list jobs scheduled today ──
  const hcpJobs = [];
  for (let page = 1; page <= 5; page++) {
    const url = `${HCP_BASE}/jobs?scheduled_start_min=${encodeURIComponent(startCT)}&scheduled_start_max=${encodeURIComponent(endCT)}&page=${page}&per_page=100`;
    let r;
    try {
      r = await fetch(url, {
    ...
    for (const j of items) hcpJobs.push(j);
    const total = d && d.page_count ? d.page_count : 1;
    if (page >= total || items.length === 0) break;
  }

  // Filter HCP to ones assigned to this pro
  const hcpMine = hcpJobs.filter((j) => {
    const pros = j.assigned_employees || [];
    return pros.some((p) => p && p.id === hcpId);
  }).map((j) => ({
    hcp_id: j.id,
    work_status: j.work_status || '',
    scheduled_start: (j.schedule && j.schedule.scheduled_start) || j.scheduled_start || '',
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      summary: {
        ...
        in_both: both.length,
        hcp_only: hcpOnly.length,
        xano_only: xanoOnly.length,
      },
