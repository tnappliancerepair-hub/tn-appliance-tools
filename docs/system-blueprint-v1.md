...
24. **`xano workspace push --include` reports success without actually deploying to runtime — for EDITS of existing endpoints only.** The CLI's storage layer accepts the push (subsequent pulls return the new source) but the runtime execution layer continues serving the previous version. Mechanism unclear. **Workaround:** paste full source via Xano dashboard XanoScript editor manually. Or use full `xano workspace push --force` (no `--include`) — but currently blocked by separate "Duplicate table name: account" validation error on full pushes that needs investigation. Discovered 2026-05-11 Day 2 trigger 1 build. **Clarification 2026-05-11 (later that day):** the silent no-op behavior is specific to EDITS of existing endpoints. For NET-NEW endpoints, `xano workspace push --include` works correctly — both creation AND subsequent edits propagate to runtime. Confirmed during `ahs_email_intake` build: first push created the endpoint (probe returned 400 input error, parser executed), and ~6 subsequent `--include` pushes during the verification cycle all propagated edits to runtime (verified via `probe_v4` literal showing up in the diag row of a later fire). So `--include` reliability appears to depend on whether the endpoint already exists at push time. Mechanism still unclear — possibly a caching layer that's only populated after first edit-cycle of an endpoint.
...
28. **`regex_replace` filter returns null in this Xano runtime regardless of match status.** Every invocation produces null output even when the pattern would have matched OR when it would have passed input through unchanged. Discovered 2026-05-11 during `ahs_email_intake` parser verification — three call sites (entity decode, phone digit-strip, email char-class strip) all silently failed. Workarounds: (a) for single-literal patterns, use `|replace:"literal":"replacement"` instead (proven in `send_sms_POST.xs:31`). (b) for char-class strips on input known to be clean, skip the step entirely. (c) for complex regex patterns with no literal substitute, this is a blocker — needs different parsing approach (e.g., split-based extraction). Note: `tech_sms_inbound_POST.xs` uses `regex_replace` successfully in some places per earlier inspection, so the failure mode may be context-dependent rather than universal — pattern unclear. When in doubt, instrument with a diag event_log row capturing both pre- and post-`regex_replace` values.
