...
## Where we actually stand (updated 2026-07-08)
...
- ✅ **Ordering-API access request SENT** — Teddy emailed Amazon on **2026-07-03** from
  tnappliance@gmail.com: subject *"Amazon Business API access request — Ordering API (TN
  Appliance Exchange)"*. **No reply yet** (this is the 1–5 week human-reviewed gate).
  `amazon-api-watch` is armed to text Teddy when Amazon responds.
- ⚠️ **Note the two-doors trap:** the **SP-API (seller side)** profile came back APPROVED
  2026-07-03 ("global Marketplace access to SP-API") — that is the WRONG product (selling/
  resale). It does NOT enable buyer ordering / drop-ship. The BUYER-side request above is
  the one that matters and is still pending.
- ✅ **Account-side setup DONE (2026-07-08, verified via screenshots):** the 3 pieces
  production needs are in place —
    - **Group:** "Parts Ordering" (org: TN Appliance Exchange Inc)
    - **Payment method:** the business Amex (Business Gold) is **shared with** the Parts
      Ordering group ("Currently Shared: Parts Ordering")
    - **Buyer email:** tnappliance@gmail.com (the Business account login)
  The exact API tokens (`GROUP_ID` GroupIdentity string, `PAYMENT_REF` instrument id) are
  generated by Amazon at **production provisioning** — they're not exposed in the buyer UI,
  so there is **nothing more Teddy can do on the account side.** When Amazon grants prod:
  finish the developer profile (identity verify), collect those two tokens → vault
  `AMAZON_BUSINESS_GROUP_ID` / `AMAZON_BUSINESS_BUYER_EMAIL` / `AMAZON_BUSINESS_PAYMENT_REF`
  → flip `AMAZON_BUSINESS_ENV=production` → $1 trial → live.
- ▶️ **Meanwhile: Track 3 browser-bot** (`colony-loop/parts/amazon-order.js`) ships the
  same drop-ship capability with ZERO approval — being wired now so it's not blocked on
  Amazon's answer.
