...
- **🗂️ MEISTERTASK API CONNECTOR — BUILT + DEPLOYED, waiting ONLY on the token.** `_lib/meistertask.js` (Bearer, `https://www.meistertask.com/api`) + `meistertask-pull(-background).js` pull projects→sections→tasks→comments into Supabase `meistertask_archive` (the 7-yr history rescue, no manual export/upload). **TOKEN STEP (Teddy):** personal access token is minted at **mindmeister.com/api** (NOT meistertask.com/api — that 404s), **logged in as the boards account = james pivacek** (tnappliance@gmail is Basic/free, can't see the boards). Scopes = **`meistertask`** (or `.readonly`) + `userinfo.profile` + `userinfo.email` → Submit → copy → save in admin-secrets.html as **`MEISTERTASK_TOKEN`**. Then `meistertask-pull?probe=1&secret=` lists projects, then fire the full pull. (As of last check the token is NOT yet set.) Also still available: HCP CSV export into a similar importer if Teddy wants it ("grab both while you still have access").
