...
- **⏭️ NEXT (the phases, from the artifact):** **01 data-aware tools** — give the shop's Ann `get_status` (this resolver) + `get_hours`, replacing the FAQ-only toolset in BOTH provisioning paths (`trial-ann-admin.js buildTools` + `platform-phone.js`), so she answers status from the board instead of taking a message · 02 pre-call recognition (resolve during pickup → dynamic vars, warm-degrade) · 03 call actions (book/confirm day, capture callback→board, text link, transfer hours-gated + warranty-rep routing — all reuse existing platform endpoints) · 04 reliability + grounding (bounded timeouts + keep-talking fallback, "one moment" fillers, warm cron, per-tenant grounding rules) · 05 measure (per-tenant trust scorecard + accuracy audit + LLM grader).
