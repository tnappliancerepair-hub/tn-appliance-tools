...
- **✅ STEP 0 verified live** (`platform-status?secret=`): billing **live** (`PLATFORM_STRIPE_SECRET_KEY` set, `cs_live_`), signup + Ann-provision **gated** (the two outward flips), demo logins both sign in (`demo@assistant247.net`/`Ant-lp1xvv89`, `demo-tech@…`/`Ant-v77hr393`), demo board = 23 jobs/18 cust, Ann brain recognizes Teddy on demo. Portal token `cbe72a90-…` is the LIVE one (consistent across explore/demo-run/system; CLAUDE's old `72103f6e` is stale — ignore it).
- **✅ signup.html message fix** — was rendering the raw `d.error` (`"signup_not_open"`) on the final CTA click (the #1 embarrassing demo moment); now shows `d.message || d.error` (friendly invite/error copy + graceful-degrade).
- **✅ `platform-phone.js action=update` (NEW)** — fixes the documented gap: a first self-serve `provision` builds the assistant BEFORE `settings.phone.number` is persisted, so the LIVE warm-transfer tool is omitted, and there was no in-place update (only release+reprovision = churns the DID). New `update`/`rebuild` re-PATCHes the assistant via `assistantBody()` (number now set → transfer lands) without releasing the DID. Owner-JWT/admin gated, shadow-safe. Mirrors `trial-ann-admin action=update`. Verified reachable live.
- **✅ Import wizard gets a "TN (Xano)" keyless source chip** (`import.html` + `platform-import.js`) — the `xano` adapter (reads TN's live Xano keylessly via vaulted `XANO_METADATA_TOKEN`, read-only) was wired but API-only; now it's a wizard chip (no key field for keyless sources). Lets Teddy import his own TN book from the same wizard as everyone else.
- **✅ RELIABILITY — `platform/vendor/boot-watchdog.js` (NEW), on all 18 supabase pages** (after supa-guard). The seats put NO timeout on their boot query → a hung/blocked network = infinite spinner (the biggest silent-hang risk). The watchdog passively observes `window.fetch`: if ANY fetch RESOLVES within ~12s the page is fine (silent); if nothing resolves (true hang/offline) it drops a friendly "Slow connection — Reload" panel that self-heals. **Zero per-page wiring** (no `bootOK()` calls needed — the fetch observer is automatic), fully try/caught so it can never break a page.
- **✅ Killed the visible "coming soon"** — removed the `needs-scheduled.html` "Intake-link texting is coming soon" stub pill (office texts from Messages).
- **✅ Self-hosted Leaflet** (`platform/vendor/leaflet.{js,css}` + `images/`) — dispatch map now loads local-first with the unpkg CDN as fallback, so the Act-2 map survives a demo network that blocks unpkg.
- **✅ Zero-charge signup smoke test** — `platform-signup` (admin bypass) → returns a real **`cs_live_` Stripe Checkout Session** URL (creating a session charges nothing). Proves the checkout half works on the live key + prices resolve. The webhook→provision→login-email half is only provable by a REAL completed signup (Teddy's customer-#1 walk).
- **⏭️ TEDDY-GATED GO-LIVE RUNBOOK (my part is done):**
  1. **Verify 2 vault items** (the two landmines that strand a paying signup): `PLATFORM_STRIPE_WEBHOOK_SECRET` set + the Stripe webhook is registered at `…/platform-stripe-webhook` for `checkout.session.completed`+`customer.subscription.*` (⚠️ the webhook reads ONLY `PLATFORM_STRIPE_SECRET_KEY`, no fallback — it must be the SAME live account checkout charges on); and `EMAIL_SHARED_SECRET` set + `send-email` works (else a paid signup provisions but the owner gets NO login link).
  2. **Flip `PLATFORM_SIGNUP_LIVE=true`** (vault) → public signup charges cards. Do LAST.
  3. **Teddy = customer #1:** `signup.html` → sign up "TN Appliance Exchange" (real card, 14-day trial = no immediate charge) → magic-login email → `owner.html`. Capture the auto-derived slug (`platform-provision?action=tenants`) — it's a FRESH tenant, NOT the old test `tn` (67ef8d33). Configure shop/crew.
  4. **Import his book:** `import.html` → new **TN (Xano)** chip → his slug → preview (~7 techs/3,543 cust/3,191 jobs) → CAPPED run first, then the full ~23-commit run watched (money/unit layers are phase-2).
  5. **Turn on his Ann:** flip `PLATFORM_PHONE_LIVE=true` (+ `TELNYX_API_KEY`/`TELNYX_SHARED_MESSAGING_PROFILE_ID`/`TELNYX_TOOL_SECRET`) → `onboard.html` "Turn on my AI receptionist" (auto-buys a DID near his area, builds full brain + per-tenant precall) → then `platform-phone action=update` to land warm-transfer.
  6. **TK = #2** (already staged: Workiz import fixed, `office-overview-tk.html`, runbook) — ⚠️ NO `ref=TK` on his own tenant.
- **HONEST RISK FLAGS:** full 3,191-job Xano import untested at scale (only 200/200 capped, onto a different tenant); Ann-live buys a real Telnyx DID (~$1/mo); the webhook/email landmines are only caught by Teddy's real customer-#1 completion.

## 🗓️🐜🤖💸 2026-09-02 (Tue) — CHATGPT/OPENAI ADS SUITE (paid arm of the AI-discovery push) — built DARK, deployed, verified — READ FIRST

Teddy shared OpenAI's ChatGPT Ads launch → "Check if it has an API first" → "Build it — I'll get the key." Built the full connector + campaign + conversion loop **mirroring the Google Ads suite**, all LIVE on `main` + branch, all **DARK** (every fn returns `{configured:false}` until the key is vaulted — charges $0). This is the PAID arm that buys the "Sponsored" spot when someone asks ChatGPT "who fixes my dryer in Nashville"; the ORGANIC AI answer is still won by the reputation/schema work (OpenAI keeps ad placement + the answer separate on purpose).
- **`_lib/openai-ads.js`** — connector. **Simpler than Google's** — OpenAI Ads uses a plain **Bearer API key** (issued in Ads Manager → Settings), NO OAuth/refresh/MCC. `creds()` reads `OPENAI_ADS_API_KEY` + `OPENAI_ADS_PIXEL_ID` via `getSecretPreferVault`; `API_BASE=https://api.ads.openai.com/v1`, `CONV_BASE=https://bzr.openai.com/v1`; `configured()` = `GET /ad_account`.
- **`openai-ads-diag.js`** — owner-gated (`?secret=`) auth check. Live: `configured:false` + vault hint.
- **`openai-ads-upload-conversion.js`** — the money piece. Conversions API `POST bzr.openai.com/v1/events?pid=<PIXEL_ID>`, event types `appointment_scheduled|order_created`, matches the person by **SHA-256-hashed phone/email** (`emails_sha256`/`phone_numbers_sha256`; UTF-8→sha256→lc-64hex; phone normalized to E.164 11-digit US). Exports `uploadOpenAiConversion` + `hashPhone`/`hashEmail` (unit-verified). `?validate=1` = validate_only.
- **`openai-ads-conversion-sweep.js` + `-cron.js`** — feeds self-pay booked (`appointment_scheduled`) + paid (`order_created`) jobs back so OpenAI learns we CONVERT (the ranking lever most shops skip). **No gclid needed** — the Conversions API matches on hashed phone/email against people who saw our ChatGPT ads, so we send ALL self-pay conversions (standard offline-CAPI) and OpenAI attributes. Dedup via `openai_conversion_uploaded` event (job|type). **Reads denorm `job.customer_phone`** (skips the customer-table read) — first cut timed out (HTTP 000) on 2×80 sequential Xano reads; fixed to 1 read/job, cap 25. **Split core+cron** (a scheduled fn edge-403s on manual HTTP): sweep is HTTP-callable (`exports.runSweep`, `?dryrun=1`), cron wrapper fires `runSweep(false)` in-process. Cron `10 */6` (offset from Google's `0 */6`). **Dryrun VERIFIED live** — found real paid jobs #21634 ($300) + #21642 ($100), matched by phone, would upload, uploaded 0 (dark).
- **`openai-ads-create-campaign.js`** — preview/apply, 24/7 **"Broken at 2am? We Answer"** copy (our real edge — gpt-5.4 Ann answers every call any hour), lands `appliance-ai.html`/`always-open.html`. Campaign body `{name,status:active,budget:{lifetime_spend_limit_micros}}` (documented shape). Preview needs no key (verified). ⚠️ **ad-group/ad field names get first-real tuning** on the first `&apply=1` (OpenAI documents the campaign body but not the full ad/targeting schema — same discipline as the HCP/Jobber adapters).
- **`openai-ads-performance.js`** — spend/results reader (dark).
- **RANKING (Teddy asked "how do they rank paying customers other than bid"):** bid × conversation-relevance × business trust × **conversion-likelihood**. The conversion loop + our reviews/schema (same entity signals that win the organic answer) lift the PAID rank too → better placement for less spend. The bid is just the entry ticket.
- **PHONE-BRAIN TIE-IN (Teddy: "we're using chatgpt intelligence on our phone brain"):** the platform Ann runs on **OpenAI gpt-5.4** (Telnyx) — so a ChatGPT ad → call → gpt-5.4 Ann books it 24/7 → the sweep feeds that booked/paid job back to OpenAI = OpenAI's own model closes the lead its own ad surfaced, and learns we convert. All one ecosystem. BUT ChatGPT Ads is a **separate product/account** (ads.openai.com) — being an API customer for the phone doesn't auto-grant it; the 4 signup steps still apply.
- **⏭️ TEDDY'S 4 STEPS to go live:** sign up at **ads.openai.com** → create the ad account → issue the **API key** (Settings) + grab the **Pixel ID** (Conversions tab) → vault both via `admin-secrets.html` as `OPENAI_ADS_API_KEY` + `OPENAI_ADS_PIXEL_ID`. Then `openai-ads-diag?secret=` goes green → launch a small test campaign (~$25/day) → watch conversions flow back. Nothing charges until the key + an applied campaign.

## 🗓️🐜🎬 2026-08-30 (Sat) — DEMO DRY-RUN KIT: pre-flight verified live + a runbook teleprompter — READ FIRST

Teddy: "I need to demo our new system" → chose a **dry-run for himself** (walk it, confirm nothing's stale, be ready to demo to anyone in any format). Four wow moments: (1) 24/7 Ann phone AI, (2) runs the whole day, (3) bring-your-data-over, (4) AI partner + honest pricing. Delivered a pre-flight + a runbook page.
- **✅ PRE-FLIGHT (all live, verified):** every platform seat serves 200 (system/ops/owner/office-board/dispatch/tech/portal/verify/intake/comms/import/partner/office-guide); the **`demo` tenant (Joey's Appliance Repair, company `60a599af-…`) was thin** (only new/scheduled/completed) → **freshened**: added an **in_progress** job (Rebecca Alvarez, GE dryer, Joey en route), an **awaiting_parts** job (Marcus Bell, Whirlpool dishwasher + a drain-pump `job_part` ordered), and a **scheduled AHS warranty** job (Donna Pierce, LG fridge, claim# + a 2-msg customer thread) → board now spans **every column** (new 5 · scheduled 6 incl. 1 warranty · in_progress 1 · awaiting_parts 1 · completed 1), 20 thread messages, the system.html portal token (`72103f6e-…`) resolves. **Phone-recognition seed intact:** Teddy (+16154855795) is a demo customer.
- **✅ LIVE ANN LINE CONFIRMED UP (the teardown note was stale — it's still running):** **+1 615-588-9400** is active on Telnyx (connection `3037559490426700908`); assistant **`assistant-1d5304e3-cca4-4711-ab51-04b26edbc729`** ("Ant — Joey's Appliance Repair (trial)", gpt-5.4, 9 tools) exists; precall webhook wired to `platform-precall?slug=demo&k=tk_…`. **Recognition works WARM** → *"Hi Teddy! Thanks for calling Joey's Appliance Repair — I see you're scheduled with Joey for Wednesday, Sep 2 for your Whirlpool refrigerator…"*. **⚠️ COLD-START FOOTGUN:** the FIRST call after idle loses the 1.8s race → generic greeting; the 2nd+ nails the by-name greeting. **Warm the line with one throwaway call ~30s before demoing** (platform-warm cron helps but don't trust it for a live demo). The runbook says this loudly.
- **✅ `platform/demo-run.html` (NEW) — the runbook / teleprompter** (noindex, office-guide.html style, mobile). Top facts box (the live number, front-door system.html, demo login `demo@assistant247.net`, the portal token link, import.html) + a **⚠️ warm-the-line** banner. Then **4 acts**, each = exact click-path + a ~20-sec **talking point with a competitor jab** drawn from the ads Teddy shared: **Podium** ("their bot takes a message; mine knows who's calling + books"), **one-system** ("not five tabs"), **Workiz** ("they say you're allowed to leave; we actually move you + keep the old one running"), **Housecall Pro comments** ("don't tell me what to charge → the customer picks 4 honest options; the AI partner acts but you can undo"). Plus an "if something's off" recovery box + the `?ref=` close. **🎬 Demo link** added to the ops.html header (next to 📥 Import · 🗺️ System).
- **The demo login gap:** runbook lists `demo@assistant247.net` as the demo-shop sign-in — Teddy needs that password handy (or reset it). Portal + phone + import need no shop login (token / call / ops key). Ops key = the same admin secret as ops.html — deliberately NOT hardcoded in the page.
- **⏭️ QUEUED (Teddy's picks, not built this round):** the **full beat-TowBook towing roadmap** for sister Melissa's tow business (4 pillars from her pains: Ann answers every tow call · live dispatch board + driver ETA · police impound/lien/rotation · cheaper+simpler than TowBook — a written product spec); the **marketing swipe file** (the Podium/Workiz/HCP ads + our counter for each); the **self-serve onboarding-clone spine** (custom mini-site at /s/‹slug› · the ‹slug›@jobs intake address + one forward rule · a live setup-checklist welcome page · "connect your old system" = the import wizard · Ann auto-built + a fleet-upgrade sweep; honest gated bits = phone DID/10DLC, custom-domain DNS, Stripe KYC). **ICP lesson (from Jake's CarGurus/CarsForSale reply):** target service-lifecycle + after-hours + a work-sender (appliance/HVAC/plumbing/electrical/garage/locksmith, **towing best**); **skip car lots** (sales, not service — thin angle, already served by the marketplaces).

## 🗓️🐜📥 2026-08-30 (Sat) — DATA-MIGRATION WIZARD (HCP → the board): "bring your data over" is the make-or-break onboarding — READ FIRST

Teddy's insight: **the fear of moving off a working system (Housecall Pro / Jobber / Workiz) is what makes or breaks the SaaS.** Answer = yes, and we'd already proven 80% of it (`hcp-pull.js` mined TN's ~49k HCP rows). Built the HCP-first import wizard end-to-end; **verified live against TN's real HCP account** (9,553 customers / 24,644 jobs) landing onto a fresh throwaway tenant, then proved idempotency. Jobber + Workiz are the same shape next.
- **`docs/sql/048_import.sql` (APPLIED):** the idempotency spine. `import_run` (one row per migration; true source totals + a **resumable cursor** {phase,page,limit} + running landed/skipped + a sample for the preview) + `import_map` (**the ledger:** company+source+kind+external_id → board id, unique — so a re-run NEVER double-creates and job→customer / job→tech FKs resolve through it). **Both owner-only** — RLS deny-all to clients (anon read → 42501), service-key only, like `partner`.
- **`_lib/import/hcp-adapter.js` (NEW):** HCP API (`Authorization: Token`, `api.housecallpro.com`) → the platform's normalized bundle. `probe(key)` (auth + true totals, no writes), `page(key,kind,n)` (normalized records). Kinds: employees→technicians, customers, jobs (+ a **job-derived invoice** inline: total_amount→cents, paid when outstanding_balance==0, one summary line — no N+1 line-item fetch). Status map (HCP work_status→new/scheduled/in_progress/completed/canceled). **Two live-data footguns burned:** (1) HCP `description`/`notes` are sometimes **objects/arrays** → scalar-only `pick` + a `text()` flattener (was landing `[object Object]`); (2) TN's HCP descriptions are **raw HTML** (`<a href="dispa.me/…">` dispatch links) → `clean()` strips tags + entities before it hits a board column (ugly + XSS-unsafe otherwise).
- **`_lib/import/land.js` (NEW, source-agnostic):** lands a normalized page onto a tenant board via the service key, idempotent through `import_map` (loads existing external_ids, inserts only fresh, records the map, resolves job→customer/tech via the map). Reusable for Jobber/Workiz.
- **`netlify/functions/platform-import.js` (NEW):** the engine. `do=probe|preview|commit|status`, admin-secret OR operator-JWT gated. `key` = the shop's HCP key (pasted in the wizard) OR falls back to vaulted `HCP_API_KEY` for a demo; **raw key never stored — only last-4 (`key_hint`)**. Commit is **resumable** (bounded PAGES_PER_COMMIT per call, cursor advances phase techs→customers→jobs; `limit_pages` caps a demo for a watchable land) so it beats Netlify's 26s cap on a 24k-job shop.
- **`platform/import.html` (NEW):** the "Bring your data over" wizard — connect (source chips: HCP live, Jobber/Workiz "coming next" + shop slug + paste key + demo-mode) → **preview** (true count tiles + sample customer/job cards + honest "what comes over / what fills in later / nothing is deleted, run both a week" notes) → **import** (progress bar looping `do=commit`, counts ticking) → done (open the board). ops-secret gated (`ant_ops_secret`, same as the operator tools).
- **VERIFIED live (fresh `hcp-import-demo` tenant, company `3e3bc731-…`, owner `hcp-import-demo@assistant247.net`/`Ant-y8tdrs43`):** probe = real HCP totals; commit landed **200 customers + 200 jobs + 1 tech** with **job→customer FK linkage working** (Steven Wilhelm's job joins to Steven Wilhelm the customer); clean plain-text problems; **idempotent** — a second full run landed **0 new / skipped 401**, board held at 200/200 (no doubling). (129/200 linked is a pure **demo-cap artifact** — some jobs' customers sit on un-imported pages; a full uncapped run lands all customers first so every link resolves. 0 invoices because HCP's first job pages are all $0 new/AHS leads — correct/honest.)
- **✅ JOBBER + WORKIZ adapters ADDED (same engine).** Generalized the adapter contract to a **uniform opaque cursor** (`page(key,kind,cursor)→{records,next}`, `START`, `PHASES`) so every source plugs into the same `land.js`. **`_lib/import/jobber-adapter.js`** (GraphQL, `Authorization: Bearer <OAuth token>`, cursor pagination, users/clients/jobs) + **`_lib/import/workiz-adapter.js`** (REST, **token-in-path** `…/api/v1/<token>/…`, offset pagination; Workiz has no customers list so the 'customers' phase **walks the jobs feed** and emits a deduped customer per job — `land.js` gained within-batch first-wins dedup for exactly this). platform-import iterates `adapter.PHASES` generically; the wizard chips light up all three (source-aware key label/hint). **Honest status:** HCP is verified live; **Jobber + Workiz are built to each platform's documented API and reach the real endpoints** (bogus tokens → clean 401/Jobber, 403/Workiz, no crash) — they get their first-real-shop field tuning (enum spellings, nested money/address shapes, whether team data is exposed) the same way HCP's two footguns surfaced live today. **HCP re-verified end-to-end through the refactored generic engine** (fresh land: 200/200/1, done; idempotent).
- **⚠️ STRATEGIC FLIP (Teddy):** done right this turns the #1 objection into the #1 sales line — *"bring your data — off Housecall Pro, Jobber, or Workiz — in an afternoon, free, and keep the old one running until you're sure."* The thin phone-only competitors can't say that.
- **📥 Import link** now in the ops.html header (next to 🗺️ System).
- **✅ TN's OWN system (Xano) → the board — the cutover blocker, PROVEN.** `_lib/import/xano-adapter.js` (same engine) reads TN's live Xano via the Metadata content API (techs=15, customers=6, jobs=7), **keyless** (uses vaulted `XANO_METADATA_TOKEN`; platform-import `KEYLESS` set). **READ-ONLY against Xano — the old system keeps running untouched, which IS "run both until you're sure."** Confirmed the mapping against the live 127-column jobs table (probe returns full column list): status ← scheduling_status/current_status, appliance (brand+type+model) folded into `problem`, + warranty_company/claim_number/dispatch_id/parts_status/service_window/completed_at, and every row carries **`xano_id`** (customer/job) / **`xano_tech_id`** (tech) so the platform's built-for-this mirror columns line up. **Skips `test_run_id` jobs** so TN's test rows never hit a real board. Provisioned a fresh **`tn` tenant** (company `67ef8d33-…`, owner `tn-owner@assistant247.net` / `Ant-nqj8um62`). **VERIFIED live:** preview read TN's real counts (**7 techs · 3,543 customers · 3,191 jobs**); a capped commit landed **200/200 with full fidelity** — 200/200 linked to their customer, 68 tech-assigned, **148/200 warranty** (Demika→SquareTrade, Gregory→AHS…), 200/200 with xano_id, real status spread (awaiting_parts/canceled/completed/scheduled). Idempotent + reversible (purge the tenant).
- **⏭️ TN migration scope:** v1 = techs + customers + jobs (appliance in the problem text). **Phase-2 (deliberate):** clean **unit** records (TN has brand/model/serial — extend landJobs to resolve unit_id) + the **money layer** (jobs carry `total_amount_cents`/`payment_status`, plus warranty-remittance splits — its own careful pass). Note: warranty_company casing is as-stored in Xano (AHS/Ahs/Squaretrade — faithful, not normalized); a few early untagged smoke-test rows come over (tiny at full scale). **Full run (3,191 jobs ≈ 23 commit calls) is one word away** — pairs naturally with Danielle's parity test (populate the real board with real TN data, she runs her day on it).
- **⏭️ NEXT (offered):** Jobber **OAuth "Connect" button** (v1 takes a pasted token — operator-driven) · a generic **CSV** mapper (universal fallback when API access is a pain) · an **attachment/photo pass** (fetch → re-host to our storage) · assign Jobber/Workiz jobs to techs (Jobber via visits, Workiz via team — left null in v1, office assigns) · teardown the `hcp-import-demo` tenant after showing Teddy.
- **HONEST LIMIT:** the import gets the DATA in; confidence still comes from a human running a day on it (same Danielle-parity discipline). First shop of each source surfaces its own quirks; after 2–3 it's a template.

## 🗓️🐜🗺️ 2026-08-30 (Sat) — SYSTEM HOME (one presentable front over all 20 surfaces) + honest TN-cutover gate — READ FIRST

Teddy wanted to switch TN onto the platform "so I know it's complete" — but the platform had grown to **20 pages across 7 seats with no unifying front**, so it didn't feel simple/complete even though the data model (004→047) + most surfaces are live. He chose "both in one": a page that reads as a product overview to SHOW prospects AND operates as an operator control tower. Built + verified live.
- **`platform/system.html` (NEW) — the one front door.** Public top = product overview + an **interactive prospect tour** (7-step pain→fix stepper with CSS proof-mocks, "see it live" demo links, a Start-free-trial CTA that carries `?ref=<partner>` so a TK-referred prospect stays attributed). Operator bottom (gated by the same `ant_ops_secret` key as ops.html) = a **control tower**: every seat as a live-link card (owner/office/dispatch/tech/customer-portal/verify/operator/partner), system status LIVE/GATED/SHADOW per area, and the honest **TN-cutover readiness in 3 buckets**. The tour doubles as the **ad/video storyboard**.
- **`platform-status.js` (NEW, admin/operator-gated):** health + readiness aggregator — reads the real go-live flags (PLATFORM_PHONE_LIVE/SIGNUP_LIVE/BILLING_LIVE, Stripe key mode, email-intake DNS), demo-tenant counts (11 jobs/6 cust/1 tech), demo seat links (portal token + verify), and the cutover buckets. Bad key → 403.
- **ops.html** header gained a **🗺️ System** link (the operator's front door). Did NOT link from owner.html (a real shop-owner customer doesn't need a system map — avoids confusion).
- **VERIFIED live:** system.html 200 (tour + gate + ref-CTA present), platform-status returns real data + 403 on bad key, all 14 platform surfaces 200.
- **⭐ THE HONEST TN-CUTOVER GATE (corrected by Teddy):** the earlier "4 money/vendor gaps" are mostly NOT blockers — **TN doesn't run them today either** (parts-ordering automation isn't live on the old system; warranty-claim *submission* is done by hand on both; AHS/Frontdoor status push is waiting on the vendor API for both). You can't lose what you don't have. **Real remaining before cutover = (1) migrate the data off Xano (the one true blocker — can't cut onto an empty board), (2) confirm operational parity by running the actual day on the board incl. manual warranty processing, (3) phone/DID + 10DLC SMS-lane cutover.** Everything else is roadmap for BOTH systems. Platform is much closer to cutover-ready than it first looked.
- **⏭️ NEXT (offered, not built):** the 3 real cutover steps above (data migration is the big one); the **ad script set** modeled on the FixCall AI competitor ads Teddy shared (talking-head + live proof cards + "try free" + appliance-owner poll — his real-owner authenticity beats their spokesperson; AssistAnt's depth beats their thin phone-only product); TK's commission % still unset (Teddy's call — my rec 25% for 12 months then his).

## 🗓️🐜🤝 2026-08-30 (Sat) — RESELLER/REFERRAL VENTURE WITH TK (attribution + commission tracker) + GREG UPGRADED — READ FIRST

Teddy is starting a **reseller/referral venture with TK** (The Appliance Guy): TK refers appliance shops → earns a commission; **platform IP stays 100% Teddy's.** Delivered the protection + the machinery, and upgraded Greg's live line.
- **✅ GREG'S ANN UPGRADED to the current call brain** (was FAQ-only). `trial-ann-admin?action=update&shop=greg&id=assistant-1272a268-…` → full toolset (get_status/get_hours/request_day/callback/send_link/transfer) + pre-call recognition on his automotive board. Number **931-632-4734 confirmed still routing** (connection_id matches TeXML app); live precall on `classic-automotive` greets his customer by name. Voice unchanged.
- **✅ REFERRAL ATTRIBUTION + COMMISSION TRACKER (all live, verified end-to-end, residue 0).** There was ZERO referral machinery on the platform before this. Built mirroring the `tech_payout` discipline (ledger stores PAID; EARNED derives at read time):
  - **`docs/sql/047_partner_referral.sql` (APPLIED):** `partner` (code, name, commission_type `sub_pct|flat_per_account`, commission_pct/flat/months, `token` for the dashboard), `company.referred_by`/`referred_at`, `partner_payout` ledger. **RLS deny-all to clients** (anon/authenticated SELECT=false, service_role only) — platform-owner data reached ONLY via service-key endpoints; a partner sees their pipeline through a read-only TOKEN.
  - **`?ref=` rides through BOTH onboarding paths:** `signup.html` (URL `?ref=`) → `platform-signup` → `platform-billing.signupCheckout` provMeta → `platform-stripe-webhook` (validated against an active partner before stamping) AND `platform-provision?action=provision&…&ref=TK` (the **primary path today** — self-serve signup is gated off, so TK refers → Teddy provisions with `&ref=TK`).
  - **`platform-partner.js` (NEW):** `do=upsert` (mints a dashboard token) · `do=list` · `do=report&code=TK` (admin) or `&token=<partner.token>` (partner, read-only) · `do=record_payout`. Report derives per-account MRR from `company.plan` (+ own_area addon), computes each account's monthly commission + earned estimate, minus recorded payouts = **owed**. Admin gate mirrors platform-provision (VAPI_ADMIN_SECRET or operator JWT).
  - **`platform/partner.html` (NEW):** TK's token-gated dashboard — his **referral link** (copy), shops he referred (status · MRR · his cut/mo), and **owed / run-rate / earned / paid** tiles. Served 200.
  - **VERIFIED live:** throwaway partner (20%) + throwaway referred shop (plan office $99) → report showed cut/mo $19.80, record_payout $19.80 → owed $0, token path works; then **purged to 0 residue** (shop + partner + payout).
- **✅ TK SEEDED as partner #1:** code **`TK`**, name "TK Cousins", email tk@servicealliancegroup.com, cell +18043346984. **Commission LEFT UNSET on purpose** — Teddy locks the % + duration WITH the signed agreement, then one command sets it: `platform-partner?do=upsert&secret=…&code=TK&commission_pct=<N>&commission_months=<0=lifetime|N>`. Until set, his dashboard shows $0 (honest). TK dashboard token: `pt_2e274606b78803f0c1db43c52f3adbb3` → `partner.html?token=…`; his referral link `signup.html?ref=TK`.
- **✅ AGREEMENTS DRAFTED (templates, NOT legal advice — for Teddy's counsel):** `docs/partner/tk-mutual-nda.md` (mutual NDA: confidentiality, no-reverse-engineer, no-license) + `docs/partner/tk-reseller-agreement.md` (non-exclusive referral: commission, **IP stays Company's**, **feedback-assignment**, **non-circumvention**, no-authority-to-bind, term/termination). Each opens with a NOT-LEGAL-ADVICE banner + a "fill these blanks" list (entity names, governing-law state, commission %/duration — **must match `partner.commission_*`**).
- **⏭️ OPEN (Teddy's calls):** (1) lock TK's commission **% + duration** (lifetime-while-active vs N months) + territory → set both the agreement AND `partner.commission_*`; (2) have counsel finalize the two agreements before TK signs; (3) DEFERRED "Everything" scope: one-command clone/upgrade tenant + "upgrade every Ann to current" fleet sweep + fix the self-serve Ann no-update gap (`platform-phone` only builds at provision) + de-drift the two `assistantBody` copies (`trial-ann-admin.js:207` vs `platform-phone.js:71`).
- **FOOTGUN:** `platform-provision` idempotent standup is the reused atom; the `&ref` stamps `referred_by` on company CREATE (gentle backfill if it later exists with none, never clobbers). Attribution capture works regardless of onboarding path (admin or self-serve).

## 🗓️🐜☎️ 2026-08-30 (Sat) — LIVE TEST LINE FOR PLATFORM ANN (call-brain, end-to-end on a real number) — READ FIRST

Teddy said "I'm ready" → stood up a **real callable line** so he can hear the new (platform/Supabase) Ann and its call brain in his ear. All on the demo tenant (Joey's Appliance Repair, slug `demo`, company `60a599af-…`).
- **📞 THE NUMBER: +1 615-588-9400** — routes inbound → Telnyx AI assistant **`assistant-1d5304e3-cca4-4711-ab51-04b26edbc729`** (store shop slug **`demoline`**, `platformSlug=demo`, model gpt-5.4, voice Inworld Max Brooke). Full toolset live: **get_status · get_hours · request_day · callback · send_link · transfer · capture_lead · message_owner**. **Pre-call recognition wired** (`platform-precall?slug=demo`) — verified on the production `tnapplianceexchange.net` URL Telnyx actually hits.
- **Seeded Teddy (615-485-5795) as a demo customer + a scheduled job** so recognition greets him by name: customer `2ca5d390-…`, job `1a115ea4-…` (`source='ann_demo_test'`, status scheduled, Joey, Whirlpool refrigerator, Wed Sep 2, morning). Live greeting proven: *"Hi Teddy! Thanks for calling Joey's Appliance Repair. I see you're scheduled with Joey for Wednesday, Sep 2 for your Whirlpool refrigerator — is that what you're calling about, or is it something else?"*
- **Bought ONE 615 DID** (`buynew`, ~$1/mo) instead of repointing a production number — all 18 owned Telnyx numbers were already wired (0 spares), so a spare bind would have hijacked a live TN line. Transfer target = Teddy's cell (only rings in business hours).
- **⏭️ TEARDOWN when done testing** (nothing customer-facing depends on it): delete assistant `assistant-1d5304e3-…` (`trial-ann-admin?action=delete&id=…`), **release the DID** `+16155889400` (Telnyx), delete store shop `demoline`, and `delete from job/unit/customer` where `source='ann_demo_test'` / customer `2ca5d390-…` (plus any thread_message / schedule_offer a test call created). Task #62 carries the same teardown note.
- **FOOTGUN:** the `transfer` tool only attaches when the shop has BOTH `annNumber` + `ownerCell` — so the order is create → **bind** (writes annNumber back to the store) → `action=update` to re-push the assistant so transfer loads. First `create` had no transfer; the post-bind `update` added it (confirmed via `?action=preview`).

## 🗓️🐜📞 2026-08-29 (Fri, late) — THE CALL BRAIN, PHASE 0 (platform): the phone AI now KNOWS the caller + speaks the shop's real data — READ FIRST

New project (Teddy): "train our phone AI to handle calls, utilize all the data in our system, and intelligently manage calls correctly." Scoped it (Teddy's picks: **port TN→platform**; call brain may **look up status + book a day + capture callback/text link + transfer to a human**), mapped current state with 3 scouts, published a **6-phase plan artifact "The Call Brain"** (`https://claude.ai/code/artifact/3d45ec4b-3a3c-4ce7-9336-f91b3e56e421`), and **built + proved Phase 0 (the keystone resolver) live on demo.**

- **The gap:** the platform's per-shop Ann is a 24/7 FAQ + lead-catcher — her ENTIRE toolset is `capture_lead`/`message_owner`/`hangup`; she reads **zero** business data on a call (can't tell a caller their own job status). Meanwhile TN's legacy Ann already does this well via `job-truth` (audience lenses, no-guess grounding, day-only, callback/transfer, trust scorecard). The platform had all the columns AND a rich caller composer (`portal_get`) — but keyed by portal TOKEN; **no phone→customer resolver existed anywhere.** That resolver is the one thing everything renders from.
- **`docs/sql/045_call_brain.sql` (APPLIED) — `platform_call_lookup(company_id, phone, claim, name)`:** SECURITY DEFINER, **granted to service_role ONLY** (server-side, no browser/anon/cross-tenant reach). Resolves the caller by **phone (normalized last-10 digits)** → claim# (whitespace-stripped) → name, picks their best job (prefer non-terminal, else newest), returns the grounded facts (status, scheduled_day, tech first name, unit label, warranty co/claim/dispatch/window, parts status/eta). The last-10 normalize **fixes the raw-string phone-equality bug in the lead writer** — proven live: Maria dialed `(615) 555-0101` matched the stored `615-555-0101`.
- **`netlify/functions/platform-call-brain.js` (NEW) — `do=lookup {slug, phone?/claim?/name?, lens?}`:** resolves company from the dialed shop's **slug → company_id** (code-scoping, same discipline as `createLeadJob`), calls the RPC, and **composes 3 grounded lenses in JS** (ported from job-truth): **homeowner** (day-only, "your technician" fallback, never-say-canceled, completed+warranty→recall redirect, part ETA, NO part#/claim#), **warranty rep** (whole status in one breath, claim#/parts allowed), **office** (terse + ⚠ flags NO TECH / AWAITING PARTS). Tool-key gate: real slugs need `?k=<TELNYX_TOOL_SECRET>`, `demo` open (mirrors platform-lead).
- **VERIFIED live on demo (read-only, no residue):** Angela (`615-555-0103`) → homeowner *"Hi Angela — you're scheduled with Joey for Monday, Aug 24 for your GE dishwasher. We don't run exact times, but we'll text a live arrival window that morning."* + warranty + office lenses; cross-format phone match (`(615) 555-0101`→stored dashed); name match ("chen"→David Chen); unknown phone → warm "let's get you set up"; real-slug w/o key → `forbidden`.
- **✅ PHASE 1 SHIPPED (data-aware tools) — the shop's Ann now has eyes on the board.** Gave both provisioning paths (`trial-ann-admin.js buildTools`/`buildInstructions` + `platform-phone.js assistantBody`) two new tools, replacing the FAQ-only toolset:
  - **`get_status`** → `platform-call-brain?do=lookup` — Ann looks up the caller's existing job by phone/claim/name and READS BACK the composed grounded line ("your dryer's scheduled Thursday with Lee"). Wired ONLY when the shop is on the board (uses the **board slug** = `shop.platformSlug` for trial, `company.slug` for self-serve — NOT the registry slug), with the tool key baked in. Instructions teach: day-only, "your technician", never "you're not in our system" (treat as new lead), homeowner never hears part#/claim#, warranty rep gets the rep summary, reading-back a shown day is fine but don't PROMISE a new appointment (that's Phase 3).
  - **`get_hours`** → `do=hours` — deterministic current CT day/time + default office-open hint (the thing LLMs guess wrong), no DB.
  - Added admin **`?action=preview`** (read-only: inspect the built persona + tool_names without a Telnyx call).
  - **VERIFIED live:** `do=hours` → "Saturday 6:08 PM, office_open:false"; Greg's (classic-automotive) preview → tools `[capture_lead, message_owner, get_status, get_hours, hangup]`, get_status URL correctly carries board slug + key; the exact baked URL authorizes (found:false on unknown), wrong key → forbidden. No residue (preview + lookups are read-only; no assistants mutated).
  - **⏭️ TO FLIP A SHOP ON (go-live step, NOT done — mutates a live customer assistant):** `trial-ann-admin?action=update&shop=<slug>&id=<assistantId>&secret=…` rebuilds the assistant body with the new tools. New shops get them automatically on create/provision. NOTE: `platform-phone` has no "update assistant" action yet — existing self-serve Anns pick up the tools only on re-provision (fine while few/none exist).
- **✅ PHASE 2 SHIPPED (pre-call recognition) — Ann greets by name + situation before the caller speaks.** `netlify/functions/platform-precall.js` (NEW) = the Telnyx **dynamic-variables webhook**: at pickup Telnyx POSTs the call context (caller in `data.payload.telnyx_end_user_target`, base64-safe decode), precall resolves the caller through the ONE brain (`platform-call-brain?do=lookup`) and returns `{ dynamic_variables:{ greeting, situation, system_context, … } }` so Ann opens *"Hi Maria! Thanks for calling Joey's Appliance Repair. I see you're scheduled with Joey for Monday, Aug 24 for your Whirlpool refrigerator — is that what you're calling about?"*.
  - **The multi-tenant trick (why no dialed-number→slug map was needed after all):** the shop is **baked into the webhook URL** (`?slug=<board>&k=<key>`) at assistant-build time — Telnyx just hands over the caller's number. So precall knows the shop from its own URL, exactly like the tool URLs.
  - **Reliability ported from TN's precall:** the whole lookup races a **hard 1.8s deadline** (Telnyx drops the webhook after ~2s and would speak the literal `{{greeting}}`) → on a slow/cold path it returns a warm generic greeting (with the shop name), never a placeholder; a **bulletproof try/catch wrapper** makes any failure a warm generic (HTTP 200), never a dead call.
  - `platform-call-brain` now also returns `shop` (company name) + `situation` (a confident one-liner, blank when unsure so it never opens with a wrong assumption — completed/canceled/no-day just greets by name).
  - **Both provisioning builders** now set `dynamic_variables_webhook_url` + `greeting:'{{greeting}}'` + safety-net `dynamic_variables` defaults + a `{{system_context}}` line in instructions — ONLY for board-connected shops.
  - **VERIFIED live:** real Telnyx POST shape (`telnyx_end_user_target` +16155550101) → "Hi Maria!… scheduled with Joey for Monday, Aug 24 for your Whirlpool refrigerator"; unknown → warm generic with shop name; cold-start → graceful warm degrade (not a placeholder); Greg's preview → `greeting:{{greeting}}`, `precall_wired:true`, webhook = `platform-precall?slug=classic-automotive&k=tk_…`. No residue.
- **✅ PHASE 3 SHIPPED (call actions) — Ann takes the wheel, safely.** `netlify/functions/platform-call-act.js` (NEW) = the close-the-loop action router, every action company-scoped (slug→company_id), reusing existing primitives, all landing on the shop's board:
  - **`do=request_day`** {phone/claim/name, day, win?, note?} — resolve caller → insert a `schedule_offer` (direction `customer`, day-only, low-promise `am/pm/any` window, `created_by:'ann'`) + a `thread_message`. **Spoken-day resolver** ("Thursday"/"tomorrow"/ISO → next date in CT). No active job → `createLeadJob` carrying the day. This is "book a day" (office confirms + texts; Ann never promises a clock time).
  - **`do=callback`** {phone, name?, note?} — `thread_message` on the caller's job (or a fresh `createLeadJob` if unknown) + best-effort owner SMS (`app_user` owner phone). Never lose a caller.
  - **`do=send_link`** {phone, kind} — find/mint `portal_grant` → text the caller `/i/<token>` (intake: video + model-sticker photo) or `/p/<token>` (portal) via `sendFrom588`. Unknown → `createLeadJob`.
  - **Transfer = Telnyx NATIVE `{type:'transfer', transfer:{from, targets}}` tool** wired into both builders (from = shop's Ann number, target Office = shop's owner cell). **Business-hours gated in INSTRUCTIONS** — Ann checks `get_hours` first; open → "let me connect you" + transfer; off-hours → take a message (callback), never ring anyone. (Warranty-rep-specific routing = same office cell for v1; shops have one cell.)
  - 045 RPC now also returns `customer.id` (writes need it). Both provisioning builders gained the 3 action webhook tools + the native transfer tool + action instructions — board-connected shops only.
  - **VERIFIED live on demo (writes, then scrubbed to 0 residue):** request_day "Thursday"→schedule_offer 2026-09-03/am + thread_message ("Perfect — I've put in a request for Thursday, Sep 3 in the morning…"); callback→thread_message on the job; send_link→minted portal token + `/i/` link; unknown-caller callback→createLeadJob. Greg's preview now shows the FULL toolset `[capture_lead, message_owner, get_status, get_hours, request_day, callback, send_link, transfer, hangup]` with transfer target=owner cell, from=Ann number. Cleaned every test row (offers/msgs/lead) to 0.
- **✅ PHASE 4 SHIPPED (reliability + grounding) — no dead air, warm hot path.**
  - **Dead-air fix = an INSTRUCTION rule, not tool config** (Telnyx AI-assistant webhook tools have NO Vapi-style filler `messages` array — TN handles it the same way). Ported TN's "**SPEAK BEFORE YOU ACT — never go silent on a tool**" rule into both builders' board instructions: say a short line ("let me pull that up real quick") BEFORE any lookup/book/text/transfer, and if a gap ever opens say "still right here with you — almost got it."
  - **`netlify/functions/platform-warm.js` (NEW, cron `2-59/5` = every 5 min)** pings `platform-call-brain?do=lookup` + `platform-precall` with a **benign demo phone (555-0199, found:false → cheap read, no writes)** so a container is always warm. **Measured the win live:** cold precall = **2.5s** (would LOSE precall's 1.8s greeting race → generic greeting) → warm = **0.57s** (wins easily). The cron keeps real callers on the fast path.
  - **VERIFIED:** Greg's preview instructions contain "SPEAK BEFORE YOU ACT" + the filler line; platform-warm returns 403 on manual GET (= deployed as a scheduled fn, the documented edge-403 footgun); latency cold 2.5s → warm 0.57s across back-to-back calls. No residue.
- **✅ PHASE 5 SHIPPED (measure) — the daily accuracy flywheel, per tenant. THE CALL BRAIN ROADMAP IS COMPLETE.**
  - **`netlify/functions/platform-call-score.js` (NEW) — `do=audit`:** runs the shop's ACTIVE jobs through the REAL brain (exported `compose`/`dayLabel` from platform-call-brain — the exact phrasing Ann speaks) and grades each answer against the board: **no invented clock time · a scheduled job names the right day · names the tech / "your technician" when none.** Also flags **data gaps** (scheduled-but-no-day jobs Ann has to hedge) so the shop fixes the record, not just the AI. Auth = admin `?secret=&slug=` (cron/testing) OR Bearer session → owner's company. Stores a `call_score` row per run (`docs/sql/046_call_score.sql`, RLS company-scoped) so a TREND accrues.
  - **`platform-call-score-cron.js` (NEW, cron `40 12 * * *` ≈ 7:40am CT):** grades every company with active jobs nightly. Split core+cron (scheduled fn edge-403s on manual HTTP).
  - **owner.html "📞 Ann's accuracy" card:** live audit via the owner's session → big **Answers correctly %** + sampled count + a gap heads-up; each view also stores a score (cron keeps it fresh regardless).
  - **VERIFIED live on demo:** 100% — 9/9 active jobs answered cleanly (no_time_ok 9/9, day_ok 9/9, tech_ok 9/9, 0 gaps, 0 mismatches); call_score row stored + verified + cleaned to 0 residue.
  - **HONEST SCOPE:** a transcript-based LLM grader of real call RECORDINGS (like TN's `ann-call-grader`) is a future add — it needs live platform call volume + Telnyx recordings, which aren't wired on the platform yet. This audit needs neither: it proves Ann WOULD answer correctly for every job on the board right now (the rigorous ground-truth check, TN's `phone-accuracy-audit` per tenant).

### THE CALL BRAIN IS COMPLETE — all 6 phases (plan artifact `https://claude.ai/code/artifact/3d45ec4b-3a3c-4ce7-9336-f91b3e56e421`)
✅ 00 resolver (`platform_call_lookup` RPC + `platform-call-brain` lenses) · ✅ 01 data-aware tools (`get_status`/`get_hours`) · ✅ 02 pre-call recognition (`platform-precall` dynamic-vars webhook, slug baked, 1.8s race + warm-degrade) · ✅ 03 call actions (`platform-call-act`: request_day/callback/send_link + native transfer) · ✅ 04 reliability + grounding (speak-before-you-act rule + `platform-warm` cron, cold 2.5s→warm 0.57s) · ✅ 05 accuracy audit (`platform-call-score` + cron + owner card). Every shop's Ann now recognizes the caller before they speak, answers from that shop's own Supabase board, books/callbacks/texts/transfers, holds the line fast + grounded, and is graded daily. **Reaches a shop's LIVE line only when its assistant is rebuilt** (`trial-ann-admin?action=update&shop=<slug>&id=<assistantId>&secret=…` for curated, or re-provision for self-serve); **new shops get the whole brain automatically** on create/provision. Greg's live line intentionally NOT flipped (a customer-facing decision).
- **Files:** `platform-call-brain.js` (reads: do=lookup/hours; exports compose/dayLabel) · `platform-precall.js` (greeting webhook) · `platform-call-act.js` (write actions) · `platform-call-score.js` + `-cron.js` (audit) · `platform-warm.js` (warm cron) · SQL `045_call_brain.sql` (platform_call_lookup RPC) + `046_call_score.sql`. Both provisioning builders (`trial-ann-admin.js` + `platform-phone.js`) wire all tools + precall + transfer, board-connected shops only. All slug-baked + `TELNYX_TOOL_SECRET`-gated (demo open).
- **⏭️ FUTURE (offered, not built):** transcript-based LLM call grader (needs Telnyx recordings) · a real per-call trust scorecard (needs a call-outcome log) · warranty-desk-specific transfer routing (v1 = one office cell) · precall dialed-number→slug map (not needed today — slug is baked in the webhook URL).
- **GO-LIVE reminder (unchanged):** the tools + precall land on a shop's LIVE assistant only when its assistant is (re)built — `trial-ann-admin?action=update&shop=<slug>&id=<assistantId>&secret=…` for curated, or re-provision for self-serve. New shops get everything automatically. NOT done for Greg's live line (a customer-facing flip).
- **KEY FACTS for the next phase:** platform phone formats VARY (`6155550142` vs `615-555-0103`) → always match normalized last-10. `scheduled_day` is a DATE (day-only is free); `scheduled_start` is a routing placeholder, NEVER speak it. Platform statuses = new/scheduled/in_progress/awaiting_parts/completed/canceled (no separate scheduling_status column — that's the Xano mirror). Company identity on a call comes from the **slug baked into the tool URL** (like trial-ann-lead `?shop=`), so no phone→company map table needed for the tool path (pre-call recognition WILL need dialed-number→slug). Two provisioning paths write leads via `_lib/platform-db.createLeadJob`; the assistant model/voice = `openai/gpt-5.4` / Inworld Brooke / Deepgram Flux.

## 🗓️🐜🔌 2026-08-29 (Fri, late) — SERVICEPOWER PER-TENANT FOUNDATION (platform): every shop runs ServicePower as ITSELF — the base every warranty phase builds on — READ FIRST

Teddy: "we have the ServicePower API but never really used it — no invoicing, weak parts/returns tracking. Investigate + build it better in the new Supabase SaaS." Delivered a 6-phase plan artifact (`https://claude.ai/code/artifact/8376b0db-98d8-49f7-bc14-50499b8ec4fb`), then **built Phase 00 — the per-tenant credential binding** so ServicePower runs AS each shop, not TN's vault. **Verified live on demo, foundation proven end-to-end, cleaned to zero residue.**

- **The gap:** TN's ServicePower connectors (`_lib/servicepower` dispatch SOAP + `_lib/servicepower-claims` claims REST) read creds from TN's own vault. On a multi-tenant platform each shop has its OWN ServicePower servicer login. The per-tenant credential vault (`_lib/tenant-creds`, envelope-encrypted) + `_lib/vendor-ctx` (AsyncLocalStorage) + integrations.html UI were ~90% built already. The 3 real gaps I closed:
  1. **`_lib/servicepower-claims.js` now honors vendor-ctx** — `auth()` + `retrieveClaims` read `vendorCtx.current('servicepower')` override (`ov.user_id/password/servicer_acct`) before falling to the vault. So **claims/invoicing is per-tenant too** (was the whole point — invoicing is the money phase).
  2. **`_lib/servicepower-tenant.js` (NEW, the keystone binding)** — `forCompany(companyId)` returns a bound handle whose every op (getCallInfo/updateCallInfo/retrieveClaims/**submitClaims**/…) runs inside `vendorCtx.withVendorCreds({servicepower: decryptedCreds}, fn)`. Returns `null` when the shop hasn't connected (callers cleanly skip). `credsFor` decrypts via tenant-creds; concurrency-safe so two shops' automations run at once without clobbering. **This is the base every future phase calls:** `const sp = await spTenant.forCompany(companyId); if(!sp) return; await sp.getCallInfo(...) / sp.submitClaims(...)`.
  3. **`platform-servicepower.js` (NEW)** — tenant-authed door: `do=status` (is SP connected for this shop?), `do=ping` (live 1-day getCallInfo AS the shop → authenticated?), `do=claimcheck {call}` (retrieveClaims as the shop). Auth = Supabase session Bearer → app_user company/role, OR `?secret=<admin>&company=<uuid>` for automation. Reads only; no writes.
- **VERIFIED live on demo (bogus creds `zztest01/ZZA00001`, intentionally — rejected IS correct; real creds are the Teddy step):** connect stored the secret **encrypted** (`secret_enc` ciphertext, `enc_v:dek1`; keyring `wrapped_dek` present, `kek_v:ded` — envelope: DEK wrapped by the vault KEK, DB never holds a usable key); meta carried ONLY non-secret `user_id`+`servicer_acct` (**no password key, no plaintext anywhere**); `verify=1` made a **live round-trip** → bogus → status `error`; **`do=ping` decrypted the shop's creds → live dispatch SOAP getCallInfo AS the shop** (http 200 from live ServicePower, bogus correctly NOT authenticated); **`do=claimcheck` ran the claims REST connector AS the shop** ("Your User ID or Password is invalid." — live rejection with the shop's creds → invoicing side is per-tenant); no-context regression intact (`vendorCtx.current()`={} → TN vault fallback unchanged). Cleaned integration+keyring to 0/0.
- **owner.html** header nav gained a **🔌 Vendors** link → integrations.html (office-board already linked it).
- **⏭️ NEXT (the phases, from the artifact):** 01 API intake + auto-accept · 02 status push · **03 invoicing/claims submission** (the money phase — `submitClaims` is already bound per-tenant; the platform is a BETTER place to auto-invoice than TN because the tech app natively captures TDR + completion, whereas TN's completion signal is dead) · 04 payment reconciliation · 05 parts + returns.
- **FOOTGUNS:** ServicePower vaulted TN `SERVICEPOWER_USER_ID` is likely a gmail address = WRONG (must be the ≤10-char ServiceDispatch UserID); creds ride INSIDE the request (UserInfo XML for SOAP, `authentication:{userId,password}` JSON body for claims), never a header; `manufacturerName="SQUARE TRADE"` (with the space); key=callNumber=dispatch#; SP004=invalid auth. **Magic-link + the slow verify SOAP round-trip (10-20s) flake a bash test's urllib helper** — prove the path with deterministic curl + sb-admin-sql, not a session helper.

## 🗓️🐜📧 2026-08-29 (Fri, late) — WARRANTY-EMAIL INTAKE (platform): emailed dispatches auto-land on the shop board — the feature that wins warranty shops — READ FIRST

Teddy: emailed warranty dispatches auto-appearing on the board is make-or-break for many shops ("determines whether companies come to us"). Built it multi-tenant on the platform, greenfield. **Verified live on demo — 9/9.**

- **The flow:** warranty co. emails `‹slug›@jobs.assistant247.net` → a **Cloudflare Email Worker** (catch-all, `cloudflare/email-intake-worker/`, postal-mime) parses the MIME + any XML attachment → POSTs to **`platform-email-intake.js`** → resolves the shop by the address's local part (=`company.slug`), lands the job on that shop's board (service key, `company_id` stamped in code), **idempotent per Message-ID**, **deduped by claim# (or dispatch#)**, and logs to the `email_intake` ledger. The worker **forwards a copy to a human inbox on failure** so a dispatch is NEVER lost.
- **`_lib/warranty-email.js` — the universal parser, three tiers best-first:** (1) **AHS/Frontdoor XML** (JS port of the proven `ahs_email_intake` tag-splitting), (2) **ServicePower/SquareTrade** (reuses the proven `_lib/parsers/servicepower.js` state-machine — note its fields nest under `dispatch.customer.*`, and the real format is `Key␠Value` space-delimited, NOT `Key: Value`), (3) **Claude fallback (Haiku 4.5)** that reads ANY dispatch format we have no parser for → normalized job. **That fallback is the moat:** onboarding a brand-new warranty company (Cinch, NSA, 2-10…) needs ZERO code. Non-dispatch emails (payment/status) are classified + skipped, no job.
- **`docs/sql/044_email_intake.sql` (APPLIED):** `email_intake` ledger (audit + idempotency, unique `(company_id, message_id)`, RLS select company-scoped) + `job.dispatch_id` / `job.service_window`. Brand/model/serial → `unit.attributes`; appliance → `unit.kind`/`label`; warranty fields → `job` (warranty_company + claim_number already existed from 033).
- **owner.html "📥 Emailed jobs" card:** shows the shop their `‹slug›@jobs.assistant247.net` address (one-tap copy) + a live feed of what came in (vendor · status · confidence), reading `email_intake` RLS-scoped. The warranty owner's "does it just work?" reassurance.
- **VERIFIED live on demo — 9/9:** AHS XML → correct job (claim, Whirlpool refrigerator, full address, service window); ServicePower/SquareTrade → correct job (GE dryer, model/serial); **an unknown "Cinch" format → Claude fallback created the job (method=claude, high confidence)**; idempotency (same Message-ID → once); claim-dedup (same claim, new Message-ID → reuse, no dup); non-dispatch → skipped. Cleaned to zero residue.
- **FOOTGUNS burned:** (a) a bad `require('./secrets')` (should be `./_lib/secrets`) made Netlify's esbuild bundler silently DROP the whole function → 404, not 500 — a mis-pathed require excludes the function. (b) Sonnet extraction blew the ~10s sync-function limit → "Connection reset"; fix = Haiku 4.5 + `[functions."platform-email-intake"] timeout = 26` in netlify.toml. (c) Claude helpfully strips test-prefix noise ("zzGloria" → "Gloria"), so zz-based cleanup misses LLM-parsed rows — clean by `source='warranty_email'` instead.
- **GO-LIVE (Teddy, ~15 min Cloudflare, code is done):** `docs/warranty-email-intake-golive.md` — enable Email Routing on `jobs.assistant247.net`, `wrangler deploy` the worker with secrets `PLATFORM_EMAIL_SECRET` (= a platform vault key of the same name) + `FALLBACK_INBOX`, bind the catch-all to the worker. Then each shop adds ONE forward rule to `‹slug›@jobs.assistant247.net`.

## 🗓️🐜📩 2026-08-29 (Fri, late) — OFFICE ANT: "text Lee the Larry Johnson job link with a note — I need this completed asap" (send a tech to a job hands-free) — READ FIRST

The front-desk companion to the driving flow: the office/owner tells their Ant *"please text Lee the Larry Johnson job link, I need this completed asap"* and Ant resolves the tech + the customer's job and texts that tech a one-tap link straight to the job with the note.

- **New mgmt-seat brain tool `text_tech_job`** (`platform-ant.js textTechJob`): resolves the tech by name → their cell (technician→app_user.phone), resolves the customer by name → their OPEN (non-terminal) job, then texts `https://tnapplianceexchange.net/platform/tech-job.html?job=<id>` + the office's note via `sendSms(...,'technician','platform_office_to_tech')`. Logs a `thread_message` (channel `assign`, sender `office`) on the job so the board shows the office pinged the tech. **Not a ledger intent** (a text can't be unsent) — same shape as the tech's `text_me_reminder`. MGMT-only (gated in both the tool and the exec branch); available to office/owner in **act** and **propose** modes (both docks default to act).
- **Disambiguates instead of guessing:** unknown tech → returns the real crew list, texts NO ONE; two jobs for the same customer → returns candidates so Ant asks "which one?" (never blasts both); tech name → **token-forgiving match** (`"Lee Harding"` still resolves via the `lee` token) so a fuller name the brain supplies still lands on the right tech.
- **FOOTGUN burned — the brain hallucinated a crew.** claude-sonnet-4-5 kept inventing the TN Appliance roster (Lee Harding / John Houk / the Pivaceks — a DIFFERENT company, likely in its training) instead of the demo tenant's real crew, which made it (a) answer "no such customer" from the snapshot without calling the tool and (b) expand "Lee" → "Lee Harding" and pass a name the tool couldn't match. Two fixes: a hard **GROUNDING** rule ("the only people who work here are the snapshot crew + tool results — never name or expand a tech/customer the tool didn't return; pass names exactly as said") and the office instruction now says **ALWAYS call text_tech_job** (it searches the FULL records; the snapshot only lists a few unscheduled jobs, so never say "not found" from the snapshot). The token-forgiving tool match is the belt-and-suspenders so even a hallucinated fuller name still resolves.
- **VERIFIED live on demo — 9/9:** the exact ask texted zzLee the Larry Johnson job link with the note "I need this completed asap" (thread_message logged on the right job), an unknown tech ("Zaphod") texted no one, and two Johnson jobs made Ant ask which rather than blast both. Cleaned to zero residue. Residual cosmetic-only: in the tech-not-found error path the reply can still embellish the crew list from training rather than relay the tool's real crew — but it never texts the wrong/unknown tech, so no action goes astray.
...
### ✅ BUILT + LIVE-PROVEN — the ledger foundation (build step #1 of the partner)
The one mechanism that makes "act AND reverse" safe: **every change routes through ONE server path that captures the BEFORE value, applies it, and logs a row — so every action carries its exact before-state and can be undone.** Ant never writes raw data; it only calls whitelisted **intents** that map 1:1 to controls the owner already has.
- **`docs/sql/043_owner_actions.sql` (APPLIED):** `owner_action` ledger (company_id, actor `owner|office|manager|ant`, via `ui|ant`, intent, label, target_table, target_id, path, op `update|insert|delete`, before_value/after_value jsonb, status `applied|reversed`, reason, timestamps). Tenant RLS: select company-scoped; insert/update **management-gated** (owner/office/manager/admin, mirrors 041). Plus **`platform_whoami()`** SECURITY DEFINER RPC → (company_id, role, technician_id) in one call, granted to authenticated.
- **`netlify/functions/platform-owner-action.js` (NEW, LIVE):** `?do=apply|undo|list|intents`. Verifies the caller's Supabase session → `platform_whoami` → **gates to management** (a tech token is rejected). Writes with the service key but **always company-scoped in code** (never cross-tenant). 8 initial intents, each 1:1 with an existing control: `set_parts_margin`, `toggle_comms`, `set_comms_text`, `set_goal` (take_home cents / rating — the goals model, no schema), `set_tech_commission`, `set_tech_stops`, `set_tech_area`, `add_tech_dayoff`. Generic reversal: settings-path updates restore the prior sub-value (or remove the key); `insert` ops (day-off) reverse by deleting the inserted row. `via:'ant'` stamps actor=ant so the ledger reads "what Ant did."
- **VERIFIED live on demo — 23/23:** apply set_parts_margin→55% (owner_action logged before→after, settings updated, actor=ant) → **undo restored the exact before (null/clean) + row status=reversed**; comms toggle off→undo (key removed); set_goal $12k stored; list returns recent; **add_tech_dayoff insert→undo deletes the row**; **a TECH token is REJECTED (role not allowed) and could not change the margin**. Cleaned to zero residue (throwaway tech + all owner_action rows removed, demo settings restored).
...
### 📈 2026-08-28 (cont.) — DAILY OWNER USAGE DIGEST (minutes + texts vs plan) — keeps the provider a black box
Teddy's ask: notify the owner once daily of their month-to-date minutes + texts. Built **customer-framed** (usage vs their plan allowance) — NEVER cost/margin/the underlying provider — so the owner learns their usage from US, keeping the phone provider (Telnyx) + per-unit cost ours (Teddy's stated short-term moat: "if they know what we know they don't need us"). **NOTE: this only works if the tenant runs through OUR managed path — a Telnyx-direct tenant sees raw Telnyx dashboards. Teddy is UNDECIDED on Telnyx-direct-vs-we-run-it; do NOT set anyone up on Telnyx directly.**
- **`usage-meter.ownerDigest(companyId)`** — MTD minutes/texts vs included allowance (no cost).
- **`platform-usage-digest.js`** (HTTP-callable core, owner-gated, `?dry=1`) + **`platform-usage-digest-cron.js`** (scheduled daily 13:00 UTC / ~8am CT). SPLIT because a function with its own `schedule` block **edge-403s on manual HTTP** (Netlify footgun) — core stays curlable, cron self-fires the live run. **SHADOW-first:** skips zero-usage tenants (no $0 emails), dedupes once/day (a `usage_event` kind=`owner_digest` marker), only truly emails when `PLATFORM_USAGE_DIGEST_LIVE=true` **AND** send-email's `EMAIL_ENABLED=true`. Owner email from `app_user` role=owner. **Verified live:** dry-run processed 6 tenants, all correctly skipped `no_usage`, mode shadow.
- **`platform/owner.html`** — a **"📈 This month"** card (call minutes + texts vs allowance, progress bars, near-limit heads-up) reading the RLS-scoped `usage_event` + `client_plan` client-side (authenticated own-read confirmed).
- **⏭️ To go live:** tenants must be running phones through our metered path (usage_event filling) → flip `PLATFORM_USAGE_DIGEST_LIVE=true` + `EMAIL_ENABLED=true` (needs SES DNS verified). Until then it's a ready mechanism that emails no one.

### 🐜💵 2026-08-28 (cont.) — MONEY LOOP CLOSED: mark-paid payout · warranty-remittance split · tech leaderboard
The three pieces that finish the pay spine end-to-end (billed → collected → tech paid → ranked). All on `main`, verified.
- **Office "mark paid"** (`owner.html` pay table): per-tech **💵 Mark $X paid** button writes a **`tech_payout` per collected-unpaid job** → the tech's `paid` state fills, `owed` drops to 0, per-job pay history builds. Verified via pay-calc: owed $60 → after payout owed $0 / paid $60 / state 'paid'.
- **Warranty-remittance split** (`office-board.html` **💵 Remittance** sheet — the accuracy hinge): **SQL 023** adds `invoice.collected_cents` + `paid_ref` (backfilled collected=total for already-paid). The CSR enters an EFT/check ref + splits the batch back to each warranty job (real money collected per claim, usually < billed) with a **live reconcile tally** (allocated vs received). Sets `collected_cents`/`status=paid`/`paid_method='warranty_eft'`/`paid_ref` per job. **Design: the shop's Collected + Take-home go to the REAL money; the tech still earns his full labor cut — the SHOP absorbs the warranty haircut, not the tech** (how real shops run it, and keeps tech pay clean). office-board writes `collected_cents=total` on full self-pay; owner money now reads real `collected_cents`.
- **Tech leaderboard** (`tech.html`): **⚡ Workhorse** (jobs done) + **🥇 First-Stop King** (FTF %, min-5-jobs fairness) from real data, own-rank highlight, honest "coming soon" for the crowns needing un-mirrored data (cleanest work / upseller / attach / CSAT / diagnosis). Performance only, never pay; company-scoped.
- **Demo left at "Joey owed $60"** (0 payouts) so the Mark-paid button is clickable live. All 3 surfaces serve 200.
- **⏭️ NEXT:** reconciliation check (flag app-said-vs-paid gaps per period); wire the remaining leaderboard crowns as add-on/CSAT/callback/brain data mirrors in; self-serve signup + Stripe billing driving `company.features`.
...
### 🏪 THREE MORE PLATFORM TENANTS STOOD UP (trade-agnostic proof #3, #4, #5)
Provisioned via `platform-provision?action=provision` (server-side, PLATFORM_SUPABASE_SERVICE_KEY, idempotent by slug). Each = a FULL tenant (plan=office), board seeded with a **trade-aware** sample card, isolation verified via `?action=tenants`:
- **Music City Aquatics** (Brandon Pack, **aquarium**, slug `music-city-aquatics`, company id `433cbf40-1678-41a6-b918-187b4835429c`) — Ant's best friend; reef-service moonshot (`docs/reef-service-moonshot.md`).
- **Mid Tenn Furniture** (Brandon Brewer, **furniture**, slug `mid-tenn-furniture`, company id `f5cfe722-f67c-4068-bbbd-3233ec2fcd38`) — delivers + custom orders; board = order/delivery pipeline.
- **NextGen Motors** (Jake Ihmeidan, **dealership**, slug `nextgen-motors`, company id `f2de80cc-7a2b-48bb-b4ae-13e2db23fd1e`) — used-car lot (work vans), Mt. Juliet TN. **NEW `dealership` trade added** to `platform-provision.js` addtrade map (unit=vehicle; sales pipeline lead→test-drive→financing→sold) — a car lot is a SALES business, distinct from Greg's auto REPAIR. Aquarium + furniture trades were already staged.
- **`platform-provision.js` seed made trade-aware** (sample card reads "Reef tank — monthly maintenance" / "Custom sectional — special order" / "Interested in a cargo van — has a trade-in" / "Alignment + front brakes" per trade).
- **⛔ STILL NEED to go fully live per shop: each owner's EMAIL** (login creation requires `owner_email`; I can provision company+board+seed without it, but not the login). Registry stubs in `_lib/trial-shops.js` record all 3 company ids + note "needs email + cell + hours." Their Ann phone assistants aren't built yet either (need cell/about/number plan).
- **3 onboarding-guide artifacts published (all signed "— T", NOT Teddy — they know him as T):** 🐠 Music City Aquatics `https://claude.ai/code/artifact/a674bd1b-767a-474e-8100-aa33e2e2e273` · 🛋️ Mid Tenn Furniture `https://claude.ai/code/artifact/3ab879aa-7e9c-4800-9350-8c1333144380` · 🚐 NextGen Motors `https://claude.ai/code/artifact/3be7419a-cbcd-4763-bd50-0668b22a3662`. **⚠️ Artifacts are private — Teddy must hit Share on each before texting the link.**
- **Pitch texts + SEO one-liners drafted (chat only, all as "T")** for Brandon Pack, Brandon Brewer, Jake — ready to send once shared. NOT yet folded into one block (Teddy said "not yet").

### 🔎 BUDDY WEBSITE/SEO AUDIT (the 4 shops)
Pulled each shop's web presence: **Classic Automotive (Greg) = NO website** (biggest clean-build win). **Music City Aquatics = NO website** (build-from-zero + reef angle). **Mid Tenn Furniture = has Shopify** `midtennfurniture.com` (improvable: no blog/schema/local pages — content layer, not rebuild). **NextGen Motors = dealer CMS** `nextgenmotorstn.com` (403s bots; wins = GBP + reviews + van-niche content, CMS limits on-page). Takeaway: the two with no site are where we move the needle most, and it's a natural Ant-platform bolt-on.

### 🔎 TN's OWN SEO — dryer + dishwasher hub FAQs deepened
Pulled `seo-scorecard` (live): **1,024 pages surfacing (▲+64), 210 on page 1 (▲+33), 34,602 impr/wk. DIY /fix conversions 7d = 0.** Top levers: striking-distance "appliance repair" #5/873, "appliance repair nashville" #5/554 (lever = **reviews naming the city**, the #1 map-pack move); content gaps "dryer repair" #21/183, "dishwasher repair" #23/138. **Built `tools/seo-build/deepen-repair-hubs-2026-08-25.js`** (idempotent, MARK `<!--FAQ25-->`): added 6 honest People-Also-Ask Q&A to EACH flagship hub (dryer-repair.html + dishwasher-repair.html), synced across the visible `.faq` block AND the FAQPage schema (**11 questions each, JSON validated**), each linking into the `/fix/` symptom cluster (all targets confirmed to exist), + sitemap `lastmod` bump. Honest note: these are competitive head terms — this wins long-tail + FAQ rich results + cluster signals, not a #21→#3 jump (that's the map-pack/reviews fight).

### 📍 TN GOOGLE BUSINESS PROFILE — UPGRADE (all via API, on-demand fns; not the scheduled 403 ones)
Teddy said "80/100" — audited via `gbp-profile` (GET) + `gbp-reviews`: profile is **near-maxed** (primary cat correct, 3 extra cats, 24/7, service area 20/20 = Google's cap, 16 services, strong description, verified, since 2012; reviews **4.5★/1,091, healthy velocity — 10 in 30d/43 in 90d**). The missing ~20 pts = dynamic engagement (photos recency/variety, Q&A empty, social links, products, attributes). Actions taken:
- **Published a fresh GBP post** via `gbp-post` (on-demand; the `gbp-post-generator` is scheduled=403). CALL CTA, no-warranty/24-7/DIY/EPA-608 hook.
- **Posted 2 privacy-screened job photos** to GBP. `gbp-photo-autopost` is the engine (sources `customer_sms_media_captured`, Vision-screens for no-people/no-interiors/no-serials) but it's scheduled (403 manual) + shadow-only. So I replicated it on-demand: pulled the job photos from Xano, **screened ~13 myself with vision — only 2 passed** (a dryer control panel + a new OEM compressor in the box; the rest were model-sticker shots w/ readable serials, phone screenshots, or customers' personal food — exactly why the gate is strict), posted both via `gbp-post` with `mediaUrl`.
- **Replied to the 1 unanswered review** (Mike Key, "great job Jimmy") → review **response rate now 100%**.
- **Q&A — BLOCKED on Google approval, wired to self-fire.** `gbp-qanda-seed` has 8 keyword-rich owner Q&A ready but the **My Business Q&A API 404s** even after Teddy ENABLED it on the correct project (`project-fb170b47-a89e-4176-9d1` = "My First Project" = the GBP/Ads/GSC project; our endpoint `mybusinessqanda.googleapis.com/v1/locations/{loc}/questions` is correct, so NOT a code bug). Cause = legacy-API activation/quota lag (project is already approved for GBP APIs — posts/reviews work — so likely just needs hours to propagate, OR the Q&A API needs its own quota via the GBP API contact form "Application for Basic API Access"). **FIX WIRED: added a daily schedule for `gbp-qanda-seed` in netlify.toml (`47 15 * * *`)** — self-authorizes (next_run), idempotent, so it auto-posts all 8 the moment access lands. **⏭️ Teddy: check the profile tomorrow; if still no Q&A by tomorrow PM, submit the GBP API contact form (Basic API Access) for project `project-fb170b47-a89e-4176-9d1`, email tnappliancerepair@gmail.com, "our own business," My Business Q&A API.** Kill switch: vault `GBP_QANDA_SEED=false`.
- **⚠️ FOOTGUN reaffirmed:** scheduled Netlify fns edge-403 on manual HTTP — so `gbp-post-generator`/`gbp-photo-autopost`/`gbp-review-responder` can't be curled; use the on-demand twins (`gbp-post`, `gbp-reviews` POST) or replicate their logic. `gbp-post`/`gbp-reviews`/`gbp-profile`/`gbp-qanda-seed`(pre-schedule) are on-demand.

### 💡 "ADD ANT TO THE SAAS TOO" (Teddy's idea — productize the GBP engine)
Teddy wants the whole Google-growth engine (health score, auto-posts, review auto-reply, privacy-screened photo autopost, Q&A seeding) as a **per-tenant module of the Ant platform** for the shops we onboard — each tenant clicks ONE "Connect Google" (GBP OAuth) and Ant runs the same playbook on THEIR listing. Everything's already built for TN; productizing = OAuth-per-tenant + a dashboard tile. Honest gating: each shop grants GBP access + Google's allowlist/quotas apply; the third-party/agency access tier (to manage OTHER shops' profiles at scale) is a bigger GBP API approval than TN's first-party one. **⏭️ OFFERED to spec the "Connect Google / Local SEO" module as a build doc — Teddy hasn't greenlit yet.**

### ⏭️ NEXT / OPEN
1. **Get emails** for Brandon Pack, Brandon Brewer, Jake → mint their logins (one command each). Then their cells/hours/about → build their Ann assistants. Greg still needs his cell too. Jake's lot is Mt. Juliet not Antioch (confirm).
2. **Q&A self-fires** once Google grants access — watch the profile.
3. **GBP score → 100:** biggest remaining = categorized Photos-tab uploads (logo/cover/team/at-work — Teddy in the GBP app, API can't) + social links + attributes; and flip `GBP_PHOTO_AUTOPOST_LIVE=true` for hands-free job-photo posting. The real map-pack lever = review velocity naming the city.
4. **Spec the "Connect Google" Ant SaaS module** (offered, awaiting go).
5. **Share the 3 onboarding artifacts** + send the drafted pitch texts.
6. Carryover: OPEN whether to flip both/all Anns to Claude Haiku 4.5 (Teddy never answered); Marshall Reddick (MRRE) vendor onboarding (task #13).

## 🗓️🐜🚀 2026-08-24 (Mon) — THE ANT SAAS PLATFORM IS LIVE (multi-tenant Supabase: office + tech + customer, three lenses one DB) · TRIAL-ANN SCAFFOLD (Joey/Greg) · PHONE→BOARD BRIDGE · À-LA-CARTE PRICE BUILDER — READ FIRST

Big strategic arc: stood up the **sellable Ant platform for other shops** from scratch, end-to-end, and it's **LIVE + demoable**. This is the "database for the masses" (L2 SaaS) becoming real. All on `main` + `claude/shop-automation-setup-r9wzpm`.

### 🟢 THE PLATFORM IS LIVE (Supabase project "ANT Platforms", SEPARATE from ANT OPS archive)
- **Substrate decision (canonical): `docs/multi-tenant-platform.md`.** Build the SaaS on **Supabase** (Postgres + Auth + **RLS** + Realtime + PostgREST) — it enforces tenant isolation at the DATABASE (a buggy query can't leak Shop A's rows to Shop B), which Xano cannot. This SUPERSEDES the Xano-retrofit plan in `docs/multi-tenant-migration.md`.
- **Schema (RAN LIVE in the new project):** `docs/sql/004_multitenant_core.sql` (WIDE/trade-agnostic core: `trade_profile` [appliance + automotive seeded — a new trade = a new ROW, never a schema change], `company`, `app_user`, `customer`, `unit` [the serviced thing; trade fields in `attributes` jsonb], `technician`, `job`, `invoice`+`invoice_line`, `thread_message`, `event`; RLS on every table via `current_company_id()`/`current_app_role()` SECURITY DEFINER resolvers; `create_company_with_owner()` self-serve provisioning) + `005_portal_access.sql` (customers reach the portal WITHOUT a login: office mints a `portal_grant` token; `portal_get(token)`/`portal_post_message(token,body)` SECURITY DEFINER, anon-callable, scoped to that one customer) + `006_demo_seed.sql` (seeded "Joey's Appliance Repair" demo tenant: Joey as owner+tech, 3 customers/units/jobs across statuses, a conversation, a portal link).
- **THREE SURFACES BUILT + LIVE (repo `platform/`, served by Netlify):** `office-board.html` (staff Auth → RLS-scoped kanban → Realtime), `tech.html` (tech's day; Start/Awaiting-parts/Complete + tap-to-call/navigate + text-the-customer), `portal.html?t=<token>` (customer status + two-way thread, per-shop branded from `company.settings`). All three = ONE pattern: sign in / token → query → RLS scopes to your tenant. **Config: `platform/config.js`** (Supabase URL + **publishable** `sb_publishable_…` key — browser-safe, RLS protects data; NEVER the secret key). `netlify.toml` CSP `connect-src` now includes `https://*.supabase.co`.
- **PROVEN END-TO-END on Teddy's phone (2026-08-24):** signed into office-board + tech, opened the customer portal, and the **magic loop landed** — a "Text the customer" from the TECH app appeared instantly in the CUSTOMER portal thread. Three windows, one live database. **This IS the product to show Joey.**
- **Project facts:** URL `https://tntbhfwitytkcoqlejwc.supabase.co`; publishable key vaulted in `platform/config.js`. New Supabase API-key model = **"Publishable key" (sb_publishable_…) is the new "anon public"; "Secret key" (sb_secret_…) is the new service_role** — keep secret out of the browser.
- **FOOTGUNS burned:** Supabase "destructive query" warning on `drop … if exists` is expected on an empty DB (Run anyway); a find-replace-all of the seed's UID placeholder also clobbered the guard's compare string (the 006 shipped now AUTO-RESOLVES the UID via `select id from auth.users order by created_at desc limit 1` — no UID pasting); the office board/tech app show ALL a tenant's jobs (the tech-scoped-jobs RLS variant is a commented worked-example in 004 to graduate to later).

### 📞 PHONE→BOARD BRIDGE (makes phones + database ONE product) — ✅ LIVE (vaulted + tested 2026-08-24)
- **`_lib/platform-db.js` `createLeadJob({slug,name,phone,what,detail,city,source})`** — server-side (Netlify), uses the Supabase **SERVICE key from the vault** (bypasses RLS, never in browser) to turn a lead into a real JOB: upsert customer → unit → job (status `new`) → thread_message → mint portal_grant. Best-effort/no-op-safe (missing config → `{ok:false}`, caller still runs).
- **`platform-lead.js`** — THE platform intake endpoint. `POST {slug,name,phone,what,detail,city}` (or `GET ?slug=demo&name=&phone=&what=&detail=` for a browser test) → creates the job on that shop's board + returns the portal_url. Any lead source (Ann, web form, office quick-add) calls this.
- **`trial-ann-lead.js` wired:** when a trial shop's config has `platformSlug`, every lead Ann captures ALSO lands on that shop's board + the owner's SMS carries the customer portal link.
- **✅ ACTIVATED:** vaulted **`PLATFORM_SUPABASE_URL`** = `https://tntbhfwitytkcoqlejwc.supabase.co` + **`PLATFORM_SUPABASE_SERVICE_KEY`** = the ANT Platforms **Secret key** (`sb_secret_…`). ⚠️ **PLATFORM-prefixed names on purpose** — the generic `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` already point to the ANT OPS archive project; don't reuse them. Tested live 2026-08-24: `…/platform-lead?slug=demo&name=…&phone=…&what=…&detail=…` → `{ok:true, job_id, portal_url}` and the card landed in the demo board's "New" column. (`platform-lead` gates real shop slugs behind `TELNYX_TOOL_SECRET`; **slug=demo is open** as a sandbox. Service key = vault only, never chat/browser.)

### 📱 TRIAL-ANN SCAFFOLD (the phones we sell other shops) — BUILT, awaiting Joey/Greg details
- Joey Grover (appliance) + Greg Long / Classic Automotive (automotive) both want an **AI CSR**. The trial Ann = the simple tier: answers 24/7, fields FAQs from a shop `about` block, captures the lead, texts the owner (no database/scheduling/warranty — that's the full Ant). Files: `_lib/trial-shops.js` (per-shop registry — name/type/ownerCell/area/hours/about/**platformSlug**; EMPTY pending their details), `trial-ann-lead.js` (the ONE tool: capture_lead → owner SMS + optional board job; message_owner), `trial-ann-admin.js` (parameterized create/update/bind a Telnyx AI assistant from an **appliance OR automotive** persona template — automotive captures year/make/model; Inworld Max Brooke voice, gpt-5.4, Deepgram Flux). **To stand a shop up:** add a registry entry → `trial-ann-admin?action=create&shop=<slug>` → `…&action=bind&id=<id>&number=+1…`.
- **⏭️ NEED per shop before go-live:** exact business name, owner cell (leads) + email, service area, hours, number plan (buy a Telnyx line or forward their business line), and for Greg general-vs-classic scope. Both messaged 2026-08-24, awaiting replies.

### 🧾 À-LA-CARTE PRICE BUILDER (sales leave-behind) — Artifact
`https://claude.ai/code/artifact/2281af92-840a-4994-9f85-55adf31fe227` — check/uncheck feature configurator with a live monthly total (Ann Answering "sell now" tier · Full Office platform · Add-ons · Own-Your-Area territory). Prices are **editable placeholders** — Teddy sets the real numbers. Also `service-pricing.html` artifact (3-tier sheet) from earlier.

### 💡 ECONOMICS (locked): **phones scale cheap, sell now to everyone; the database is the ONE heavy build — build it ONCE (done today), funded by phone revenue + validated demand.** TN Appliance = platform tenant #1 (future shadow-migration); Joey = tenant #2 (the forcing function that keeps the core generic).

### ⏭️ OPEN / NEXT (the platform roadmap)
1. **Vault the service key** (above) → the phone→board bridge goes live; test via `platform-lead?slug=demo`.
2. **Lock Joey/Greg details** → stand up their trial Anns.
3. **Wire real TN Ann** (`telnyx-precall-context`/`telnyx-ai-tool`) to also write leads into the platform once TN is a tenant (today the bridge is via the trial-Ann tool + `platform-lead`).
4. Build the **write** side of the surfaces (add-customer/add-job/schedule forms — today they're read+status+message); **self-serve signup + Stripe billing** driving `company.features` (the à-la-carte builder maps 1:1); the **tech-scoped-jobs RLS** graduation; the **cross-tenant flywheel** views (common failures/parts by model across shops) at ≥3 tenants.
5. Open questions in `docs/multi-tenant-platform.md`: tech auth (PIN/magic-link vs full login), Realtime-vs-poll, billing timing.
- **DON'T:** put the Supabase **secret** key in `platform/config.js` or chat (publishable key only in the browser); run platform SQL in the ANT OPS archive project (use the "ANT Platforms" project); assume the surfaces enforce per-tech row scoping yet (tenant isolation yes, intra-tenant tech scoping is the commented next step).

## 🗓️🐜📦🔎📞 2026-08-18→19 (Mon/Tue, Teddy in the field) — AMAZON DROP-SHIP ENGINE (auto-placer + ASIN worklist) · 13 NEW /fix/ PAGES · AGE-AWARE SEO TRIAGE · PHONE-NUMBER REVERT (my mistake) · JIMMY REPEAT-VISIT HISTORY FIX · PHONE WHISPER ON→OFF · TESLA WATCH — READ FIRST

Teddy fed discrete asks between repair jobs. All on `main` (Netlify auto-deploys; deploy queue was badly backed up today — many poll cycles 404'd before functions landed).

### 📦 AMAZON DROP-SHIP — the loop is now built end-to-end (shadow-safe, gated on Amazon's production flip)
Both halves of the missing turnkey drop-ship machinery shipped:
- **`amazon-autoplace.js` (NEW) + `-cron.js` (*/15 wrapper)** — mirrors `parts-autoplace` for the Amazon branch. Watches `parts_orders` (order_status=`to_order`, supplier=`amazon`, notes.ship_to=`customer`). **DOUBLE-GATED:** `live = AMAZON_AUTOPLACE_LIVE===true && env===production && isConfigured && !dry`. Rows without an ASIN → `needs_asin` (never places blind). Places via `amazon-business-order {order_id, asin, quantity, live:true}`; idempotent (that fn marks the row ordered). Kill: `AMAZON_AUTOPLACE=false`. Live shadow test found 1 candidate (order 537320, part 5303935066, job 21267, needs_asin).
- **`amazon-asin-resolve.js` (NEW) + `amazon-asin.html` (NEW, office worklist)** — the ASIN last-mile. Amazon orders by ASIN, not part#, and Amazon blocks server-side catalog search (datacenter IPs → anti-bot shell). So the office gets a 10-sec assist: `?list=1` returns Amazon orders needing an ASIN with a ready `amazon.com/s?k=<part#>` search link; paste the `/dp/B0…` ASIN back, Save → the auto-placer ships it next run. Admin-gated, noindex.
- **⏭️ GO-LIVE:** Amazon grants production → flip `AMAZON_BUSINESS_ENV=production` + `AMAZON_AUTOPLACE_LIVE=true`. Connector already proven (`configured:true`, token mints). Full hands-off ASIN resolution = a Mac parts-daemon follow-on (logged-in Amazon browser), documented not built.
- **`amazon-api-watch.js` fixed** — it had false-flagged our OWN outbound follow-up email as "possible Amazon API email." Query now excludes `-in:sent -from:tnappliancerepair@gmail.com -from:tnappliance@gmail.com`. (The "new Credential" email Teddy got = a Small Business badge on the SELLER profile, NOT the Ordering API — no action.)

### 🔎 SEO — AGE-AWARE TRIAGE (proved "218 dead pages" was a mirage) + 13 NEW /fix/ PAGES
Teddy's strategy locked: **AI + DIY leads** (not organic blue links) → `/fix/` authority content that AI engines cite + a DIY parts funnel. So we do NOT prune young pages.
- **`gsc-triage.js` rewritten age-aware:** parses `<lastmod>` from the sitemap; 0-impression English pages younger than `minAge` (default 45d) → `too_young` (HELD, not pruned); no-lastmod → `prune_unknown_age`. Live result: **prune=0, too_young=139** — the newborn /fix/ pages are correctly held, nothing to cut.
- **13 new `/fix/` pages** appended to `scripts/troubleshooting-content.js` (81→94) + rebuilt: refrigerator-ice-maker-leaking-water, refrigerator-door-seal-not-sealing, freezer-leaking-water-on-floor, washer-smells-like-mildew, top-load-washer-not-agitating, dryer-tripping-the-breaker, dishwasher-leaking-onto-floor, dishwasher-not-filling-with-water, oven-smells-like-gas, glass-cooktop-cracked, electric-burner-wont-turn-off, microwave-sparking-inside, garbage-disposal-leaking. Each with FAQ+HowTo+Breadcrumb+Article schema; gas/240V/sealed route to Pro. Sitemap 826→839 (dated 2026-08-18), all live + JSON-LD validated.

### 📞 PHONE-NUMBER SWEEP — I MADE A MISTAKE, REVERTED IT (standing lesson)
Teddy: "make sure we have the correct phone numbers on all pages" → then "where the area code is for that area should show that local number." **I first mis-audited the per-metro `-1234` numbers as fake placeholders and normalized 1,353 pages to 888 — WRONG.** A Telnyx inventory pull proved those are REAL active local lines routing to Ann (629=Nashville, 504=NOLA, 225=BR, 985=North Shore, 931=Clarksville). **`git revert` of the fake-fix, pushed.** The per-metro local setup was already correct (better for local SEO). Also learned my regex over-matched: `755-199-3545` was a video-filename substring, `555-1234` a form placeholder. **LESSON: do NOT bulk-normalize phone numbers — the per-metro locals are intentional; verify against Telnyx inventory before ever calling one "fake."**

### 🔧 JIMMY'S SQUARETRADE REPEAT-VISIT — history match broadened (`job-history.js`)
Jimmy hits the same customer across several SquareTrade work orders; notes "disappear" because each WO = a NEW job record AND often a NEW customer_id (ST re-intakes per work order). Danielle wants a **separate ticket per WO** (billing), Teddy wants a **link to the prior visit**. Fix: prior-visits search broadened from customer_id-only to **customer_id OR customer_phone OR service_address** (single-field searches merged, since metadata API can't AND fields), deduped. Tickets stay separate; this only READS siblings. `job-history-card.js` already renders tap-to-open links on `?job_id=` pages. **⏭️ Unverified end-to-end** (the test customer turned out single-visit) — will prove on Jimmy's next real repeat; if gaps persist, add normalized/fuzzy phone match.

### 📞 "PRESS 1 TO ACCEPT" WHISPER — built, then TURNED OFF (caused a worse problem)
Built the transfer-confirm whisper in `office-texml.js` (TeXML `<Number url=…?whisper=1>` Gathers "Press 1 to take this call"; `answerOnBridge=true` keeps caller on ringback; cascade counts answered only when `DialCallDuration>0` under `cfm=1`; gated by vault `TRANSFER_CONFIRM`). **Sofia then reported callers hang up before she can press 1** → set vault `TRANSFER_CONFIRM=off` (verified live cascade restored to plain `<Number>`). **Real fix = Sofia disables carrier call-screening / "Silence Unknown Callers" on her cell.** Flag stays available to re-enable later.

### 🚗 TESLA ROBOTAXI WATCH (Teddy interested in buying/earning)
Set a recurring watch for two triggers: (1) Tesla opens **consumer Cybercab ordering** to the public, (2) Tesla's **paid robotaxi network goes live in Nashville/Middle TN**. Baseline as of 08-19: consumer ordering NOT open (Tesla filling its own fleet first; Cybercab in mass production at Giga TX, target <$30k); robotaxi LIVE in **Austin (fully driverless), Bay Area (supervised), Dallas, Houston** — Nashville still in the announced pipeline, not commercial. **⚠️ The recurring cron is session-only + auto-expires ~7 days** — re-arm on request; Teddy can also ping "any Tesla news?" anytime for a fresh check.

### 🔌 API STATUS (unchanged, both waiting on the other party)
- **Amazon Business Ordering** — our side fully configured (token mints); only Amazon granting production gates it. Follow-up email staged.
- **Frontdoor** — dev Shivam actively building; ball in their court.
...
- **#1 "customers not knowing when tech is otw" — NOT a code break; BUILT a reliability nudge.** Verified live: the on-the-way text path is intact + un-gated, and a prominent per-stop "🚚 On my way" already exists on the daily dashboard — but **Andre tapped only 4× in 4 days** (≈1/day; should be per stop). So it's behavioral: the gap is the moment he finishes a job + drives to the next. **Built: `tech-daily-dashboard.html` — the moment a tech completes/files a job on the tile, it surfaces a one-tap "🚚 Heading to [next customer] in [city] next? — Let [name] know I'm on the way"** (`nextStopAfter()` finds the next route stop, `otwNext()` fires tech_on_the_way via tech-lifecycle). Opt-in ("Not yet"), catches the exact transition. sw-tech v49→v50. Still pair with coaching the crew to hit it every stop. (Follow-on: same prompt on tech-job.html's post-complete "next job" CTA, for techs who complete from the job page.)
...
### 🚪 AHS/FRONTDOOR — dev starts Tue 8/11 (Brian Bullock). Our side built + authenticating.
- **`docs/ahs-api-plan-2026-08-06.md` (NEW, the reference).** Two independent directions: **OUTBOUND** (FD→us: Schedule auto-intake / Status / notes / NCC) + **INBOUND** (us→FD: dispatch Status Update + note, one call does both). Prioritized use-case stack + full **automation menu** (§5b: auto-intake, status-sync, autho-gate, NCC→durable-pay-link, part-return 5-day watchdog, reconciliation audit, the money loop). Open decisions to lock with Brian.
- **✅ Status push WIRED (shadow-ready for 8/11) — the Danielle-killer.** `frontdoor-push-job.js` (NEW) = browser-safe, job-scoped wrapper: `POST {job_id, status_key, note?}` → resolves the dispatch # + vendor (822418 John / 822218 Andre / 839828 TN crew) server-side, hands off to `frontdoor-push-status` (admin secret stays server-side). No-ops for non-AHS jobs. Wired into `tech-job.html`'s warranty-push helper (`pushSP` now also fires `pushFD`) → on-my-way→EN_ROUTE, start→IN_PROGRESS, complete→COMPLETE + note. Self-gates on `isFDJob` (warranty_company AHS/Frontdoor family; SquareTrade/NSA still go only through ServicePower). **SHADOW-logs (`frontdoor_push_shadow`) until `FRONTDOOR_PUSH_LIVE=1` + the key is authorized.** sw-tech v48→v49. Verified live: bogus job → `{ok:true,skipped:"not_frontdoor"}`.
- **✅ TDR → PORTAL, Level 1 WIRED (shadow).** On **Complete**, `frontdoor-push-job` composes the WHOLE TDR server-side (diagnosis, failed part+#, work-performed prose, labor, parts-to-return) via shared **`_lib/frontdoor-tdr.js` `composeTdrNote()`** (same voice as `frontdoor-queue`) and pushes it onto the dispatch as the **note** — the report lands in the AHS portal automatically, no retyping. **Level 2 (TDR files itself as a structured estimate + invoice)** is assembled already (`frontdoor-queue` composes the fields; the SquareTrade side already auto-submits the structured claim via `servicepower-claims-build`/`-submit`) — it just needs Frontdoor to expose a **claims/estimate/invoice SUBMISSION API** → **#1 early question for Brian**.
- **✅ Note to Brian ready:** `docs/frontdoor-brian-note-2026-08-06.md` — send from Gmail to Brian.Bullock@ahs.com. Confirms we're ready + asks the 7 gating questions (account provisioned? **submission API?** which inbound endpoint? status-code list? sandbox test dispatches?) + Client ID `040c014f-…` + webhook URL + vendor IDs. **No secrets in it** (password never appears; webhook token already sent 7/13). **⏭️ Teddy sends it.**
- **⏭️ Go-live (after 8/11):** Brian links Client ID (clears 403; `frontdoor-auth-watch` texts Teddy) → Frontdoor points webhook at us → watch 2-3 shadow payloads → flip `FRONTDOOR_WEBHOOK_LIVE=1` (auto-intake) → flip `FRONTDOOR_PUSH_LIVE=1` (status + TDR-note push). Follow-on: mirror the push from the office board too (server-side, so it fires even when Danielle moves a job, not just the tech).

### 📦 AMAZON API — both tracks BLOCKED ON AMAZON; our side fully configured + authenticating (verified live)
- **Business Ordering API (the valuable one — drop-ship parts):** UNDER REVIEW by Amazon (Prashanth `chintamp@amazon.com`). Keep-warm sent 8/02; **do NOT re-nudge until ~8/11.** Verified live this session: `amazon-business-test?config=1` → ALL vault pieces present (`group_id`/`buyer_email`/`payment_ref`, `configured:true`), and `?secret=` → `token_acquired:true` (sandbox LWA mints). So the connector is proven — **only Amazon granting production + flipping `AMAZON_BUSINESS_ENV=production` gates it.** Inbox check (gmail-search from:chintamp / ab-api-access): **0 new replies** — still waiting.
- **SP-API (sell our own listings): PARKED.** Amazon closed case 21424102471 (won't link the seller account; it's a "become a Professional seller" fork, not an API bug). Associates link (`tnappliance-20`) monetizes the angle live. Don't chase.
- **The buildable-now Amazon gap** (for when production lands): `amazon-business-order.js` is a MANUAL single-order placer (needs an ASIN passed + `order_id`). Missing = **an auto-placer** (watch PAID + amazon-supplier + to-order `parts_orders` → fire the order) + **ASIN/fitment resolution** (diagnosed part → ASIN). Building these = turnkey the day production flips. NOTE: `amazon-business-test.js ?real=1` trial ship-to has Teddy's cell as the phone (pre-existing test fixture; buys nothing) — harmless but scrub if it ever goes near a message.
- **⏭️ Next Amazon session:** either (a) build the auto-placer + ASIN resolution so the drop-ship loop is turnkey, or (b) build the customer-facing drop-ship store front-end (`docs/amazon-dropship-store-plan-2026-07-18.md`) — both buildable now, both gated only on the production flip for actual placement. `amazon-api-watch` texts Teddy the moment Amazon replies.
- **📌 ERP (Exact Replacement Parts) — MOVED TO QUESTIONNAIRE STAGE (revisit in a few days).** Stacey Ball (Senior Sales Rep, `stacey@erparts.com`) emailed the **New Distributor Questionnaire (Form NDQ001)** 8/6 — "fill out, after review we'll let you know next steps." ERP = the value-tier aftermarket source that would make Amazon the *fallback*. Filled-in answer sheet drafted in chat (company info from vault: TN Appliance Exchange LLC · EIN 38-3886067 · 3137 Skinner Dr Antioch TN 37013 · NAICS 811412 · est 2012). **Teddy is STEWING on it for a few days** — the real decision is the questionnaire's **$20,000/yr purchase commitment + "maintain stock in all categories"** (cuts against the per-job drop-ship model). Also flag before sending: shipping address "cannot be residential," need 3 credit references (Marcone/Tribles/+1), and the "sell online?" framing (reseller vs repair-only). **Do NOT fill/send until Teddy decides.**

## 🗓️🐜💳 2026-08-06 (Thu, home) — PAYMENTS SIMPLIFIED: ONE tech "Get paid" card (QR/text/cash/charge) + tip on the pay page — READ FIRST

Teddy: *"simplifying the payments for the customer and for the tech to give the customer."* He picked **all four** pay options + **tips**. Built subtraction-first (his "don't break what works"): collapsed the tech page's TWO overlapping payment cards into ONE, everything on the DURABLE pay page (`pay.html`, never expires), killed the last expiring-Stripe-checkout paths on the tech side.
- **`pay-link.js` (NEW, LIVE, verified) — the field "get paid" brain.** `POST {job_id, amount?}` → mints the durable `pay.html?job=&t=` link (HMAC token stays server-side), server-renders a **QR** of it (`qrcode-generator`, zero-dep, added to package.json — verified 18.8KB SVG at build), and reads what's owed via **pay-owed** (warranty-safe: out-of-pocket add-ons only on a covered job). Optional `amount` records the self-pay invoice so the page charges exactly what the tech confirmed. No secret in the browser.
- **`tech-job.html` — ONE "💳 Get paid" card**, up top (out of the "More" fold), amount auto-filled. Teddy 2026-08-06 (round 2): *"the tech should have as many options to accept a payment as possible."* Two groups: **Card · Apple Pay · Google Pay** (our durable page) = **📱 Show QR** (customer scans with THEIR phone) · **📲 Text link** (`send-pay-link`) · **📋 Copy link** (paste anywhere) · **💳 Charge on my phone**; and **Paid you directly** (→ `record_payment_received`, marks paid) = **💵 Cash · 🧾 Check · Zelle · Venmo · Cash App**. Paid → "✅ Paid"; covered warranty w/ no extras → "🛡️ Covered". Removed the redundant "Take a payment" card + its expiring-link JS. **sw-tech v46→v48** (crew reopen once).
  - **⏭️ To light up the fee-free wallets:** drop **`SHOP_ZELLE` / `SHOP_VENMO` / `SHOP_CASHAPP`** (the shop's receiving handles) into the vault (admin-secrets.html). `pay-link` returns only the configured ones → each button appears the moment it's set, hides when unset (a tech never sees a method the shop can't take). Tapping a wallet shows the customer where to send, then records on confirm.
- **Tips on `pay.html` (customer side) — 100% to the tech.** Folded into the SAME checkout as the balance (no second payment): "💚 Add a tip for <tech>?" chips ($5/$10/$20/Other), Pay total updates live. Server split so a tip **never inflates repair revenue**: `pay-owed` now returns `technician_id`+`tech_first`; `create-stripe-payment-link` takes `tip_cents` → adds a "Tip for your technician (100% to them)" line (both durable+checkout paths, untaxed) + metadata; `record-payment` records the repair payment at the NET (tip excluded) + writes `tech_tip_paid` for the tip. Only sessions carrying `tip_cents` are affected — the existing pure-tip flow + everything else is untouched.
- **⚠️ Money path — needs a $1 end-to-end test** (pay a self-pay job WITH a tip → confirm the invoice records net + a separate `tech_tip_paid`) before wide use. Same test also covers the durable-link happy path (`PAYMENT_LINKS_DURABLE` is still default-off → uses the proven Checkout Session; flip on after the test).
- **⏭️ Follow-ons (offered, not built):** tip-only after a balance is already paid (today the tip folds into the balance checkout); office-board could reuse the same QR.
...
- **⏭️ Before wide use:** run one **$1 test** end-to-end (mint a link on a self-pay job → pay → confirm it records + flips paid).
- Office usage now: `pay-owed?job=<id>&mint=1&secret=<admin>` → copy the returned URL → text it. Durable, itemized, warranty-safe.
- **✅ PHASE 2 SHIPPED (one-tap send):** `send-pay-link.js` — one server-side call resolves the customer phone, mints the durable link (HMAC token stays server-side, never in the browser), texts it from the approved 10DLC human line (857-8800), logs to the per-job thread (opt-out absolute, idempotent resend). **office-board.html**: both "💳 Text pay link" buttons (invoice worksheet + drawer) now send the DURABLE link instead of the expiring Stripe checkout. **⏭️ Remaining surface:** add the same one-tap button to tech-job.html (fast follow). Verify (SAFE, no real send): `POST send-pay-link {job_id:99999999}` → `no_phone`.
- **⚠️ SEPARATE BUG surfaced:** `addons-for-job` HANGS on some jobs (HTTP 000 on job 20673) — pay-owed hard-timeouts around it, but it likely slows the office invoice worksheet too. Worth a real fix.
...
### ✅ PHONE-TRANSFER-OUTCOME LOGGING — "are the humans answering?" is now a number (asset-vs-cost on Sofia)
Teddy added an office person (Sofia) this week + asked "asset or cost?" — but her calls/claims/reports weren't attributed to her (calls via Vapi/Telnyx not in Xano; claims/reports have no actor). **Fix = log who catches each transferred call.**
- **`office-texml.js`** (the transfer TeXML, rings Sofia→Danielle→Teddy): on every Telnyx action-webhook callback it now logs a **`phone_transfer_outcome`** event `{ outcome: answered|missed, answered_by/missed_by, tier, dial_status, caller, call_sid, leg, at_ms }`. **PURE ADDITIVE — zero change to how calls ring** (couldn't live-test Telnyx from here). The action URL is set to `?leg=<i+2>` for the tier at index `i`, so on callback `leg=L` the tier just rung = **`tiers[L-2]`** (holds even when a tier is skipped for being off). Log wrapped in a **1200ms race** so a slow Xano can never delay the caller's next ring.
- **Known v1 gap (safe, documented):** the LAST reachable tier has no callback webhook (never did) → in the normal config only **Teddy's** final-resort answer isn't captured. **Sofia is always tier 1, Danielle tier 2 → both dispatchers' catches are EXACT every call** — which is exactly the question asked.
- **`phone-trust-scorecard.js`** (nightly ~7PM): new **`humanAnswerWindow(24)`** reads the outcome events, dedups by `call_sid` for the transfer/answered totals, counts per-person catches. Adds one SMS line when >0 transfers: **`📲 Humans answered N/M transfers — Sofia X, Danielle Y`**. Stored on the `phone_trust_score` event for the trend; also surfaced in the on-demand **`phone-score?secret=…&live=1`** reader. Exact for Sofia/Danielle; N/M can slightly undercount (Teddy-only final-tier answers).

### 🛒 "BEAT THE TECH TO THE DOOR" PARTS ORCHESTRATOR — `parts-beat-tech.js` (SHADOW; the first-visit-fix lever)
Teddy: "once a job is pre-diagnosed, order the part to beat the tech to the customer's home — the single biggest improvement." Built the missing **bridge + timing brain** on top of proven plumbing (didn't rebuild).
- **THE FORK (shapes everything):** **CASH/self-pay** = *we* order Marcone drop-ship to the home (100% ours, automatable). **WARRANTY (~95%)** = the **vendor supplies the part** — ordering it ourselves **breaks the claim**, so it's EXCLUDED by design (warranty timing = Phase 2, alert-only off the parts-watch feeds).
- **What existed** (mapped via Explore): `ant-brain-predict` (part prediction), human pre-diagnosis (Teddy Tool → `oem_part_number`/`verified_part_number` on TDR + `flag-parts-to-order` sets `jobs.parts_status='to_order'`), `_lib/msupply.js` (real Marcone lookup+ETA+placeOrder, order #74992380 proven), `create-parts-order.js`, **`parts-autoplace.js`** (the PLACER — watches `parts_orders` marcone+customer+`to_order`, places via marcone-order, SHADOW until `SELF_CHECKOUT_AUTOPLACE_LIVE=true`). **The gap:** nothing turns a pre-diagnosis into an order row, and nothing compares the Marcone ETA to the scheduled visit.
- **`parts-beat-tech.js` (shipped SHADOW-only):** hourly (biz hrs), finds cash `parts_status='to_order'` jobs with a human-confirmed part #, pulls the **live Marcone ETA**, computes **`beats_visit`** (true=lands first / false=will MISS→reschedule / null=not-scheduled→order-now). **Identifies + times ONLY — creates nothing, buys nothing** (logs `parts_beat_tech_run`). Reports warranty-skipped so you see the cash-vs-warranty split of the prize. Dry-run: `parts-beat-tech?secret=<admin>`.
- **Live path (waits on Teddy decisions)** in **`docs/parts-beat-tech-plan-2026-08-06.md`**: reuses `parts-autoplace` (create the ship-to-customer row → autoplace places it), **two-flag graduation** (`PARTS_BEAT_TECH_LIVE`=create rows → verify → `SELF_CHECKOUT_AUTOPLACE_LIVE`=ship). **DECISIONS TO LOCK:** (1) cash approval gate — order before the customer pays/approves? (recommend office one-tap first, then auto); (2) confidence bar (recommend human-picked part only); (3) miss handling (recommend order-anyway + flag office to move the day); (4) warranty Phase 2 scope.

### 🔎 SEO / MAP-PACK READ — pulled the REAL numbers from Xano (both tools cache there)
Scoreboard artifact (private): `https://claude.ai/code/artifact/25d3f40d-0489-4c36-8922-b2dcdc27df22`. Data as of 2026-08-03.
- **Organic (GSC, `gsc_snapshot`) — WORKING, compounding:** pages Google shows **383 → 743 in 4 wks** (nearly doubled), page-1 pages 65 → 115, impressions 18.3k → 26.8k. BUT **near-zero CLICKS** despite big impressions (refrigerator repair #7.1/386 impr/1 click; appliance repair #11.4/366/1) — because the **map pack sits above organic**, so the clicks live in the pack, not in more organic pages.
- **Map pack (`rank-grid` geo-grid, `rank_grid_scan`) — mixed:** **Nashville** present-not-dominant (top-10 ~78% of grid, **top-3 only ~15%**, avg #6.4; dryer weakest at 58% found). **LA + Clarksville = found at 0 of 36 / 0 of 16** (`sab_hidden:true` — Places can't verify our address-hidden SAB AND competitors ARE showing → effectively invisible; Clarksville is Lee's base = real lost ground). Tools: `rank-grid.html` (visual) / `rank-grid?secret=&metro=tn|tn-clarksville|la&scan=1`.
- **Reviews (`review_velocity_score`) — the leak:** 4.5★ / 1,082 (strong base) BUT **velocity ZERO** — asks_sent 0, `top_leak: no_throughput/ask_throughput`. Reviews are the #1 map-pack ranking lever and the engine was OFF.

### ✅ REVIEW-ENGINE STARVATION — ROOT-CAUSED + FIXED (`review-request-sweep.js`)
Why velocity was 0: the nightly sweep (cron fine, `30 23 * * *`) only scanned **`tech_job_complete`** events = **0 in 14 days** (techs finish elsewhere — the SAME dead signal that starved the SP claims pipeline). The real end-of-job signal **`office_invoice_logged` fires ~14/day (193/14d)** and was unused. The only asks going out (8/14d) were **`via:'tech'`** (manual tech taps); the sweep sent **none**.
- **FIX:** feed candidates from **BOTH** `tech_job_complete` AND `office_invoice_logged`, deduped by job. The sweep's existing **live-status recheck** (`scheduling_status/current_status === 'completed'`) still gates every send — verified on real data that invoiced jobs split **completed vs awaiting_parts** (3/3 in a 6-sample), so an invoice on a parts-pending job is correctly skipped; only truly-done jobs get "how'd we do?". 60-day per-customer dedup + opt-out unchanged. Stamps `source: complete|invoice` on the ask. **Verify:** `review-request-sweep?dryrun=1` (lists who'd be asked + `sources{}`, sends nothing); live run auto-fires ~6:30PM CT.

### ✅ COMPLETION TRIGGER — canonical `job_completed` backbone (`job-completion-watch.js`, the root fix)
Follow-on to the review fix: instead of leaning on `office_invoice_logged` (which fires on `awaiting_parts` jobs too), built THE reliable "job is done" signal. **`tech_job_complete` is dead (0/14d), but the `office_set_job_status → to:'completed'` transition IS flowing — ~7 jobs/day / 100 distinct jobs in 14d, mostly `actor:'tech'` (techs DO record completion via status, just not via the Complete event).** `job-completion-watch` (hourly cron `23 * * * *`) watches that transition and emits **ONE canonical `job_completed` event per job** — the backbone every pipeline should consume now (reviews wired; SP claims / follow-ups / metrics next). Internal event only (no SMS/money); **forward-only** (36h window → no historical flood), **per-job dedup** (once ever), capped 60/run. Kill: `JOB_COMPLETION_WATCH=false`. `review-request-sweep` now takes `job_completed` as its PRIMARY candidate source (ahead of office_invoice_logged), so every genuinely-completed job with a phone gets the ask — sweep's live-status recheck + 60-day dedup + opt-out still gate each send. Verify: `job-completion-watch?secret=&dry=1` then `review-request-sweep?dryrun=1`.
- **Measured coverage before this (today's window):** 9 invoiced jobs → sweep would ask 3 (6 correctly held as `awaiting_parts`, 0 no-phone). The completion trigger closes the residual gap (jobs that finish but were never invoiced / invoiced before completing).

### ⚠️ STANDING FOOTGUN (cemented this session) — `tech_job_complete` is a DEAD signal (0/14d)
Techs don't press Complete in Ant, so **`tech_job_complete` fires ~0 times.** ANY automation keyed on it is silently starved (bit the SP claims pipeline AND the review engine). **The canonical "job is done" signal is now `job_completed`** (emitted by `job-completion-watch` off the `office_set_job_status→completed` transition) — consume THAT going forward, not `tech_job_complete`. Fallbacks if reading raw: `office_set_job_status` (`to:'completed'`, the transition) and `office_invoice_logged` (~14/day, but can fire on an `awaiting_parts` job — gate on live `scheduling_status === 'completed'`, don't trust the invoice event alone). Completion STATUS is set (board-side) even though the tech Complete EVENT isn't emitted.

### ⏭️ OPEN / NEXT
1. **parts-beat-tech — DECISIONS LOCKED + CASH ONE-TAP WIRED (2026-08-06):** (1) office one-tap approve, (2) HUMAN-picked `oem_part_number` only, (3) order anyway + flag office to move the day if ETA misses the visit, (4) warranty = alert + early vendor push (NOT ordered). **Built:** `parts-beat-tech-order.js` (office one-tap: cash-only + human-part + dedup gates → live Marcone drop-ship via `marcone-order` + ledger row + `parts_beat_tech_reschedule_flag` on a miss; SHADOW until vault `PARTS_BEAT_TECH_LIVE=true`) + **`parts-beat-tech.html`** (office "Beat the Tech" queue, office-pw gated, one-tap "Order to beat the tech" per candidate) + `parts-beat-tech.js` read now takes `?office=<pw>`. **⏭️ To go live:** eyeball `parts-beat-tech.html` in shadow (verify candidates/parts/addresses) → flip `PARTS_BEAT_TECH_LIVE=true` → first taps place real Marcone drop-ships. **⏭️ Warranty (#4):** build the ETA-vs-visit ALERT off `ahs-parts-watch`/`servicepower-parts-watch` feeds (buildable now, no order); the early vendor PUSH is gated on the Frontdoor/ServicePower write API (Frontdoor dev starts 8/11).
2. **Review engine:** completion trigger SHIPPED (`job-completion-watch` → `job_completed` → sweep). Eyeball `job-completion-watch?secret=&dry=1` then `review-request-sweep?dryrun=1`; watch `review_ask_sent`/velocity climb. Residual: jobs that FINISH but never get flipped to `completed` status at all (no transition event) still won't be caught — that's board status-hygiene, a smaller remaining gap.
   - **`review-backfill` — ONE-TIME 2-week catch-up SCHEDULED for tonight** (Teddy 2026-08-06: "fire it at 6:30pm as a thank-you for the opportunity + how'd we do to all eligible"). Cron `12,26,42,56 23 * * *` (6:12–6:56pm CDT), paced (SMS breaker = 50/10min; ~223 completed in window, ~100 eligible after gates), cursor-walks the whole window without re-texting, reuses the 60-day dedup marker, business-hours-gated, newest-first (6:30 sweep no-ops on today's). English ask warmed to lead with "thank you for the opportunity." Texts Teddy a per-batch summary. **⏭️ REMOVE the `review-backfill` cron block from netlify.toml once the backlog is cleared** (it self-terminates functionally — candidates go empty — but clean it up). Kill mid-run: vault `REVIEW_BACKFILL=off`.
3. **Map pack:** restarting review velocity is the top lever for Nashville top-10→top-3 + a foothold in LA/Clarksville (both invisible). GBP category/SAB verification for LA+Clarksville is the deeper fix.
4. **Phone-transfer-outcome:** first Sofia/Danielle catch numbers appear on the nightly scorecard once a day of transferred calls accrues.

## 🗓️🐜⚡ 2026-08-05 (Wed PM) — "SLOW TODAY" PERF FIX + API DAY (Frontdoor SCHEDULED, FedEx returns auto-close, ERP, SP-API parked) — READ FIRST

Egress this session: Xano + applianceant.com reachable; **403 on tnapplianceexchange.net + *.netlify.app** — couldn't invoke Netlify/Vapi/Gmail; measured via Xano + read code. All on `main`.

### ⚡ "SYSTEM IS SLOW TODAY" — root-caused + fixed (concurrency pileup on the board feed)
Measured from Xano: `get_job_for_dashboard` 0.3s + light reads 0.55s = **hot paths healthy**. But `get_office_kanban` (office board feed, ~700 jobs, 7-status OR) = **3–4s single, 8–15s at 6 concurrent** → Xano **queues under load**; single requests look fine while everything *feels* slow.
- **Fix (Netlify, live):** staggered + thinned the frequent crons that hit the raw endpoint so no two run the heavy query the same minute — `tech-prep-link` */10→7,27,47 (6→3/hr), `denorm-job-customer` */30→13,43, `servicepower-contact-backfill` 0,15,30,45→10,40, `intake-link-guarantee` 5,20,35,50→25,55 (all-distinct minutes). AND routed 5 more office pages (office-do-next/ready/today/kanban, reports-desk) through the cached **board-feed** (75s shared edge cache, raw fallback) — collapses all human office traffic to ~1 heavy query/75s.
- **Deep XS fix = DEAD END (tested live).** Staged `get_office_kanban_v2` (7-status OR → `in [...]`) + a verify harness (`tools/kanban-perf-verify.sh`). Teddy pushed it; result: `in [...]` **parses** but was **NOT faster** (4–7.7s) AND **over-matched** — 800 rows (hit cap) vs v1's 701. So the OR isn't the lever; base 3–4s is row-volume + compute contention. Discarded v2 (runbook `docs/kanban-perf-runbook-2026-08-05.md` kept as the record). **⏭️ Delete the `get_office_kanban_v2` endpoint in the Xano UI.**
- **Xano audit briefing for Cassie** (`docs/xano-audit-brief-2026-08-05.md`): Cassi Hall (Xano) offered an efficiency audit. Brief points her at `get_office_kanban` with our measurements + what we already did (denorm N+1, composite index, board-feed, cron stagger) + what failed (`in[...]`), and the questions only Xano can answer (why the OR won't use the index; is it query- or compute-bound; queue depth). **Framing: efficiency first, compute/Boost bump last** (their team upsells compute). ⏭️ Teddy forwards it / steers the call.

### 🔌 API DAY
- **🎉 FRONTDOOR / AHS Status API — SCHEDULED (the win).** Sent Brian Bullock a warm check-in (he'd owed us a timeline since 7/13); he replied same day: **dev team starts our integration next sprint — Tuesday 8/11** — a few projects wrap first, then ours; **he'll email Tuesday 8/11 to confirm they've begun.** Moved from "stuck/waiting" → "scheduled, start 8/11." Our side built + authenticating; test the moment they link our Client ID. This is the biggest labor-saver (kills Danielle's manual portal updating). **⏭️ Watch for Brian's Tuesday 8/11 note.**
- **Amazon SP-API — CLOSED/PARKED.** Amazon support (Nicole M.) closed case 21424102471 without linking the seller account, pointing at register-as-a-public-developer. Root cause: there's **no Professional Seller Central account** to authorize against (login has Amazon Pay + Login-with-Amazon, not a seller account) → it's a **strategic fork (become an Amazon seller), not an API bug.** Parked; Associates link already monetizes the angle.
- **Amazon Business Ordering API — under review, nudged correctly.** Keep-warm sent 8/02; **do NOT nudge again until ~8/11** (3-days-later reads as pestering). Our side done.
- **📦 FedEx returns auto-close — BUILT (shadow).** FedEx API creds vaulted (Teddy). New `fedex-returns-autoclose.js` (cron 35 14,20): pulls open returns w/ a FedEx tracking #, asks FedEx Track v1 if delivered to the distributor, closes the delivered ones via the same `warranty_part_status:'shipped'` path + stamps FedEx delivery date/status (`by:fedex_auto`) = **automatic proof-of-delivery** (chargeback shield). Fixes the "230 open = nobody taps Shipped ✓" backlog. SHADOW until vault **`FEDEX_AUTOCLOSE_LIVE=true`**; verify `?dry=1`. (`fedex-pickup.html` now works too — creds vaulted. Tomorrow's ~30-box pickup was scheduled MANUALLY.)
- **ERP (Exact Replacement Parts) — applied 7/30, no response.** Aftermarket value-tier distributor. Sales **(800) 377-1355** / Sales@erparts.com (closes 5 CT). Called after-hours; follow-up email drafted. **`vendor-api-watch` was BLIND to ERP** (required an API phrase + a vendor list that excluded ERP) → broadened it: added distributor-account phrases (account approved / account number / wholesale account / …) + the in-flight distributors (exact replacement/erp/corecentric/servicerparts/supco). Now their approval pings Teddy. ⏭️ Teddy calls ERP tomorrow AM + searches inbox for "Exact Replacement".
- **Parts-brain APIs to initiate (moat, the #1 goal):** MarconeAI (email Tim Wangelin — API access feeds the troubleshooting brain), Reliable Parts (sign Services Agreement → 2nd OEM source). Teddy's to kick off.

### 💳 DURABLE (NON-EXPIRING) PAYMENT LINKS — built, gated OFF (a customer's links expired)
Jennifer Roher (AHS dryer + washer, 2 × $50 add-ons = $100+tax) wanted to pay but her **Stripe *Checkout* links (`cs_live_`) expired** — they die 24h after creation, and she went to pay days later. `create-stripe-payment-link.js` now has a **durable path = Stripe *Payment Links*** (one-time restriction, don't expire). **SAFE-gated (it's money code, couldn't test from this env):** opt-in via `body.durable` OR vault **`PAYMENT_LINKS_DURABLE=true`**; **DEFAULT OFF**; ANY failure falls through to the proven Checkout Session so it can never hand out a bad link; shared metadata on the link + `payment_intent_data` so verify/webhook record identically. **⏭️ To turn on: test ONE link end-to-end ($1 → confirm it marks paid), then flip `PAYMENT_LINKS_DURABLE=true`.**
- **⚠️ I CANNOT generate/send a Stripe link OR an SMS from this env** (no Stripe key, Netlify 403) — Teddy sends. For Jennifer: resend fresh add-on links from her job's "🛒 Customer asked for these" card (=$100), copy the `checkout.stripe.com/…` URL into the text. Recommended tonight: a NORMAL fresh link (she pays tonight; 24h is fine) rather than the untested durable path.
...
### 🐞 ANT'S GUESS BUG — "oven control board on every machine" ROOT-CAUSED + FIXED (field bug, live)
Teddy reported the field app's **"Ant's Guess"** (the Tech-vs-Ant part predictor in `ant-tdr-card.js`, calls `ant-brain-predict` with `{job_id}`) was **wrong on every machine — an oven control board for everything** (a fridge water filter on a dryer, etc.). Root-caused live: **`ant-brain-predict`'s job_id path read the WRONG fields from `get_job_for_dashboard`** — it read `appliance.appliance_type` + `job.problem_summary`, but that endpoint returns the appliance as **`d.appliance.{type, model_number, brand, problem_summary}`** (NOT `appliance_type`, and the problem lives on the appliance obj, not the job). So `appliance` resolved BLANK → the hard appliance-gate was skipped → it fell through to **brand-only** matching → returned any part that brand ever used across ALL appliances (Whirlpool → oven control board; Samsung dryer → fridge filter). **Fix (`ant-brain-predict.js`): read `a.type` / `a.model_number` / `a.brand` / `a.problem_summary` first.** Verified live on real jobs: dryer→dryer repair kit (was fridge filter), washer→washer suspension rod, oven→oven-scoped/honest-thin. Same field-placement gotcha class as the documented `get_job_for_dashboard` tech={id,name} / customer.state ones. Also bumped `sw-tech.js` cache v35→**v36** to force techs onto the fresh app.

### 🧹 ANT OPS SUPABASE CLEANUP — 1.9 GB → 655 MB (runaway backup capped)
Teddy asked to clean up the DB to run efficiently. Diagnosed via the size query: **`xano_backup_chunks` = 978 MB (over half the DB), 26,103 chunk-rows** — the nightly off-site backup wrote a full snapshot every night and **NEVER pruned old ones** (`clearSnapshot` only de-duped the same day). That unbounded backup accretion was the #1 autovacuum-churn source behind the Nano crash — NOT disk-full (disk was 1.94/8 GB). **Fix (`_lib/backup.js`): added `pruneOldSnapshots()` — keeps a rolling `BACKUP_RETENTION_DAYS` (default 7) after every run.** Teddy ran a one-time `delete from xano_backup_chunks where snapshot_date < current_date - interval '7 days'` → **978 MB → 12 MB** (autovacuum reclaimed on Small). DB now ~655 MB of real data: hcp_vectors 450 MB (embeddings, used), hcp_archive 110 MB + meistertask_archive 42 MB (cold, kept), event_log 41 MB, brain_predictions 120 kB. **Autovacuum confirmed running again** (xano_backup_chunks shows a real `last_autovacuum` — first since the resize). Backup stays in Supabase (satisfies off-Xano DR) — just capped now. The archives (hcp_vectors/hcp_archive/meistertask) relocate off this project before the Phase-2 queue lands.

### 🔭 db-size-check ENDPOINT — self-serve Supabase monitoring (no more SQL screenshots)
Built `netlify/functions/db-size-check.js` (owner-gated `?secret=<admin>`) + `sb.rpc()` helper in `_lib/supabase.js` + the read-only SECURITY DEFINER function **`docs/sql/002_db_sizes.sql`** (`ant_db_sizes()`, service_role-only, run once in the SQL editor — DONE). Returns every table's size / live+dead rows / last_autovacuum + a `bloat_watch` (tables where dead>live). Verified live end-to-end. This is how I watch DB health for cleanup + the pre-Phase-2 "is this project lean enough for the queue?" check without asking Teddy for screenshots.

### 🔮 MIGRATION DOC — Phase 5 (horizon): automate Xano → Supabase
Teddy asked "possibility of automation building what we have on Xano onto Supabase + improving our idea." Captured the full answer as **Phase 5 in `docs/loop-redesign-postgres-migration.md`**: high possibility + right direction, as an automation-*assisted* + shadow-verified + INCREMENTAL rebuild (NOT a push-button transpile). Data layer = auto (extend the nightly mirror to a dual-write); logic = LLM-rewrites-then-review the ~127 LIVE endpoints (the 724 count is mostly dead); improves the idea = one brain-adjacent DB (join live jobs against history+embeddings+eval in one query) + kills the XanoScript footgun class + cheaper/reliable; the **decision-diff harness is the highest-value automation** (proves new==old on real traffic before any flip). **The loop/queue migration (Phases 0–4) is the PILOT; Phase 5 does NOT start until it proves the pattern.** Subsystem-by-subsystem, Xano decommissioned last, hot rollback throughout.

### ⏭️ TOMORROW — pick up here
1. **Confirm the eval harness is capturing** — `brain-eval-check?secret=<admin>` should show `rows_seen` climbing as the */20min sweep predicts new jobs (was 30 at sign-off). Grader runs hourly.
2. **Watch the field app** — Teddy to fully close+reopen the tech app once (v36) and confirm Ant's Guess now tracks the appliance on a few machines.
3. **Continue Week 1:** Sentry error-wrapper + `/health` (Teddy vaults the DSN), then the Phase-2 Postgres spine (`store.js` pg adapter + tee). Use `db-size-check` to confirm ANT OPS is lean before any queue load moves over (it is now — 655 MB, autovacuum healthy).
4. Dead-agent archive (VERIFIED pass), decision-diff harness, backtest script.
