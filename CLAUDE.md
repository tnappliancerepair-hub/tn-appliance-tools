...
- **⏭️ NOT yet done (next):** the **office-board.html tile thread** uses its OWN inline renderer (not ant-spine) → needs the same lane labels. Let the **customer type from their portal** (portal thread is read-only today). Build the full **unified two-thread view** (🤖 AI read-only + 👤 human read/write) on the office tile.
