...
- **🛟 Leak-detector kit Yes/No ON THE WAIVER (added 2026-06-27)** — Teddy's floor-protection upsell + coverage, placed on the waiver (his call) right under the hose Yes/No. Pitch: smart 5/6-pc sensor kit at every leak point (behind fridge, under dishwasher/washer, by toilet/water heater) — smart version texts your phone, cheaper version just alarms so you shut the water off; ~$120 installed (Teddy still researching exact kit/price — adjust freely). Cited IBHS stat. Shows on **every** waiver (required Yes/No = the documented-decline shield, same as the hose); **personalized line when the job is floor-flagged** ("you told us you're concerned about your flooring — this is the protection"), read from `customer_preference_text` `🛟 FLOORS:`. A **Yes records an add-on via `save-extra`** → flows to the tech's "Customer asked for these" card + office invoice worksheet + to-fulfill banner (ties into the add-on payment system). Saved in `acknowledgments_json` as `leak_kit_choice`/`wants_leak_kit`.
