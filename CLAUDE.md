...
- **📞 MULTILINGUAL PHONE IS LIVE on Ant Inbound** (assistant `7cc98b0c-54a7-4d19-bd48-6dfac606e55d`). Transcriber flipped Deepgram `nova-2-phonecall`/`en-US` → `nova-2`/**`language: multi`** (warranty keywords + AssemblyAI-en fallback preserved); voice already Cartesia **`sonic-2`** (multilingual, untouched); a **"answer the caller in THEIR language"** prompt block added (idempotent, wrapped in `<!-- ML-START -->…<!-- ML-END -->`). **Spanish+English = strong; VI/AR/HI/FR = best-effort (phase 2: needs nova-3 or per-lang + test).** Teddy chose to go live without a Spanish tester. **One-command control via `vapi-admin` NEW `lang` action:** read-only dump = `…/vapi-admin?secret=tn-vapi-admin-9f83b1c4e7a206d5&action=lang`; enable = `&action=lang&apply=multi`; **instant revert** = `&action=lang&apply=english` (transcriber back to English-locked + block removed, nothing else touched). **NEXT: pull the first real non-English call transcript via `&action=lastcall` to verify Deepgram heard it + Ant replied in-language; if Spanish pronunciation is rough, the fix is a voice tweak (drop `voice.language:en` or lean on the 11labs `eleven_multilingual_v2` fallback).**
