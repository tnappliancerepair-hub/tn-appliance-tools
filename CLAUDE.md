...
- **`tech-job.html` — ONE "💳 Get paid" card**, up top (out of the "More" fold), amount auto-filled, four ways in order: **📱 Show QR** (customer scans with THEIR phone → Apple Pay), **📲 Text the durable link** (`send-pay-link`), **💵 Cash/check** (`record_payment_received`), **💳 Charge on my phone** (durable page, hand over). Paid → "✅ Paid"; covered warranty w/ no extras → "🛡️ Covered — nothing to collect". Removed the redundant "Take a payment" card + its expiring-link JS. **sw-tech v46→v47** (crew reopen once).
