...
### 📞 PHONE→BOARD BRIDGE (makes phones + database ONE product) — BUILT, dormant until vaulted
...
- **⏭️ TO ACTIVATE (Teddy, ~2 min):** vault **`SUPABASE_URL`** = `https://tntbhfwitytkcoqlejwc.supabase.co` + **`SUPABASE_SERVICE_KEY`** = the Supabase **Secret key** (`sb_secret_…` from Settings → API Keys → Secret keys) via admin-secrets.html. Then test: `…/platform-lead?slug=demo&name=Test%20Caller&phone=6155551234&what=Whirlpool%20dryer&detail=Not%20heating` → a new job appears in the demo office board's "New" column. (Service key is powerful → vault only, never chat/browser.)
