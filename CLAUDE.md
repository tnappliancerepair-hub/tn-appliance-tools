...
- **⚠️ STRATEGIC FLIP (Teddy):** done right this turns the #1 objection into the #1 sales line — *"bring your data, we move you off HCP in an afternoon, free, and keep the old one running until you're sure."* The thin phone-only competitors can't say that.
- **⏭️ NEXT (offered):** Jobber (GraphQL/OAuth) + Workiz (REST token) adapters (same land.js spine) · a generic **CSV** mapper (universal fallback when API access is a pain) · an **attachment/photo pass** (fetch from HCP → re-host to our storage) · wire the `import.html` link into ops.html/system.html · teardown the `hcp-import-demo` tenant after showing Teddy.
