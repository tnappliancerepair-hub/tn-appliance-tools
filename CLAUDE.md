...
- **Footguns learned:** (a) must log in IN the daemon's window (not normal Chrome/Safari — separate browser); (b) old daemons held port 8787 → now auto-port avoids it; (c) windows open behind others → Mission Control / "Google Chrome for Testing".
- **NEXT (the wire-up):** daemon runs on the Mac (localhost) — cloud can't reach it. So: a colony-loop agent calls `localhost:PORT/lookup`, writes parts (OEM cost + stock from Marcone, aftermarket from Amazon) to Xano, and the cash-TDR's 4 options + tech tool read them = auto-filled exact parts + pricing. Also: launchd the daemon so it auto-runs; add a keep-alive + re-login alert (the live session expires if the browser closes/times out). Amazon search works without login (public).
