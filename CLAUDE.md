...
Techs don't press Complete in Ant, so **`tech_job_complete` fires ~0 times.** ANY automation keyed on it is silently starved (bit both the SP claims pipeline AND the review engine). **The real "job reached its end" signal = `office_invoice_logged`** (~14/day). But note it's not "done" by itself — an invoice can be logged on an `awaiting_parts` job (first-trip labor billed) — so gate on the job's LIVE `scheduling_status === 'completed'`, don't trust the invoice event alone. Completion STATUS is set (board-side) even though the EVENT isn't emitted.
...
2. **Review engine:** eyeball tonight's `review-request-sweep?dryrun=1`; watch `review_ask_sent`/velocity climb. **Status-hygiene gap** (finished jobs that never reach `completed` status) is the next thread — same root as the dead `tech_job_complete`.
