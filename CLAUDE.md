...
| **ServicePower API** 🥈 | Auto status/claim submit for SquareTrade/NSA-dispatched jobs | **🟢 6/24 BIG PROGRESS — NO approval gate (unlike Frontdoor).** It's a **SOAP/SPDService** API (`urn:SPDServicerService`); docs + WSDL are right in the HUB's "Servicer API Integrations" article (servicer acct **TNA00001**). Connector BUILT (`_lib/servicepower.js`: `getTestService` + `updateCallInfo`, UserInfo auth) + `servicepower-test.js`. Creds VAULTED (`SERVICEPOWER_USER_ID`=tnappliancerepair@gmail.com, `_PASSWORD`, `_SVCR_ACCT`=TNA00001, `_ENV`=development). **CONNECTIVITY CONFIRMED** (reached fssstag, SOAP response). WSDL key op = `updateCallInfo` (CallNumber + CallStatus/SPCallStatusID + Remarks/Notes + ETA/CompletedDate). **REMAINING: the `SPCallStatusID` status-code values** — from the **Dispatch Web Service Interface v2.8** PDF (in the HUB) OR read live via `getCallInfo` (also validates creds). Then map our lifecycle → codes + wire `updateCallInfo`. Prod URL `fss.servicepower.com/sms/services/SPDService`. Spec: `docs/servicepower-api-spec-2026-06-24.md`. |
