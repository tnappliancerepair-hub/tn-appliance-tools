...
- **OPERATOR PUSH NEEDED (Mac, XS deploy):** `git fetch origin main && git checkout origin/main -- api/intake/check_service_zone_GET.xs api/intake/get_tech_route_days_GET.xs && /opt/homebrew/bin/xano workspace push -i "api/**/{check_service_zone,get_tech_route_days}*" --force`. Until pushed, schedule cards just don't show the suggestion (dropdown still works manually). Colony agents hot-reload on edit, but the new `xano.findExtraWorkForTech` helper needs a loop kickstart: `launchctl kickstart -k gui/$UID/com.tnappliance.colony-loop`.
