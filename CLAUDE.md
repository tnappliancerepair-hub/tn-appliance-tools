...
- **⏭️ NEXT (the LAST phase):** 05 measure — per-tenant trust scorecard + accuracy audit + LLM call grader (the daily "gets better" flywheel per shop, mirroring TN's `phone-trust-scorecard`/`phone-accuracy-audit`/`ann-call-grader`).
