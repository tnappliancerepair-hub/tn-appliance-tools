...
- **⏭️ NEXT (the phases, from the artifact):** 03 call actions (book/confirm day, capture callback→board, text link, transfer hours-gated + warranty-rep routing — all reuse existing platform endpoints) · 04 reliability + grounding (fillers, warm cron for these hot-path fns since cold-start loses the 1.8s race, per-tenant grounding rules) · 05 measure (per-tenant trust scorecard + accuracy audit + LLM grader).
