...
- **⏭️ NEXT (the phases, from the artifact):** 02 pre-call recognition (resolve during pickup → dynamic vars, warm-degrade — needs a dialed-number→slug map since the tool-URL slug trick doesn't cover the pre-call webhook) · 03 call actions (book/confirm day, capture callback→board, text link, transfer hours-gated + warranty-rep routing — all reuse existing platform endpoints) · 04 reliability + grounding (bounded timeouts + keep-talking fallback, "one moment" fillers, warm cron) · 05 measure (per-tenant trust scorecard + accuracy audit + LLM grader).
