...
### ⏭️ OPEN / NEXT
1. **parts-beat-tech:** Teddy runs `?secret=` dry-runs a few days → lock the 4 decisions → wire the live create path (office one-tap first) → flip `PARTS_BEAT_TECH_LIVE` → verify To-Order rows → flip `SELF_CHECKOUT_AUTOPLACE_LIVE`.
