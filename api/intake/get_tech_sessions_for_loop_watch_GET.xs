...
query get_tech_sessions_for_loop_watch verb=GET {
  ...
  stack {
    ...
    var $cutoff_ms {
      value = $input.now_ms - (24 * 3600 * 1000)
    }
      ...
      each as $s {
        ...
        foreach ($msg_rows.items) {
          each as $mr {
            ...
            var $mr_strip {
              value = $mr_meta_raw|replace:$job_marker:""
            }
          
            conditional {
              if (($mr_meta_raw|strlen) > ($mr_strip|strlen)) {
                var.update $job_msg_count {
                  value = $job_msg_count + 1
                }
              }
            }
          }
