...
query get_hcp_migration_status verb=GET {
  ...
  stack {
    ...
    var $total_in_window  { value = 0 }
    var $with_hcp_id      { value = 0 }
    var $without_hcp_id   { value = 0 }
    var $open_jobs        { value = 0 }
    var $completed_jobs   { value = 0 }
    var $canceled_jobs    { value = 0 }
    var $scheduled_jobs   { value = 0 }
    var $not_ready_jobs   { value = 0 }
    var $prediag_pending  { value = 0 }
    var $other_status     { value = 0 }
    var $source_hcp       { value = 0 }
    var $source_ahs       { value = 0 }
    var $source_servicepower { value = 0 }
    var $source_warranty  { value = 0 }
    var $source_office    { value = 0 }
    var $source_web_chat  { value = 0 }
    var $source_other     { value = 0 }

    foreach ($items) {
      each as $j {
        var.update $total_in_window { value = ($total_in_window + 1) }

        var $hcp { value = (($j.housecall_pro_job_id ?? "")|trim) }
        var $status { value = (($j.scheduling_status ?? "")|trim) }
        var $src { value = (($j.source_type ?? $j.intake_source ?? "")|trim) }

        conditional {
          if ($hcp != "") {
            var.update $with_hcp_id { value = ($with_hcp_id + 1) }
          }
          else {
            var.update $without_hcp_id { value = ($without_hcp_id + 1) }
          }
        }

        conditional {
          if ($status == "completed") {
            var.update $completed_jobs { value = ($completed_jobs + 1) }
          }
          elseif ($status == "canceled" || $status == "cancelled") {
            var.update $canceled_jobs { value = ($canceled_jobs + 1) }
          }
          elseif ($status == "scheduled" || $status == "in_progress" || $status == "booked") {
            var.update $scheduled_jobs { value = ($scheduled_jobs + 1) }
            var.update $open_jobs { value = ($open_jobs + 1) }
          }
          elseif ($status == "not_ready") {
            var.update $not_ready_jobs { value = ($not_ready_jobs + 1) }
            var.update $open_jobs { value = ($open_jobs + 1) }
          }
          elseif ($status == "prediagnosis_pending" || $status == "needs_pre_diagnosis") {
            var.update $prediag_pending { value = ($prediag_pending + 1) }
            var.update $open_jobs { value = ($open_jobs + 1) }
          }
          else {
            var.update $other_status { value = ($other_status + 1) }
            var.update $open_jobs { value = ($open_jobs + 1) }
          }
        }
        ...
        conditional {
          if ($src == "hcp" || $src == "hcp_webhook" || $src == "hcp_poll") {
            var.update $source_hcp { value = ($source_hcp + 1) }
          }
          elseif ($src == "ahs_email") {
            var.update $source_ahs { value = ($source_ahs + 1) }
          }
          elseif ($src == "servicepower" || $src == "servicepower_email") {
            var.update $source_servicepower { value = ($source_servicepower + 1) }
          }
          elseif ($src == "warranty_jotform" || $src == "warranty") {
            var.update $source_warranty { value = ($source_warranty + 1) }
          }
          elseif ($src == "office_ant") {
            var.update $source_office { value = ($source_office + 1) }
          }
          elseif ($src == "web_chat") {
            var.update $source_web_chat { value = ($source_web_chat + 1) }
          }
          else {
            var.update $source_other { value = ($source_other + 1) }
          }
        }
      }
    }
