      ...
      if ($mode != "pre_diagnosis") {
        ...
        var $summary_text {
          value = """
            ...
            Tech: %s %s
            """
            |sprintf:($job.id|to_text):$customer_name:$job.brand ?? "Unknown":$job.appliance_type ?? "Unknown":$job.model_number ?? "Not provided":$input.diagnosis ?? "Not provided":$input.repair_completed ?? "N/A":($input.labor_time_hours|to_text):($input.second_visit_needed ? "Yes" : "No"):$parts_used_list:$parts_not_used_list:$office_lookup_list:$input.final_recommendation ?? "N/A":$input.technician_first_name ?? "Tech":$input.technician_last_name ?? ""
        }
