      ...
      each as $j {
        ...
        conditional {
          if (!$has_customer_response && !$intake_sms_assumed_sent) {
            var.update $stage {
              value = "needs_scheduled"
            }
          }
        
          elseif (!$has_customer_response) {
            var.update $stage {
              value = "awaiting_customer_info"
            }
          }
        
          elseif (!$has_prediag) {
            var.update $stage {
              value = "awaiting_prediagnosis"
            }
          }
        
          elseif ($parts_needed && ($parts_eta == null)) {
            var.update $stage {
              value = "awaiting_parts_eta"
            }
          }
        ...
        conditional {
          if ($stage == "needs_scheduled") {
            var.update $stage_1 {
              value = $stage_1|push:$summary
            }
          }
        
          elseif ($stage == "awaiting_customer_info") {
            var.update $stage_2 {
              value = $stage_2|push:$summary
            }
          }
        
          elseif ($stage == "awaiting_prediagnosis") {
            var.update $stage_3 {
              value = $stage_3|push:$summary
            }
          }
        
          elseif ($stage == "awaiting_parts_eta") {
            var.update $stage_4 {
              value = $stage_4|push:$summary
            }
          }
        
          else {
            var.update $stage_5 {
              value = $stage_5|push:$summary
            }
          }
        }
      }
