  ...
  stack {
    ...
    conditional {
      if ($existing_job != null && $existing_job.customer_id != null && $existing_job.customer_id > 0) {
        var.update $cust_id_final { value = $existing_job.customer_id }
      }
    }
    ...
    conditional {
      if ($cust_id_final == 0 && $phone_clean != "") {
        ...
        conditional {
          if ($cust_match != null) {
            var.update $cust_id_final { value = $cust_match.id }
            var.update $customer { value = $cust_match }
          }
        }
      }
    }
      ...
      if ($cust_id_final == 0) {
        conditional {
          if ($first_clean != "" || $phone_clean != "") {
            ...
            var.update $customer { value = $new_cust }
    ...
    var $sched_status_mapped { value = "not_ready" }
    ...
    conditional {
      if ($ws_lower == "scheduled" || $ws_lower == "in progress" || $ws_lower == "in_progress" || $ws_lower == "schedule appointment" || $ws_lower == "schedule_appointment") {
        var.update $sched_status_mapped { value = "scheduled" }
      }
      elseif ($ws_lower == "completed" || $ws_lower == "complete" || $ws_lower == "complete unrated" || $ws_lower == "complete_unrated" || $ws_lower == "complete rated" || $ws_lower == "complete_rated") {
        var.update $sched_status_mapped { value = "completed" }
      }
      elseif ($ws_lower == "canceled" || $ws_lower == "cancelled" || $ws_lower == "pro canceled" || $ws_lower == "pro_canceled" || $ws_lower == "pro cancelled" || $ws_lower == "pro_cancelled") {
        var.update $sched_status_mapped { value = "canceled" }
      }
    }
    ...
    var $action_taken { value = "skipped" }
    var $resulting_job_id { value = 0 }
    ...
    conditional {
      if ($existing_job == null) {
        ...
        var.update $resulting_job_id { value = $new_job.id }
        ...
        var.update $resulting_job_id { value = $existing_job.id }
