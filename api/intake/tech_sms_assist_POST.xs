...
query tech_sms_assist verb=POST {
  ...
  stack {
    ...
    var $media_urls_in {
      value = ($input.media_urls ?? [])
    }
  
    var $media_count {
      value = $media_urls_in|count
    }
      ...
      each as $pm {
        conditional {
          if (!$is_parallel) {
            ...
            var $pm_meta {
              value = ($pm.metadata ?? "")|json_encode
            }
          
            var $pm_strip {
              value = $pm_meta|replace:$job_marker:""
            }
          
            conditional {
              if (($pm_meta|strlen) > ($pm_strip|strlen)) {
                var.update $is_parallel {
                  value = true
                }
              }
            }
    ...
    var $appliance {
      value = (($job.appliance_type ?? "appliance")|trim)
    }
    ...
    var $latest_pause_state {
      value = ""
    }
  
    var $tech_marker {
      value = "\"tech_id\":" ~ ($tech_id|to_text)
    }
      ...
      each as $pe {
        conditional {
          if ($latest_pause_state == "") {
            ...
            var $pe_meta_raw {
              value = ($pe.metadata ?? "")|json_encode
            }
          
            var $pe_strip {
              value = $pe_meta_raw|replace:$tech_marker:""
            }
          
            conditional {
              if (($pe_meta_raw|strlen) > ($pe_strip|strlen)) {
                var.update $latest_pause_state {
                  value = ($pe.action ?? "")
                }
              }
            }
    ...
    var $body_u {
      value = $body_in|to_upper|trim
    }
  
    var $is_save_cmd {
      value = ($body_u == "SAVE" || $body_u == "DONE" || $body_u == "SUBMIT" || $body_u == "FINALIZE" || $body_u == "COMPLETE" || $body_u == "THATS IT" || $body_u == "THAT'S IT")
    }
    ...
    conditional {
      if ($is_save_cmd) {
        ...
        var $sess {
          value = $sess_rows.items|get:0
        }
      
        // captured_data is a json column on tech_assist_session — should
        // come back as an object directly (no json_decode needed)
        var $captured {
          value = ($sess.captured_data ?? {})
        }
      
        var $diag {
          value = (($captured.diagnosis ?? "")|trim)
        }
      
        var $failed {
          value = (($captured.failed_component ?? "")|trim)
        }
      
        var $labor_raw {
          value = ($captured.labor_hours ?? "")
        }
      
        var $labor_str {
          value = $labor_raw|to_text
        }
      
        var $repair {
          value = (($captured.repair_completed ?? "")|trim)
        }
      
        var $part {
          value = (($captured.verified_part_number ?? "")|trim)
        }
        ...
        conditional {
          if ($failed == "") {
            var.update $missing {
              value = ($missing ~ " failed_component")
            }
          }
        }
      
        conditional {
          if ($labor_str == "" || $labor_str == "0") {
            var.update $missing {
              value = ($missing ~ " labor_hours")
            }
          }
        }
        ...
        conditional {
          if ($repair == "") {
            var.update $missing {
              value = ($missing ~ " repair_completed")
            }
          }
        }
        ...
        var $tdr_status {
          value = ($tdr_resp.response.status ?? 0)
        }
      
        var $tdr_ok {
          value = ($tdr_status >= 200 && $tdr_status < 300)
        }
    ...
    var $existing_sess_count {
      value = $existing_sess_rows.items|count
    }
    ...
    conditional {
      if ($existing_sess_count > 0) {
        var $sess_row {
          value = $existing_sess_rows.items|get:0
        }
      
        var.update $existing_captured {
          value = ($sess_row.captured_data ?? {})
        }
      
        var.update $existing_sess_id {
          value = $sess_row.id
        }
      }
    }
    ...
    var $nc_diag {
      value = (($new_captured.diagnosis ?? "")|trim)
    }
    ...
    var $nc_comp {
      value = (($new_captured.failed_component ?? "")|trim)
    }
    ...
    var $nc_repair {
      value = (($new_captured.repair_completed ?? "")|trim)
    }
  
    conditional {
      if ($nc_repair != "") {
        var.update $merged_captured {
          value = $merged_captured
            |set:"repair_completed":$nc_repair
        }
      }
    }
  
    var $nc_part {
      value = (($new_captured.verified_part_number ?? "")|trim)
    }
    ...
    conditional {
      if ($nc_part != "") {
        var.update $merged_captured {
          value = $merged_captured
            |set:"verified_part_number":$nc_part
        }
      }
    }
  
    var $nc_replaced_by {
      value = (($new_captured.replaced_by_part_number ?? "")|trim)
    }
    ...
    conditional {
      if ($nc_replaced_by != "") {
        var.update $merged_captured {
          value = $merged_captured
            |set:"replaced_by_part_number":$nc_replaced_by
        }
      }
    }
  
    var $nc_parts_status {
      value = (($new_captured.parts_status ?? "")|trim)
    }
    ...
    conditional {
      if ($nc_parts_status != "") {
        var.update $merged_captured {
          value = $merged_captured
            |set:"parts_status":$nc_parts_status
        }
      }
    }
  
    var $nc_recommend {
      value = (($new_captured.recommendation ?? "")|trim)
    }
    ...
    conditional {
      if ($nc_recommend != "") {
        var.update $merged_captured {
          value = $merged_captured
            |set:"recommendation":$nc_recommend
        }
      }
    }
    ...
    var $m_diag {
      value = (($merged_captured.diagnosis ?? "")|trim)
    }
  
    var $m_comp {
      value = (($merged_captured.failed_component ?? "")|trim)
    }
  
    var $m_labor {
      value = (($merged_captured.labor_hours ?? "")|to_text|trim)
    }
  
    var $m_repair {
      value = (($merged_captured.repair_completed ?? "")|trim)
    }
  
    var $m_part {
      value = (($merged_captured.verified_part_number ?? "")|trim)
    }
  
    var $m_replaced {
      value = (($merged_captured.replaced_by_part_number ?? "")|trim)
    }
      ...
      if ($all_4_present) {
          ...
          each as $sv {
            conditional {
              if (!$already_saved) {
                ...
                var $sv_strip {
                  value = $sv_meta_raw|replace:$sv_marker:""
                }
              
                conditional {
                  if (($sv_meta_raw|strlen) > ($sv_strip|strlen)) {
                    var.update $already_saved {
                      value = true
                    }
                  }
                }
        ...
        conditional {
          if (!$already_saved) {
            ...
            var $auto_tdr_status {
              value = ($auto_tdr_resp.response.status ?? 0)
            }
