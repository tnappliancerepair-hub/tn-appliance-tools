...
query get_tech_weekly_recap_sent verb=GET {
  ...
  stack {
    var $cutoff_ms {
      value = $input.week_start_ms - 3600000
    }
    ...
    var $tech_marker {
      value = ("\"tech_id\":" ~ ($input.tech_id|to_text))
    }
  
    var $week_marker {
      value = ("\"week_start_ms\":" ~ ($input.week_start_ms|to_text))
    }
      ...
      each as $row {
        conditional {
          if (!$found) {
            ...
            var $strip_both {
              value = $strip_tech|replace:$week_marker:""
            }
          
            var $stripped_diff {
              value = ($meta_raw|strlen) - ($strip_both|strlen)
            }
          
            var $expected_diff {
              value = ($tech_marker|strlen) + ($week_marker|strlen)
            }
            ...
            conditional {
              if ($stripped_diff >= $expected_diff) {
                ...
                var.update $last_sent_at {
                  value = ($row.created_at ?? 0)
                }
              }
            }
          }
