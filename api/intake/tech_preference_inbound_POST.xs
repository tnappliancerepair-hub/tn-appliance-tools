...
query tech_preference_inbound verb=POST {
  ...
  stack {
    ...
    // numbers → job_ids.
    var $is_extra_cmd {
      value = ($body_upper == "EXTRA" || $body_upper == "OPEN" || $body_upper == "WHO NEEDS HELP" || $body_upper == "MORE WORK")
    }
    ...
    conditional {
      if ($is_extra_cmd) {
        ...
        var $extra_body {
          value = ($extra_resp.response.result ?? {})
        }
      
        var $local_arr {
          value = ($extra_body.prediag_and_local ?? [])
        }
      
        var $other_arr {
          value = ($extra_body.other_candidates ?? [])
        }
      
        var $total {
          value = ($extra_body.total_in_pool ?? 0)
        }
        ...
        conditional {
          if ($total == 0) {
            return {
              value = {
                matched: true
                reply  : "No extra work in your area right now. I'll keep watching and text you if something opens up."
              }
            }
          }
        }
        ...
        foreach ($combined) {
          each as $c {
            ...
            var $prediag_mark {
              value = ($c.has_prediag == true) ? " ✓pre-diag" : " (no pre-diag yet)"
            }
            ...
            var.update $reply_lines {
              value = ($reply_lines == "" ? $line : ($reply_lines ~ "\n" ~ $line))
            }
          
            var.update $job_ids_csv {
              value = ($job_ids_csv == "" ? ($c.id|to_text) : ($job_ids_csv ~ "," ~ ($c.id|to_text)))
            }
          }
        }
    ...
    var $is_blast_cmd {
      value = ($body_upper|substr:0:6) == "BLAST "
    }
    ...
    conditional {
      if ($is_blast_cmd) {
          ...
          each as $or {
            conditional {
              if ($offered_for_tech == null) {
                ...
                var $or_meta {
                  value = $or_meta_raw|json_decode
                }
              
                conditional {
                  if (($or_meta.tech_id ? 0) == $tech_id) {
                    var.update $offered_for_tech {
                      value = $or_meta
                    }
                  }
                }
        ...
        conditional {
          if ($offered_for_tech == null) {
            return {
              value = {
                matched: true
                reply  : "No active candidate list. Text EXTRA to see open jobs near you."
              }
            }
          }
        }
        ...
        var $job_ids_csv {
          value = ($offered_for_tech.job_ids_csv ?? "")
        }
      
        var $job_ids_arr {
          value = $job_ids_csv|split:","
        }
      
        var $job_count {
          value = $job_ids_arr|count
        }
      
        // Determine which indices to blast
        var $send_all {
          value = ($blast_arg == "ALL" || $blast_arg == "EVERYONE")
        }
      
        var $idx_tokens_raw {
          value = $blast_arg|split:" "
        }
              ...
              each as $jid_str {
                ...
                conditional {
                  if ($jid > 0) {
                    ...
                    var $offer_status_a {
                      value = ($offer_resp_a.response.status ?? 0)
                    }
                  
                    conditional {
                      if ($offer_status_a >= 200 && $offer_status_a < 300) {
                        var.update $blast_count {
                          value = $blast_count + 1
                        }
                      }
              ...
              each as $tok {
                ...
                conditional {
                  if ($tok_int >= 1 && $tok_int <= $job_count) {
                    ...
                    var $jid_b {
                      value = `($job_ids_arr|get:$target_idx)|to_int`
                    }
                    ...
                    conditional {
                      if ($jid_b > 0) {
                        ...
                        var $offer_status_b {
                          value = ($offer_resp_b.response.status ?? 0)
                        }
        ...
        conditional {
          if ($errors_count > 0) {
            var.update $blast_reply {
              value = ($blast_reply ~ " (" ~ ($errors_count|to_text) ~ " failed to send)")
            }
          }
        }
    ...
    conditional {
      if ($tech_id == 1) {
        var $is_pause_cmd {
          value = ($body_upper|substr:0:21) == "PAUSE TECH ASSIST FOR"
        }
      
        var $is_resume_cmd {
          value = ($body_upper|substr:0:22) == "RESUME TECH ASSIST FOR"
        }
      
        conditional {
          if ($is_pause_cmd) {
            var $pause_arg {
              value = ($body_upper|substr:21)|trim
            }
          
            var $pause_tech_id {
              value = $pause_arg|to_int
            }
            ...
            conditional {
              if ($pause_tech_id > 0) {
                ...
                return {
                  value = {
                    matched: true
                    reply  : ("Tech assist paused for tech_id " ~ ($pause_tech_id|to_text) ~ ". Their messages route to legacy handler until you RESUME TECH ASSIST FOR " ~ ($pause_tech_id|to_text))
                  }
                }
              }
            }
            ...
            return {
              value = {
                matched: true
                reply  : "Usage: PAUSE TECH ASSIST FOR <tech_id>"
              }
            }
          }
          ...
          elseif ($is_resume_cmd) {
            var $resume_arg {
              value = ($body_upper|substr:22)|trim
            }
          
            var $resume_tech_id {
              value = $resume_arg|to_int
            }
            ...
            conditional {
              if ($resume_tech_id > 0) {
                ...
                return {
                  value = {
                    matched: true
                    reply  : ("Tech assist resumed for tech_id " ~ ($resume_tech_id|to_text) ~ ".")
                  }
                }
              }
            }
            ...
            return {
              value = {
                matched: true
                reply  : "Usage: RESUME TECH ASSIST FOR <tech_id>"
              }
            }
          }
        }
      ...
      if ($body_upper == "OOPS" || $body_upper == "UNDO") {
          ...
          each as $log_row {
            conditional {
              if ($found_for_tech == null) {
                ...
                var $meta_obj {
                  value = $log_meta_raw|json_decode
                }
              
                conditional {
                  if (($meta_obj.tech_id ? 0) == $tech_id) {
                    var.update $found_for_tech {
                      value = $meta_obj
                    }
                  }
                }
        ...
        conditional {
          if ($found_for_tech == null) {
            return {
              value = {
                matched: true
                reply  : "Nothing to undo. Your last change was more than 10 min ago, or you haven't made one yet."
              }
            }
          }
        }
        ...
        conditional {
          if ($change_type == "hours") {
            ...
            var.update $undo_reply {
              value = ("Undone — hours back to " ~ ($found_for_tech.prior_start ?? "08:00") ~ "-" ~ ($found_for_tech.prior_end ?? "16:00"))
            }
          }
          ...
          elseif ($change_type == "preference_row_added") {
            ...
            var.update $undo_reply {
              value = ("Undone — removed " ~ ($found_for_tech.summary ?? "last preference"))
            }
          }
        }
        ...
        db.add event_log {
          data = {
            ...
            metadata: {tech_id: $tech_id, reverted: $found_for_tech}
          }
        } as $undo_audit
    ...
    conditional {
      if ($is_hours_cmd) {
        ...
        conditional {
          if ($dash_pos < 1) {
            return {
              value = {
                matched: true
                reply  : "Try HOURS 9-6 or HOURS 09:00-18:00"
              }
            }
          }
        }
        ...
        var $start_h_int {
          value = `($start_has_colon >= 0) ? ($start_raw|substr:0:$start_has_colon)|to_int : ($start_raw|to_int)`
        }
      
        var $end_h_int {
          value = `($end_has_colon >= 0) ? ($end_raw|substr:0:$end_has_colon)|to_int : ($end_raw|to_int)`
        }
        ...
        var $start_min_str {
          value = ($start_has_colon >= 0) ? ($start_raw|substr:($start_has_colon + 1)) : "00"
        }
      
        var $end_min_str {
          value = ($end_has_colon >= 0) ? ($end_raw|substr:($end_has_colon + 1)) : "00"
        }
      
        var $start_str_h {
          value = ($start_h_int < 10) ? ("0" ~ ($start_h_int|to_text)) : ($start_h_int|to_text)
        }
      
        var $end_str_h {
          value = ($end_h_promoted < 10) ? ("0" ~ ($end_h_promoted|to_text)) : ($end_h_promoted|to_text)
        }
      
        var $start_final {
          value = ($start_str_h ~ ":" ~ $start_min_str)
        }
      
        var $end_final {
          value = ($end_str_h ~ ":" ~ $end_min_str)
        }
        ...
        return {
          value = {
            matched: true
            reply  : ("Got it — hours set to " ~ $start_final ~ "-" ~ $end_final ~ ". Text OOPS within 10 min to undo.")
          }
        }
      }
    }
    ...
    // ─── SHORTCUT: OFF <day> ─────────────────────────────────────────
    var $is_off_cmd {
      value = ($body_upper|substr:0:4) == "OFF "
    }
  
    conditional {
      if ($is_off_cmd) {
        var $off_arg {
          value = ($body_upper|substr:4)|trim
        }
        ...
        conditional {
          if ($off_arg == "MON" || $off_arg == "MONDAY") {
            var.update $day_canon {
              value = "monday"
            }
          }
        
          elseif ($off_arg == "TUE" || $off_arg == "TUES" || $off_arg == "TUESDAY") {
            var.update $day_canon {
              value = "tuesday"
            }
          }
        
          elseif ($off_arg == "WED" || $off_arg == "WEDS" || $off_arg == "WEDNESDAY") {
            var.update $day_canon {
              value = "wednesday"
            }
          }
        
          elseif ($off_arg == "THU" || $off_arg == "THUR" || $off_arg == "THURS" || $off_arg == "THURSDAY") {
            var.update $day_canon {
              value = "thursday"
            }
          }
        
          elseif ($off_arg == "FRI" || $off_arg == "FRIDAY") {
            var.update $day_canon {
              value = "friday"
            }
          }
        
          elseif ($off_arg == "SAT" || $off_arg == "SATURDAY") {
            var.update $day_canon {
              value = "saturday"
            }
          }
        
          elseif ($off_arg == "SUN" || $off_arg == "SUNDAY") {
            var.update $day_canon {
              value = "sunday"
            }
          }
        ...
        conditional {
          if ($day_canon != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — " ~ $day_canon ~ "s off going forward. Text OOPS within 10 min to undo, or STATUS to see your schedule.")
              }
            }
          }
        }
        ...
        return {
          value = {
            matched: true
            reply  : "I think you meant OFF <day>, like OFF SAT or OFF FRIDAY. Try again?"
          }
        }
      }
    }
    ...
    // ─── SHORTCUT: MAX <N> ───────────────────────────────────────────
    var $is_max_cmd {
      value = ($body_upper|substr:0:4) == "MAX "
    }
    ...
    conditional {
      if ($is_max_cmd) {
        ...
        conditional {
          if ($max_n < 1 || $max_n > 12) {
            return {
              value = {
                matched: true
                reply  : "MAX needs a number 1-12. Try MAX 5"
              }
            }
          }
        }
        ...
        return {
          value = {
            matched: true
            reply  : ("Got it — max " ~ ($max_n|to_text) ~ " jobs per day. Text OOPS within 10 min to undo.")
          }
        }
      }
    }
    ...
    var $is_no_cmd {
      value = ($body_upper|substr:0:3) == "NO "
    }
  
    conditional {
      if ($is_no_cmd) {
        var $no_arg {
          value = ($body_upper|substr:3)|trim|lower
        }
        ...
        conditional {
          if ($no_arg != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — won't send you " ~ $no_arg ~ " jobs. Text OOPS within 10 min to undo, or YES " ~ ($no_arg|upper) ~ " to reverse later.")
              }
            }
          }
        }
    ...
    // ─── SHORTCUT: SKIP <zone> ──────────────────────────────────────
    var $is_skip_cmd {
      value = ($body_upper|substr:0:5) == "SKIP "
    }
  
    conditional {
      if ($is_skip_cmd) {
        var $skip_arg {
          value = ($body_upper|substr:5)|trim|lower
        }
        ...
        conditional {
          if ($skip_arg != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — won't send you " ~ $skip_arg ~ " jobs. Text OOPS within 10 min to undo.")
              }
            }
          }
        }
    ...
    conditional {
      if ($classified == null) {
        return {
          value = {
            matched: false
            reason : "claude_unparseable_json"
            raw    : $raw_text|substr:0:200
          }
        }
      }
    }
    ...
    conditional {
      if ($intent == "not_a_preference" || $conf < 0.6) {
        return {
          value = {
            matched   : false
            reason    : "claude_low_confidence_or_not_pref"
            intent    : $intent
            confidence: $conf
          }
        }
    ...
    conditional {
        ...
        conditional {
          if ($dow_param != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — " ~ $dow_param ~ "s off going forward." ~ $clarif_suffix ~ " Text OOPS within 10 min to undo.")
              }
            }
          }
        }
      ...
      elseif ($intent == "working_hours") {
        var $start_p {
          value = (($params.start_time ?? "")|trim)
        }
      
        var $end_p {
          value = (($params.end_time ?? "")|trim)
        }
        ...
        conditional {
          if ($start_p != "" && $end_p != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — hours set to " ~ $start_p ~ "-" ~ $end_p ~ "." ~ $clarif_suffix ~ " Text OOPS within 10 min to undo.")
              }
            }
          }
        }
      ...
      elseif ($intent == "max_jobs") {
        var $mn {
          value = ($params.max_jobs ?? 0)|to_int
        }
        ...
        conditional {
          if ($mn >= 1 && $mn <= 12) {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — max " ~ ($mn|to_text) ~ " jobs per day." ~ $clarif_suffix ~ " Text OOPS within 10 min to undo.")
              }
            }
          }
        }
      ...
      elseif ($intent == "appliance_decline") {
        var $app_p {
          value = (($params.appliance ?? "")|trim|lower)
        }
        ...
        conditional {
          if ($app_p != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — won't send you " ~ $app_p ~ " jobs." ~ $clarif_suffix ~ " Text OOPS within 10 min to undo.")
              }
            }
          }
        }
        ...
        conditional {
          if ($zn_p != "") {
            ...
            return {
              value = {
                matched: true
                reply  : ("Got it — won't send you " ~ $zn_p ~ " jobs." ~ $clarif_suffix ~ " Text OOPS within 10 min to undo.")
              }
            }
          }
        }
