  ...
  stack {
    ...
    foreach ($job_rows.items) {
      each as $j {
        ...
        var $svc_state_raw {
          value = (($j.service_state ?? "")|trim|upper)
        }
        ...
        conditional {
          if ($svc_addr != "" || $svc_city != "" || $svc_zip != "") {
            ...
            var $is_la_state {
              value = ($svc_state_raw == "LA") || ($svc_state_raw == "LOUISIANA")
            }
          
            var $is_tn_state {
              value = ($svc_state_raw == "TN") || ($svc_state_raw == "TENNESSEE")
            }
            ...
            conditional {
              if ($region_clean == "tn") {
                var.update $include_row {
                  value = $is_tn_state || ($svc_state_raw == "")
                }
              }
