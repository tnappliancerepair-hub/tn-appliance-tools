...
query assign_parallel_test_job verb=POST {
  ...
  stack {
      ...
      each as $ar {
        conditional {
          if (!$already_assigned) {
            ...
            var $ar_strip {
              value = $ar_meta|replace:$tech_marker:""
            }
          
            conditional {
              if (($ar_meta|strlen) > ($ar_strip|strlen)) {
                var.update $already_assigned {
                  value = true
                }
              }
            }
    ...
    var $candidate_count {
      value = $candidate_rows.items|count
    }
    ...
    var $chosen {
      value = $candidate_rows.items|get:0
    }
