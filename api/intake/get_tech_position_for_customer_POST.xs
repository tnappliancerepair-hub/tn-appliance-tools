...
query get_tech_position_for_customer verb=POST {
  ...
  stack {
    ...
    var $cust_phone_raw {
      value = (($cust.phone ?? "")|replace:"+":""|replace:"-":""|replace:"(":""|replace:")":""|replace:" ":"")
    }
  
    var $cust_phone_len {
      value = $cust_phone_raw|strlen
    }
  
    var $cust_last4_start {
      value = $cust_phone_len - 4
    }
  
    var $cust_last4 {
      value = ($cust_phone_len >= 4) ? ($cust_phone_raw|substr:$cust_last4_start) : ""
    }
    ...
    var $sched_status {
      value = (($job.scheduling_status ?? "")|trim)
    }
  
    var $en_route_at {
      value = ($job.tech_en_route_at ?? 0)
    }
  
    var $started_at {
      value = ($job.job_started_at ?? 0)
    }
  
    var $is_eligible {
      value = ($sched_status == "in_progress") || ($en_route_at > 0 && $started_at == 0)
    }
      ...
      if ($is_eligible) {
        ...
        foreach ($pings.items) {
          each as $p {
            conditional {
              if ($latest_ping_id == 0) {
                var $meta_str {
                  value = ($p.metadata ?? "")|json_encode
                }
              
                var $strip {
                  value = $meta_str|replace:$needle:""
                }
              
                conditional {
                  if (($meta_str|strlen) > ($strip|strlen)) {
                    var.update $latest_ping_id {
                      value = $p.id
                    }
                  
                    var.update $latest_ping_ts {
                      value = $p.created_at
                    }
                  
                    var.update $latest_lat {
                      value = (($p.metadata.lat ?? "")|to_text)
                    }
                  
                    var.update $latest_lng {
                      value = (($p.metadata.lng ?? "")|to_text)
                    }
                  }
                }
              }
        ...
        conditional {
          if ($latest_ping_id > 0) {
            ...
            var $age_sec {
              value = (($now_ms - $latest_ping_ts) / 1000)|round
            }
