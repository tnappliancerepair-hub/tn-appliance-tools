...
// Scout specialist detector — matches scout_<source>_<appliance>
// pattern. The scout army that builds the model_intelligence corpus.
function isScoutSpecialist(agent) {
  const name = String(agent.name || '').toLowerCase();
  const id = String(agent.id || '').toLowerCase();
  const outputs = (agent.outputs || []).join(' ');
  const triggers = (agent.triggers || []).join(' ');
  if (/MODEL_INTELLIGENCE_FINDING|SCOUT_RUN_COMPLETE/i.test(outputs)) return true;
  if (/SCOUT_REQUEST_/i.test(triggers)) return true;
  if (name.startsWith('scout_') || id.startsWith('sc')) return true;
  return false;
}

// Parse "scout_<source>_<appliance>" → { sourceSlug, applianceSlug }
function scoutSlugsFromAgent(agent) {
  const name = String(agent.name || '').toLowerCase().replace(/[^a-z0-9_]/g, '_');
  const m = name.match(/^scout_([a-z0-9]+)_([a-z0-9]+)/);
  if (m) return { sourceSlug: m[1], applianceSlug: m[2] };
  // Fallback: pull from triggers if name doesn't match the pattern.
  const trig = (agent.triggers || []).find((t) => /SCOUT_REQUEST_/i.test(t));
  if (trig) {
    const tm = trig.match(/SCOUT_REQUEST_([A-Z0-9]+)_([A-Z0-9]+)/i);
    if (tm) return { sourceSlug: tm[1].toLowerCase(), applianceSlug: tm[2].toLowerCase() };
  }
  return { sourceSlug: 'webgeneric', applianceSlug: 'general' };
}
...
// Scout specialist — the army of web-scouring agents that build the
// model_intelligence corpus per (source × appliance) pair. Each scout
// enumerates known issues, failure modes, service bulletins, common
// parts, repair tips for its specific source + appliance, then
// persists each finding as a row in model_intelligence. Searchable
// later by diagnose_* + Ant Field Assist.
//
// Signal in: SCOUT_REQUEST_<SOURCE>_<APPLIANCE>
// Signal out: MODEL_INTELLIGENCE_FINDING (per finding) +
//             SCOUT_RUN_COMPLETE (per run summary)
function renderScoutSpecialist({ agent, sourceSlug, sourceDisplay, applianceSlug, applianceDisplay, systemPromptText }) {
  const compoundSlug = `${sourceSlug}_${applianceSlug}`;
  const signalType = `SCOUT_REQUEST_${compoundSlug.toUpperCase()}`;
  const filename = `scout_request_${compoundSlug}.js`;
  const safePrompt = escapeForBacktick(systemPromptText);

  const code =
`// Auto-generated by colony_architect (scout_specialist) on ${new Date().toISOString()}.
// Agent ${agent.id} — ${escapeForBacktick(agent.name || '')}
// Source: ${sourceDisplay}  ·  Appliance: ${applianceDisplay}
// Signal in:  ${signalType}
...
const APPLIANCE_SLUG = '${applianceSlug}';
const APPLIANCE_DISPLAY = '${escapeForBacktick(applianceDisplay)}';
const AGENT_ID = '${agent.id}';

const SCOUT_PROMPT = \`${safePrompt}\`;
...
export async function generateAgent(agent, claude, config) {
  ...
  if (isScoutSpecialist(agent)) {
    const { sourceSlug, applianceSlug } = scoutSlugsFromAgent(agent);
    const sourceDisplay = sourceSlug.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
    const applianceDisplay = applianceSlug.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());

    const resp = await claude.callClaude({
      system: 'You write Claude system prompts for specialist agents. Output only the prompt text — no commentary, no preamble.',
      messages: [{ role: 'user', content: metaPromptForScout(sourceDisplay, applianceDisplay) }],
      model: config.claudeModel,
      maxTokens: 2048,
    });
    const promptText = (claude.textFromResponse ? claude.textFromResponse(resp) : '').trim();
    ...
    if (!promptText || promptText.length < 60) {
      throw new Error('Claude returned empty/short scout prompt for ' + sourceSlug + '_' + applianceSlug);
    }

    return renderScoutSpecialist({
      agent,
      sourceSlug,
      sourceDisplay,
      applianceSlug,
