...
query create_test_tech_job verb=POST {
  ...
  stack {
    var $tech_id_in { value = ($input.tech_id ?? 1) }
    var $appliance_in_raw { value = (($input.appliance ?? "Refrigerator")|trim) }
    var $appliance_clean { value = ($appliance_in_raw == "") ? "Refrigerator" : $appliance_in_raw }
    var $problem_in_raw { value = (($input.problem ?? "Ant Assist test — fridge not cooling, possible compressor")|trim) }
    var $problem_clean { value = ($problem_in_raw == "") ? "Ant Assist test — fridge not cooling, possible compressor" : $problem_in_raw }
    ...
    conditional {
      if (($existing_cust.items|count) > 0) {
        var.update $cust_id { value = ($existing_cust.items|get:0|get:"id") }
      }
      else {
        db.add customer {
          data = {
        ...
        var.update $cust_id { value = $new_cust.id }
      }
    }

    // Schedule the job for 30 min from now so the dashboard shows it
    var $scheduled { value = $now_ms + 1800000 }
    ...
    var $assist_url { value = "https://tnapplianceexchange.net/tech-ant-live.html?job_id=" ~ $job.id ~ "&tech_id=" ~ $tech_id_in }
  }
