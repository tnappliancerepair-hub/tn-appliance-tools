...
query create_job_from_email verb=POST {
  ...
  stack {
    ...
    var $intake_enabled_str { value = (($env.EMAIL_INTAKE_ENABLED ?? "false")|lower) }
    var $intake_enabled {
      value = ($intake_enabled_str == "true")
    }

    var $dry_run_flag { value = ($input.dry_run ?? false) }
    ...
    conditional {
      if (!$intake_enabled && !$dry_run_flag) {
        ...
        return {
          value = {success: false, error: "EMAIL_INTAKE_ENABLED=false", gated: true}
        }
      }
    }

    // Gate 2: forward-only — reject if email older than parser activation
    var $activation_ts { value = (($env.PARSER_ACTIVATION_TS_MS ?? "0")|to_int) }
    ...
    var $intake_src_clean { value = (($input.intake_source ?? "")|trim|lower) }

    var $allowed_sources_csv {
      value = "|email_servicepower|email_ahs|email_allstate|web_chat|manual|"
    }

    var $src_marker { value = ("|" ~ $intake_src_clean ~ "|") }
    var $src_strip { value = $allowed_sources_csv|replace:$src_marker:"" }
    var $src_valid { value = ($intake_src_clean != "") && (($allowed_sources_csv|strlen) > ($src_strip|strlen)) }
    ...
    var $phone_in { value = (($input.customer_phone ?? "")|trim) }
    var $phone_digits { value = $phone_in|replace:"+":""|replace:"-":""|replace:"(":""|replace:")":""|replace:" ":"" }
    var $phone_len { value = $phone_digits|strlen }
    var $last10_start { value = $phone_len - 10 }
    var $phone_last10 { value = ($phone_len >= 10) ? ($phone_digits|substr:$last10_start) : $phone_digits }

    // Claim/dispatch # for dedup
    var $claim_trimmed { value = (($input.claim_number ?? "")|trim) }
    var $disp_trimmed { value = (($input.dispatch_number ?? "")|trim) }
    var $claim_or_disp {
      value = ($claim_trimmed != "") ? $claim_trimmed : $disp_trimmed
    }

    // Dedup: any existing job with matching claim OR matching phone_last4 + city
    var $dedup_found { value = false }
    var $dedup_job_id { value = 0 }
    ...
    conditional {
      if ($claim_or_disp != "") {
        ...
        var $bc_count { value = ($by_claim.items|count) }
        conditional {
          if ($bc_count > 0) {
            var.update $dedup_found { value = true }
            var $bc_first { value = $by_claim.items|get:0 }
            var.update $dedup_job_id { value = $bc_first.id }
          }
        }
      }
    }
    ...
    conditional {
      if ($dedup_found) {
        ...
        return {
          value = {success: true, action: "dedup_skip", existing_job_id: $dedup_job_id}
        }
      }
    }
    ...
    conditional {
      if ($phone_last10 != "") {
        ...
        var $cust_count { value = ($cust_rows.items|count) }
        conditional {
          if ($cust_count > 0) {
            var $cust_first { value = $cust_rows.items|get:0 }
            var.update $cust_id_final { value = $cust_first.id }
          }
        }
      }
    }
    ...
    conditional {
      if ($cust_id_final == 0) {
        ...
        var.update $cust_id_final { value = $new_cust.id }
      }
    }
    ...
    var $dn_last { value = (($input.customer_last_name ?? "")|trim) }
    var $dn_city { value = (($input.service_city ?? "")|trim) }
    var $dn_full_name { value = (($dn_first ~ " " ~ $dn_last)|trim) }
    var $danielle_body {
