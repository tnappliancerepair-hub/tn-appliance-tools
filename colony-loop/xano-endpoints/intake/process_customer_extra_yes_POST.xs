...
query process_customer_extra_yes verb=POST {
  ...
  stack {
    ...
    var $is_yes { value = ($body_u == "YES" || $body_u == "Y" || $body_u == "YEAH" || $body_u == "OK" || $body_u == "OKAY" || $body_u == "SURE") }
    var $is_no { value = ($body_u == "NO" || $body_u == "N" || $body_u == "NOPE" || $body_u == "PASS") }
    ...
    var $plus_e164 { value = "+" ~ $digits_only }
    var $plus_us { value = "+1" ~ $last10 }
    ...
    var $customer { value = (($cust_rows.items|first) ?? null) }
    ...
    var $my_offer { value = null }
    var $my_offer_meta { value = null }

    foreach ($offer_rows.items) {
      each as $or {
        conditional {
          if ($my_offer == null) {
            var $om_raw { value = ($or.metadata ?? "") }
            var $om { value = $om_raw|json_decode }
            conditional {
              if (($om.customer_id ?? 0) == $cust_id) {
                var.update $my_offer { value = $or }
                var.update $my_offer_meta { value = $om }
              }
            }
          }
        }
    ...
    var $offer_tech_id { value = ($my_offer_meta.tech_id ?? 0) }
    var $offer_job_id { value = ($my_offer_meta.job_id ?? 0) }
    ...
    var $existing_winner { value = null }
    var $existing_winner_meta { value = null }

    foreach ($winners.items) {
      each as $wr {
        conditional {
          if ($existing_winner == null) {
            var $wm_raw { value = ($wr.metadata ?? "") }
            var $wm { value = $wm_raw|json_decode }
            conditional {
              if (($wm.tech_id ?? 0) == $offer_tech_id) {
                var.update $existing_winner { value = $wr }
                var.update $existing_winner_meta { value = $wm }
              }
            }
          }
        }
    ...
    conditional {
      if ($existing_winner != null) {
        ...
        var $tech_phone { value = (($tech_for_alert.phone ?? "")|trim) }
        var $tech_first { value = (($tech_for_alert.first_name ?? "")|trim) }
        var $cust_first { value = (($customer.first_name ?? "")|trim) }
        var $loser_appliance { value = (($loser_job.appliance_type ?? "")|trim) }
    ...
    var $book_start_ms { value = $now_ms }
    var $book_end_ms { value = $now_ms + 7200000 }
    ...
    var $book_ok { value = (($book_resp.response.status ?? 0) >= 200 && ($book_resp.response.status ?? 0) < 300) }
    ...
    var $win_tech_phone { value = (($win_tech.phone ?? "")|trim) }
    var $win_tech_first { value = (($win_tech.first_name ?? "")|trim) }
    var $win_cust_first { value = (($customer.first_name ?? "")|trim) }
    var $win_cust_last { value = (($customer.last_name ?? "")|trim) }
    var $win_addr { value = (($win_job.service_address ?? "") ~ " " ~ ($win_job.service_city ?? "") ~ " " ~ ($win_job.service_zip ?? ""))|trim }
    var $win_appliance { value = (($win_job.appliance_type ?? "")|trim) }
