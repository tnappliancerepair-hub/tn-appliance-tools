...
query count_pending_signals_for_job verb=GET {
  ...
  stack {
    var $stype { value = (($input.signal_type ?? "")|trim) }
    ...
    var $oldest_unprocessed_id { value = 0 }
    ...
    foreach ($rows.items) {
      each as $r {
        ...
        var $payload_str { value = ($r.payload ?? "") }
        var $stripped { value = $payload_str|replace:$needle:"" }
        conditional {
          if (($payload_str|strlen) > ($stripped|strlen)) {
            var.update $hit_count { value = $hit_count + 1 }
            conditional {
              if ($oldest_unprocessed_id == 0 || $r.id < $oldest_unprocessed_id) {
                var.update $oldest_unprocessed_id { value = $r.id }
              }
            }
          }
        }
