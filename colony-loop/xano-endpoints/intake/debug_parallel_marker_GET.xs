...
query debug_parallel_marker verb=GET {
  ...
  stack {
    var $lim_raw { value = ($input.limit ?? 5) }
    var $lim { value = ($lim_raw > 50) ? 50 : $lim_raw }
    ...
    foreach ($rows.items) {
      each as $r {
        ...
        var $meta_str_attempt { value = $meta_raw|json_encode }

        var $hit_int { value = false }
        var $hit_str { value = false }
        conditional {
          if ($jid > 0) {
            var $needle_int { value = "\"job_id\":" ~ ($jid|to_text) }
            var $needle_str { value = "\"job_id\":\"" ~ ($jid|to_text) ~ "\"" }
            var $strip_int { value = $meta_str_attempt|replace:$needle_int:"" }
            var $strip_str { value = $meta_str_attempt|replace:$needle_str:"" }
            conditional {
              if (($meta_str_attempt|strlen) > ($strip_int|strlen)) {
                var.update $hit_int { value = true }
              }
            }
            conditional {
              if (($meta_str_attempt|strlen) > ($strip_str|strlen)) {
                var.update $hit_str { value = true }
              }
            }
          }
        }
