...
query get_tech_assist_eod_stats verb=GET {
  ...
  stack {
    ...
    var $today_start_ms { value = $now_ms - (16 * 3600 * 1000) }
    ...
    var $sessions_today { value = ($sess_rows.items|count) }
    ...
    var $saved_today { value = 0 }

    foreach ($saved_rows.items) {
      each as $sv {
        var $meta_raw { value = ($sv.metadata ?? "") }
        var $strip { value = $meta_raw|replace:$tech_marker:"" }
        conditional {
          if (($meta_raw|strlen) > ($strip|strlen)) {
            var.update $saved_today { value = $saved_today + 1 }
          }
        }
      }
    }
    ...
    var $loops_today { value = 0 }

    foreach ($loop_rows.items) {
      each as $lp {
        var $lp_meta { value = ($lp.metadata ?? "") }
        var $lp_strip { value = $lp_meta|replace:$tech_marker:"" }
        conditional {
          if (($lp_meta|strlen) > ($lp_strip|strlen)) {
            var.update $loops_today { value = $loops_today + 1 }
          }
        }
      }
    }
    ...
    var $is_paused { value = false }
    var $found_state { value = "" }

    foreach ($pause_rows.items) {
      each as $pr {
        conditional {
          if ($found_state == "") {
            var $pr_meta { value = ($pr.metadata ?? "") }
            var $pr_strip { value = $pr_meta|replace:$tech_marker:"" }
            conditional {
              if (($pr_meta|strlen) > ($pr_strip|strlen)) {
                var.update $found_state { value = ($pr.action ?? "") }
              }
            }
          }
        }
    ...
    conditional {
      if ($found_state == "tech_assist_paused") {
        var.update $is_paused { value = true }
      }
    }
  }
