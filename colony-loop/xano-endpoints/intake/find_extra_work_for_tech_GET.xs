...
query find_extra_work_for_tech verb=GET {
  ...
  stack {
    var $lim_raw { value = ($input.limit ?? 3) }
    var $lim { value = ($lim_raw > 5) ? 5 : $lim_raw }
    ...
    var $tech_id_marker { value = ("," ~ ($input.tech_id|to_text) ~ ",") }
    var $tech_clusters_csv { value = "" }

    foreach ($all_zones.items) {
      each as $z {
        var $allowed_padded { value = ("," ~ (($z.allowed_technicians ?? "")|trim) ~ ",") }
        var $strip { value = $allowed_padded|replace:$tech_id_marker:"" }
        conditional {
          if (($allowed_padded|strlen) > ($strip|strlen)) {
            var $cl { value = (($z.cluster ?? "")|trim|lower) }
            conditional {
              if ($cl != "") {
                var.update $tech_clusters_csv { value = ($tech_clusters_csv ~ "|" ~ $cl ~ "|") }
              }
            }
          }
        }
    ...
    var $declined_appliances { value = [] }
    var $declined_zones { value = [] }

    foreach ($tech_prefs.items) {
      each as $p {
        var $notes_txt { value = (($p.notes ?? "")|trim) }
        conditional {
          if (($notes_txt|substr:0:13) == "appliance_no:") {
            var $app_slug { value = ($notes_txt|substr:13)|trim|lower }
            var.update $declined_appliances { value = $declined_appliances|push:$app_slug }
          }
          elseif ($notes_txt == "zone_skip") {
            var $zn_slug { value = (($p.zip_or_area ?? "")|trim|lower) }
            conditional {
              if ($zn_slug != "") {
                var.update $declined_zones { value = $declined_zones|push:$zn_slug }
              }
            }
          }
        }
    ...
    var $matches { value = [] }
    var $with_prediag_first { value = [] }
    var $without_prediag { value = [] }
      ...
      each as $job {
        conditional {
          if (($matches|count) < 20) {
            ...
            var $app_lower { value = (($job.appliance_type ?? "")|trim|lower) }
            var $app_strip { value = $declined_appliances|to_text|replace:$app_lower:"" }
            var $app_blocked { value = (($declined_appliances|to_text|strlen) > ($app_strip|strlen)) && ($app_lower != "") }
              ...
              if (!$app_blocked) {
                ...
                conditional {
                  if ($zip != "") {
                    ...
                    var $zone_row { value = (($zone_rows.items|first) ?? null) }
                    var $cluster_lower { value = (($zone_row.cluster ?? "")|trim|lower) }
                    var $cluster_strip { value = $declined_zones|to_text|replace:$cluster_lower:"" }
                    var $cluster_blocked { value = (($declined_zones|to_text|strlen) > ($cluster_strip|strlen)) && ($cluster_lower != "") }
                    ...
                    conditional {
                      if (!$cluster_blocked) {
                        ...
                        var $has_prediag { value = false }
                        var $owned_by_other_tech { value = false }

                        foreach ($tdr_all.items) {
                          each as $tdr {
                            var $tid { value = ($tdr.technician_id ?? 0) }
                            conditional {
                              if ($tid == 1) {
                                var.update $has_prediag { value = true }
                              }
                              elseif ($tid > 1) {
                                var.update $owned_by_other_tech { value = true }
                              }
                            }
                          }
                        }
                        ...
                        conditional {
                          if (!$owned_by_other_tech) {
                            ...
                            var $cluster_marker { value = ("|" ~ $cluster_lower ~ "|") }
                            var $tcs_strip { value = $tech_clusters_csv|replace:$cluster_marker:"" }
                            var $in_tech_cluster { value = ($cluster_lower != "") && (($tech_clusters_csv|strlen) > ($tcs_strip|strlen)) }
                            ...
                            conditional {
                              if ($has_prediag && $in_tech_cluster) {
                                var.update $with_prediag_first { value = $with_prediag_first|push:$summary }
                              }
                              elseif ($has_prediag) {
                                // Has prediag but outside cluster — second priority
                                var.update $without_prediag { value = $without_prediag|push:$summary }
                              }
                              else {
                            ...
                            var.update $matches { value = $matches|push:$summary }
                          }
                        }
                      }
                    }
