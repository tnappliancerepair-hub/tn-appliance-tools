...
query find_nearby_open_jobs verb=GET {
  ...
  stack {
    ...
    conditional {
      if ($limit_val > 50) { var.update $limit_val { value = 50 } }
    }
    conditional {
      if ($limit_val < 1)  { var.update $limit_val { value = 1 } }
    }
      ...
      each as $j {
        ...
        conditional {
          if ($score > 0) {
            ...
            var $cust_first { value = (($cust.first_name ?? "")|trim) }
            var $cust_last  { value = (($cust.last_name ?? "")|trim)  }
            var $cust_full  { value = (($cust_first ~ " " ~ $cust_last)|trim) }
            ...
            var $cust_name_final { value = ($cust_full != "") ? $cust_full : "(unknown)" }
            var $cust_phone_v    { value = ($cust.phone ?? "") }
            var $svc_address_v   { value = ($j.service_address ?? "") }
            var $svc_city_v      { value = ($j.service_city ?? "") }
            var $svc_zip_v       { value = ($j.service_zip ?? "") }
            var $appl_type_v     { value = ($j.appliance_type ?? "") }
            var $brand_v         { value = ($j.brand ?? "") }
            var $warranty_v      { value = ($j.warranty_company ?? "") }
            var $status_v        { value = ($j.scheduling_status ?? "") }
            var $parts_status_v  { value = ($j.parts_status ?? "") }
            var $ready_now { value = false }
            conditional {
              if ($parts_status_v == "arrived" || $parts_status_v == "" || $parts_status_v == "not_needed") {
                var.update $ready_now { value = true }
              }
            }
            ...
            var.update $scored { value = $scored|push:$entry }
          }
        }
      }
    }
