  ...
  stack {
    ...
    foreach ($rows.items) {
      each as $r {
        ...
        var $include { value = false }
        conditional {
          if ($vendor_in == "") {
            var.update $include { value = true }
          }
          else {
            // Substring match on the warranty_company field inside the JSON.
            var $needle { value = "\"warranty_company\":\"" ~ $vendor_in ~ "\"" }
            var.update $include { value = ($meta_str|to_text|contains:$needle) }
          }
        }
        conditional {
          if ($include == true) {
            ...
            var.update $filtered { value = $filtered|push:$entry }
          }
        }
      }
    }
