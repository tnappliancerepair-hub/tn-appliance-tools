...
query get_pricing_modifier verb=GET {
  ...
  stack {
      ...
      if ($sched_in != "") {
        ...
        conditional {
          if ($diff_hours <= 24 && $diff_hours >= 0) {
            var.update $surge_pct { value = 25 }
            var.update $surge_reason { value = "same_day" }
          }
          elseif ($diff_hours <= 48 && $diff_hours >= 0) {
            var.update $surge_pct { value = 10 }
            var.update $surge_reason { value = "next_day" }
          }
        }
      }
    }
    ...
    conditional {
      if (($input.is_emergency ?? false) == true) {
        var.update $surge_pct { value = $surge_pct + 25 }
        var.update $surge_reason { value = $surge_reason ~ "+emergency" }
      }
    }
      ...
      if (($input.customer_id ?? 0) > 0) {
        ...
        conditional {
          if (($cust_completed ?? 0) >= 5) {
            var.update $discount_pct { value = 5 }
          }
        }
      }
    }
