...
query cleanup_duplicate_name_customers verb=POST {
  ...
  stack {
    var $dryrun { value = (($input.dry_run ?? true) != false) }
    ...
    var $hits { value = [] }
    var $fixed_count { value = 0 }

    foreach ($all.items) {
      each as $c {
        var $f { value = (($c.first_name ?? "")|trim) }
        var $l { value = (($c.last_name ?? "")|trim) }
        conditional {
          if ($f != "" && $f == $l) {
            ...
            var.update $hits { value = $hits|push:$hit }
            ...
            conditional {
              if (!$dryrun) {
                ...
                var.update $fixed_count { value = $fixed_count + 1 }
              }
            }
          }
        }
