...
(function (w) {
  ...
  function render(job, opts){
    ...
    if (bits.length){
      ...
      return '<div style="'+base+';'+hc+'">🕒 ' + (opts.style==='panel' ? '<b>Scheduling constraints</b> ' : '') + bits.join(' · ') + '</div>';
    }
    ...
    return '<div style="'+base+';'+nc+';font-weight:600">🕒 no time info — gather at intake</div>';
  }

  w.AntConstraints = { parse: parse, render: render };
})(window);
